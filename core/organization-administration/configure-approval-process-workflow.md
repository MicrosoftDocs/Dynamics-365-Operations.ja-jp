---
title: "作業の現在の承認プロセスをコンフィギュレーションします。"
...
# <a name="configure-an-approval-process-in-a-workflow"></a>作業の現在の承認プロセスをコンフィギュレーションします。

承認プロセスのプロパティをコンフィギュレーションするには、次の手順に従います。

承認プロセスは、作業の現在エディタでコンフィギュレーションするには、承認の要素を右クリックし、[開くに**プロパティ** **プロパティ**をクリックします。
承認プロセスに名前を付ける
-------------------------
...
9.  通知の送信先ユーザーを指定します。 次の表のいずれかのオプションを選択し、オプションの追加手順を実行してから、手順 10 に進みます。
    ...
    <li><strong>使用できるユーザー</strong>: リストは、工程のユーザーのすべてのMicrosoft Dynamics 365が含まれます。 通知の送信先のユーザーを選択して、これらのユーザーを [<strong>選択されたユーザー</strong>]: リストに移動します。</li>
    </ol></td>
    </tr>
    </tbody>
    </table>
...
[**注記**]: [**エンタープライズ ポータルの作業一覧からのアクションの有効化**] チェック ボックスは使用されなくなりました。
...
3.  承認ステップをコンフィギュレーションするには、[承認ステップのコンフィギュレーション](http://axhelp.dynamics.com/en/wiki/configure-an-approval-step/)を参照してください。
