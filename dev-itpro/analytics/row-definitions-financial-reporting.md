...
-   工程**メモのMicrosoft Dynamics 365の分析コード値を含めることができる財務データへのリンク、: **レポートが生成されるように財務分析コード システムからデータを取得するたびに行定義を設定できます。
...
| オプション                | 説明                                                                                                                                                                                                                                                                      |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 分析コード             | 行定義に追加する分析コードを識別するパターンです。 このパターンに一つのアンパサンド (&) または分析コードの各職位の番号記号 (\#) があります。 通常、[主勘定分析コード] にはすべてアンパサンドを、他の分析コードにはすべて番号記号を使用します。 |
...
4.  行定義を特定の範囲の分析コード値に限定するには、[**分析コード範囲の開始**] セルに開始分析コード値を、[**分析コード範囲の終了**] セルに終了分析コード値を入力します。 選択した分析コードのすべての値を対象とする場合は、これらのセルは空白のままにします。 **注記: **分析コードの範囲のワイルドカード文字 (\* または場合)。戻さないERPのデータベースのデータの設定方法に応じてによっては、目的のすべての結果をかもしがあります。
5.  [**開始行コード**] フィールドで、行定義に追加する最初の分析コード値の行コードを指定します。
6.  [**行の増分**] フィールドで、連続する行コード間の間隔を指定します。 たとえば、最初の行コードが100の場合、インクリメント値30は、最初の新しい行コードを持っている100、130、160、190、および220です。 新しいフォームおよび式の行を挿入するために十分な領域を提供するインクリメント値を使用します。
...
3.  適用するフォーマットのツール バーのボタンをクリックします。 たとえば、行をインデントするには、行を選択し、[]をクリックします。**見越計上のインデント**。[見越し計上インデント] (https://i-technet.sec.s-msft.com/dynimg/IC679497.gif 「増加インデント」) ツール バーの。
