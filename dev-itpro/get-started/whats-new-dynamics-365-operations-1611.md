...
<tr class="even">
<td>Excel を使用してレポートを作成する。 <strong>注:</strong> Microsoft Excel 2016 を実行する必要があります。</td>
...
| 経費精算書に関連付けられていない経費の作成および管理。    | 最初に経費精算書を作成することなく、(たとえば、領収書を添付するまたは明細化する、またはゲストを割り当てることによって)、ユーザーは経費を作成および維持できるようになりました。 承認のために送信できるように、1 つ以上の経費が選択でき、新しい経費精算書に追加できます。 管理の経費の新しいページが**経費管理** **自分の経費 &gt; に使用する** &gt; **経費**。                                                                                                                                                                                                                                                       |
...
<td>以前は、2 つの別のタイプの評価は固定資産価値を追跡するのに使用されました: 価値モデルと減価償却簿です。 現在のバージョンでは、これらの 2 つの概念は帳簿という名前の 1 つの評価概念にマージされています。 帳簿には価値モデルおよび減価償却簿の両方のすべての機能があります。 たとえば、総勘定元帳への転記をオフにできます。 総勘定元帳に転記する帳簿は、会社の財務報告に通常使用されます。 総勘定元帳に転記しない帳簿は、税務報告のために使用できます。</td>
</tr>
<tr class="even">
...
<li>再評価プロセスを実行するための主要勘定と通貨の範囲を選択することがより簡単になりました。</li>
...
| アクセス5新しいPower BIの内容は、人事管理に詰まります                                     | 新しいコンテンツPackは、人事管理組織、人事管理リソース マネージャが次を分析することができます: 従業員のメトリック • 期間、性別、配偶者の有無による人口統計学の詳細 • 年における加害率年を比較し、組織のままについては、理由の従業員の一覧を与えるを参照してください • 開職位の一覧、終了位置の報酬とその利点に対する未処理の比較を表示します。• 時間レートとサラリー従業員を比較します。• 使用可能な利点に登録されている従業員数を表示します。• 雇用タイプの採用して、従業員を表示します。• から来ている場合、職位は組織のトレーニングを適用する申請者の数に基づいて申請者を分析する • 場所に応じて登録をコース。• ステータス別Couseの出席 • 従業員の一覧は、コースへの従業員の能力や開発に登録される • 従業員が保持するスキルの一覧 • チーム メンバ別のスキル タイプの詳細 |
...
<td>Dynamics 365 for Operations は 18 の追加国で使用できます。 ローカライズを容易にし、コンフィギュレーションを可能にする Microsoft の労力の一部として、規制に関する電子レポート機能は電子申告 (ER) のコンフィギュレーションに変換され、規制に関する Microsoft SQL Server Reporting Services (SSRS) のレポートは、Excel テンプレートを使用する ER コンフィギュレーションに変換されます。 これらの変換された機能は、後にこの表で説明されます。</td>
</tr>
<tr class="even">
...
<td>登録 ID のフレームワークと新しい <strong>VAT ID</strong> 登録カテゴリを使用して、当事者の住所に対して VAT 登録番号を設定することができるようになりました。</td>
</tr>
<tr class="even">
<td>フィンランド – 登録 ID フレームワークを使用して当事者の住所に対して税関顧客番号を設定します。</td>
<td>登録 ID のフレームワークと新しい<strong>税関顧客 ID</strong> 登録カテゴリを使用して、当事者の住所の税関顧客番号を設定することができます。</td>
</tr>
<tr class="odd">
...
<td>銀行 (そのほか) LC。 サウジアラビアの雑費の配賦のフォーム</td>
</tr>
<tr class="odd">
...
<td>給与支払における現金および銀行管理の確認後支払機能に新しい拡張が追加されました。 別のメニュー項目がコア プロセス全体を通して、給与固有の独立しているコンフィギュレーションを有効にするために追加されました。 機能は、Microsoft Dynamics AX アプリケーション バージョン 7.0.1 (2016 年 5 月) にリリースされたコア確認後支払機能と同じです。 この拡張により、給与データは他の確認後支払トランザクションからは完全に独立しています。 この分離は、給与ユーザーのみが給与に関連するデータにアクセスでき、表示できるということを保証するのに役立ちます。</td>
</tr>
<tr class="even">
...
| 実行可能事項                                                                                                                                                                                                      | これが重要である理由                                                                                                                                                                                                                                                                                                                                                                                                                 |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 製品マスターの有効な製品分析コードに基づいて、販売と購買の既定注文設定を設定します。 したがって、最小在庫管理単位 (SKU) または部分的 SKU の既定注文設定を定義できます。 | 製品分析コードまたは製品分析コードの組み合わせに適用される既定注文設定を定義できます。 **例Polo ** T-shirtという名前の製品を販売します。 この製品は、緑色と青色で利用可能です。 また、小と中の 2 サイズで展開されています。 色とサイズは Polo T-shirt の有効な製品分析コードです。 サイズに関係なく、すべての緑色 Polo T-shirts の購入をブロックできます。 |
...
<td>この機能は、生産性の拡張機能です。 <strong>Example</strong> You sell a product that is named Polo T-shirt. この製品は、緑色と青色で利用可能です。 また、小と中の 2 サイズで展開されています。 色とサイズは Polo T-shirt の有効な製品分析コードです。 緑色でサイズ小の Polo T-shirt の販売明細行を入力する場合、<strong>品目番号</strong>、<strong>色</strong>、および<strong>サイズ</strong> フィールドにデータを入力する必要はありません。 これらの手順の 1 つを実行し、明細行を作成できます:
<ul>
...
### <a name="personalized-product-recommendations"></a>カスタマイズされた製品の推奨事項

| 実行可能事項                                                                                                                                                                                                                                                                        | これが重要である理由                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 別の顧客は、オンラインと実店舗で購入した顧客が、品目欲しい基づく品目のリストに必要な発注の履歴、品目にかもしがあります。を決定するPOS (POS)の複数のtouchpointsでカスタマイズされた製品の推奨事項を表示します。 | 大規模カタログの小売業者に対して、カスタマイズされた製品の推奨事項が検出の顧客情報を確認し、処理機能をもったclientelingの店舗の関連にアクセス許可を提供します。 顧客の対象に、購買既に対象となる製品の展示して製品の推奨事項をupsellの小売業者を確認し、顧客の管理を高めることができます。 Dynamics 365 for Operationsで、[小売し、製品の推奨事項が認識サービスおよびMicrosoftのAzureの機械学習によって動力が割り当てられます。 |
...
### <a name="universal-windows-platform-app-for-retail-modern-pos"></a>Retail Modern POS の Universal Windows Platform アプリ
...
| Dynamics 365 for Operations の委託サポートの詳細を表示します。                                                                                                         | 委託販売プロセスのサポートに関する詳細については、[委託] (/dynamics365/operations/scm/inventory/consignment)、[設定の委託] (/dynamics365/operations/scm/inventory/setup委託) 表示します ([委託の補充注文 (作業ガイドを作成する) ] (http://ax.help.dynamics.com/en/wiki/create-a-consignment-replenishment-order/)、[、生産の要求 (作業ガイド基づいて委託の在庫の所有権の変更。) ] (http://ax.help.dynamics.com/en/wiki/change-the-ownership-of-consignment-inventory-based-on-production-demand/)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
<tbody>
<tr class="odd">
<td>リクエストがどこから送信される必要があり、どのように送信される必要があるかを販売注文明細行に対して直接定義します。 <strong>例</strong> 販売注文明細行を作成し、配送タイプとして直納を指定します。 主要仕入先はソーシング ポイントとして販売注文明細行に追加されます。</td>
...
<tr class="even">
<td>注文約定書を販売注文明細行から直接作成します。 <strong>例</strong> 会社間仕入先から提供される販売注文明細行を作成します。 行を残します。 配送日は調達先の会社の利用状況に基づいて計算されます。</td>
