...
## <a name="install-the-oneclick-designer"></a>oneclickデザイナをインストールします。
1.  、Microsoft Dynamics 365では、**情報を表示するのに** **左上とコマース]に**メニューを使用して、**設定されているチャンネル** &gt; **設定されているPOS ** &gt; ** ** &gt; ** POS画面レイアウト**。
...
5.  インストール完了後に、デザイナーを開始する場合、**サインイン**ページで Microsoft Dynamics 365 for Operations のユーザー名とパスワードを入力し、[**サインイン** ] をクリックします。
6.  ログイン情報が認証されデザイナーを開始した後に、独自のレイアウトを作成するか、または既存のレイアウトを変更することができます。 [ワンクリック デザイナの![レイアウト] (。/media/screenlayoutdesign_mposdownload-1024x664.png) ] (。/media/screenlayoutdesign_mposdownload.png) 
...
-   [**デザイナー**] をクリックしても、インストーラーをダウンロード (または実行) するプロンプトが表示されません、または現在のセキュリティ設定によりファイルがダウンロードできません。 **ソリューション**
    -   Internet Explorer で、ポップアップ ブロックがこのサイトに対して無効であることを確認します。 変更が必要 &gt; な場合、[ &gt; **設定** &gt; **オプション** **プライバシー** **ポップアップ ブロッカーあります。**、設定を変更します。
    -   Internet Explorerで、Dynamics 365 for Operations の URL を信頼済みサイトへ追加します。 [**設定** &gt; **オプション** &gt; **セキュリティ** &gt; **信頼されたサイト** &gt; **サイト** &gt; **追加します。**。
-   プログラムが起動せず、仕入先に連絡するように指示されます。 **ソリューション:** Internet Explorer で、Dynamics 365 for Operations の URL を信頼済みサイトへ追加します。 [**設定** &gt; **オプション** &gt; **セキュリティ** &gt; **信頼されたサイト** &gt; **サイト** &gt; **追加します。**。
