---
title: "組織と組織階層の概要 (商業用) "
description: "コマースの概要に組織にするのに役立つ業務プロセスを実行するか、または目標を達成するために定義できる組織の内部3つのタイプがあります。"
author: josaw1
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="organizations-and-organizational-hierarchies-commerce-essentials"></a>組織と組織階層の概要 (商業用) 

コマースの概要に組織にするのに役立つ業務プロセスを実行するか、または目標を達成するために定義できる組織の内部3つのタイプがあります。 
...
Commerce エッセンシャルでは、各階層に目的が割り当てられます。 階層の目的により、階層に含めることができる組織のタイプが決まります。 この目的により、どのアプリケーションのシナリオで階層を使用できるかが決まります。 小売店舗の製品を使用している場合でも、たとえば、小売階層が使用できます。 各階層の組織では、パラメータ、ポリシー、およびトランザクションを共有できます。 親組織のパラメータは、各組織で継承または上書きできます。 ただし、製品やアドレス帳などの共有マスタ データは組織全体に適用され、個々の組織では上書きすることはできません。
