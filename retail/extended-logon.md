---
title: "MPOSおよびクラウドPOSのログオン拡張機能を設定します。"
description: "この wiki では、クラウド POS と Retail Modern POS (MPOS) の拡張ログオンを設定するためのオプションについて説明します。"
author: josaw1
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="set-up-extended-logon-functionality-for-cloud-pos-and-mpos"></a>MPOSおよびクラウドPOSのログオン拡張機能を設定します。

この wiki では、クラウド POS と Retail Modern POS (MPOS) の拡張ログオンを設定するためのオプションについて説明します。

<a name="setting-up-extended-logon"></a>拡張ログオンの設定
=========================

** [小売りとコマース** **設定されているチャンネルで**バーコード マスクの &gt; 設定を参照してください &gt;。**設定されているPOS ** &gt; ** POSのプロファイル** &gt; **機能プロファイル**。 [**機能**] クイック タブには、拡張ログオンに関連付けられる次のオプションが含まれています。
...
既定では、マネージャのみが作業者に拡張ログオンを割り当てることができます。 拡張ログオンを割り当てるには、拡張ログオン** POS.では**入力します。 [検索]フィールドで自身のオペレーターIDを入力すると、ワーカーを検索します。 作業者を選択し、[**割り当て**] をクリックします。 次のページで、作業者に割り当てる拡張ログオンを機械に通すかスキャンします。 読み取りまたはスキャンが正常に読み取られた場合、[**OK**] ボタンが使用できるようになります。 その作業者の拡張ログオンを保存する場合、[**OK**] をクリックします。
