...
ms.locfileid: "7920701"
---
# <a name="quantity-exceeds-under-delivery-percentage-during-packing-slip-generation"></a>梱包明細の生成中に数量が過少配送率を超えている場合

エラーコード: SYS24921

## <a name="symptoms"></a>現象

梱包明細を生成すると、出庫積荷に過少配送率を超える数量が含まれます。

梱包明細を生成しようとすると、次のエラー メッセージが表示されます:

> ラインの部分納品率は %1 パーセントですが、部分納品許可率は %2 パーセントだけです。

したがって、積荷の梱包明細を生成できません。

## <a name="cause"></a>原因

積荷または出荷の選択された数量が注文済数量を下回り、過少配送率の範囲内ではありません。
...
### <a name="adjust-the-under-delivery-percentage"></a>過少配送率を調整する

次の手順を使用して、過少配送率を調整します。

1. **売掛金勘定 \> 注文 \> すべての注文** の順に移動します。
1. 積荷の梱包明細を転記できない販売注文を選択します。
1. **販売注文明細行** タブで、過少配送率を超える品目の販売注文明細行を選択します。
1. **明細行の詳細** タブで、**配送** を選択します。
1. **過少配送** フィールドを、積荷数量に対してピッキングされた数量に対応するよりも大きな割合に設定して、梱包明細の生成が続行されるようにします。
...
1. アクション ペインの、**出荷と入荷** タブの、**取消** グループで、**梱包明細のキャンセル** を選択します。
...
1. **倉庫管理 \> 積荷 \> すべての積荷** に移動します。
1. アクション ペインの **積荷** タブの、**作業** グループで、**作業の取消** を選択します。
