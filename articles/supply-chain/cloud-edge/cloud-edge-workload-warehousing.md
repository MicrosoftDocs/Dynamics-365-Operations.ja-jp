...
現在スケール ユニットでは、その他のタイプのソース ドキュメント処理または倉庫作業はサポートされていません。 たとえば、スケール ユニットの WES ワークロードの場合、移動オーダー入庫プロセス (移動入庫) またはプロセス循環棚卸作業を実行できません。

> [!NOTE]
> モバイル デバイスのメニュー項目とサポートされていない機能のボタンは、スケール ユニット展開に接続されている場合、_倉庫アプリ_ には表示されません。
...
- カスタム作業タイプによる倉庫作業処理
- 出荷メモによる倉庫作業処理
- 循環棚卸しきい値トリガーによる倉庫作業処理
- 材料取り扱い/Warehouse Automation による倉庫作業処理
- 製品マスター データ画像の使用 (倉庫アプリでの使用など)
...
| 発注書製品受領処理                          | あり | なし |
| 過少出荷による発注書受入                        | <p>はい、倉庫オーダーがある場合</p><p>いいえ、倉庫オーダーがある場合</p> | いいえ、キャンセルできるのは倉庫注文明細行の全数量のみです |
