...
description: この記事では、スケール ユニット ワークロードのメッセージ プロセッサ メッセージ機能に関する情報を提供します。
author: perlynne
ms.date: 04/22/2021
ms.topic: article
ms.prod: ''
...
- **メッセージ コンテンツ** – このフィルターはメッセージの内容を全文検索します。 (グリッドにメッセージ コンテンツは表示されません。) このフィルターでは、ほとんどの特殊な記号 (「-」など) をスペースとして処理し、すべての空白文字をブール OR 演算子として処理します。 たとえば、「USMF-123456」 と等しい特定の `journalid` 値を検索すると、システムは 「USMF」 または 「123456」 を含むすべてのメッセージを検索します。これは長いリストである可能性があります。 したがって、「123456」と入力した方がより正確な結果が得られるでしょう。
...
スケール ユニットの分散ハイブリッド トポロジを実行する場合、処理に対して新しいメッセージが作成される際に *メッセージ プロセッサ* のバッチ ジョブが自動的に呼び出されるので、このジョブを手動でスケジュールする必要はありません。
