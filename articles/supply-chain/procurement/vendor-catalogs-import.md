---
title: 仕入先カタログのインポート
description: この記事では、仕入先カタログ データをインポートするプロセスについて説明します。
...
5. **調達** \> **カタログ** \> **仕入先カタログ** を選択して、仕入先のカタログを作成します。 仕入先から受け取るカタログ保守要求 (CMR) ファイルがこのカタログでグループ化されます。
