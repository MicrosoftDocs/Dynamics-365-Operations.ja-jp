...
作業指示書の予測明細行を追加または削除できる場合があります。 ワーク オーダーのライフサイクル状態の設定、関連するプロジェクト タイプ、およびプロジェクト タイプに関連するステージ ルールにより、仕訳帳明細行を追加または編集できるかどうかが決まります。 ワーク オーダーのライフサイクル状態と、関連するプロジェクト ステージの詳細については、[プロジェクト管理および会計への統合](../integration-to-project-management-and-accounting/forecasts-work-orders-and-projects.md)を参照してください。
...
7. 消費仕訳帳を転記した後は、ワーク オーダーのライフサイクル状態を更新 (たとえば、「終了」など) して、ワーク オーダーが完了したことを示すことができます。

- **ワーク オーダー仕訳帳**ページの上部にある**表示**フィールドで、表示する仕訳帳明細行を、すべて、未転記、または転記済から選択します。 転記済仕訳帳の場合は、**転記済**チェック ボックスがオンになります。  
...
>[!NOTE]
>3 つの登録タイプすべてについて、財務分析コードの組み合わせが検証され、無効な組み合わせは空白になります。 これは、Dynamics 365 for Finance and Operations の標準設定です。
