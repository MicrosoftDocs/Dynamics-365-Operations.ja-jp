...
2. **資産消費**ダイアログでは、該当するトグル ボタンで "はい" を選択して表示するパラメータおよび詳細レベルを選択し、**表示**セクションで機能的な場所レベルを挿入します。
    - **レベル** フィールドを使用すると、機能の場所に関する資産明細行の詳細度を指定できます。 たとえば、フィールドに数値 "1" を挿入し、複数レベルの機能的な場所の構造がある場合、機能的な場所に対するすべての資産が最上位レベルに表示されます。そのため、明細行が下位レベルにある機能的な場所から追加されます。 **レベル** フィールドに数値 "0" を挿入すると、関連するすべての機能の場所レベルのすべての資産を示す詳細結果が表示されます。 
