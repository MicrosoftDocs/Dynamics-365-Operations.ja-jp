...
- **CDS 環境サブドメイン** - このフィールドには、すでに値が表示されているはずです。 このフィールドには、ガイドを作成する Common Data Service 環境のサブドメインが含まれています。 サブドメインは URL の最初の部分であり、通常は組織の後に名前が付けられます。 たとえば、Common Data Service URL が "contoso.crm4.dynamics.com" の場合、ここで *contoso* を入力する必要があります。 この値は、ガイドのアドレスを作成するために使用され、QR コードにエンコードされます。
