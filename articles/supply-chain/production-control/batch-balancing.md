...
**在庫バッチ属性**ページで、バッチの有効成分の実際値を検索して編集することができます。 

-  **在庫管理** \> **照会およびレポート** \> **追跡用分析コード** \> **バッチ** \> **在庫バッチ属性**を選択します。
...
成分タイプが**なし**のときにバッチ バランシング プロセスを適用すると、バッチ注文の見積数量およびフォーミュラー明細行の残高数量は同じになります。
...
1.  **製品情報管** \> **表、部品およびフォーミュラ** \> **フォーミュラ**を選択し、フォーミュラ明細行を開きます。そして**補償原則**ページを開くために**成分**を選択します。

2.  補償原則を表す明細行を選択し、補正する有効成分を選択します。
...
3.  見積済の補償数量は、補償数量残高を決定するために –4.72 が差し引かれます。
...
成分タイプが**有効**のフォーミュラ明細行を少なくとも 1 つ含む場合、バッチ オーダがバッチ バランシングに適用されるというルールに対して例外が 1 つあります。

倉庫管理プロセスに対して有効な製品の有効成分がフォーミュラに含まれていても、バッチ番号が引当階層の場所の下にある場合、バッチ注文はバッチ バランシングには適用されません。
