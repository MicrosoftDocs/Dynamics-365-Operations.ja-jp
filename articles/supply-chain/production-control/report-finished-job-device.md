...
作業者は、ジョブ カード デバイス上の **レポートの進捗情報** ページを使って、生産ジョブで完了した数量を報告します。
...
次のサブセクションでは、バッチ品目に関するレポートの 3 つのシナリオをサポートするために、番号グループの追跡を設定する方法について説明します。

### <a name="enable-batch-number-reporting-on-the-job-card-device"></a>ジョブ カード デバイスのバッチ番号の報告を有効にする
...
1. **一般** クイックタブで、**手動** オプションを **はい** に設定します。
    ...
    ![追跡番号グループ ページ](media/tracking-number-group-manual.png "追跡番号グループ ページ")
...
1. **数量ごと** フィールドを使って、入力した値に基づいて、数量ごとにバッチ番号を分割します。 たとえば、10 個の製造オーダーがあり、**数量ごと** フィールドが *2* に設定されているとします。 この場合、製造オーダーを作成すると、5 つのバッチ番号が製造オーダーに割り当てられます。
    ...
    ![追跡番号グループ ページ](media/tracking-number-group-predefined.png "追跡番号グループ ページ")
...
1. **手動**オプションを **いいえ** に設定します。
    ...
    ![追跡番号グループ ページ](media/tracking-number-group-fixed.png "追跡番号グループ ページ")
