---
title: サプライ チェーンのカレンダーおよびマスター プランへの影響
...
ms.locfileid: "363640"
---
# <a name="supply-chain-calendars-and-how-they-affect-master-planning"></a>サプライ チェーンのカレンダーおよびマスター プランへの影響

[!include [banner](../includes/banner.md)]

このトピックでは、サプライ チェーンのカレンダーおよびマスター プランへの影響の概要を示します。  計画オーダーの出入荷日にどう影響するかを含む、マスター プラン エンジンで使用される異なるカレンダーについて説明されます。 最後に、カレンダーの割り当て、使用、および更新に関して推奨事項が示されます。
...
補充グループ カレンダーを別のページで設定でき、次の順序で優先されることに注意してください。
    ...
    2. **品目補充**ページの品目補充グループ
    3. **マスター プラン パラメーター**の既定の品目補充グループ
...
計画移動オーダーの出荷日は、「移動元」倉庫から商品が出荷される日付を示します。 出荷可能日を指定するために使用されるカレンダーは、優先順位別に一覧表示されます。 
    1. 「移動元」倉庫の倉庫カレンダー
    2. 補充グループ カレンダー (上記のこのカレンダー上の予備注文を参照) 倉庫カレンダー セットがある場合、出荷日はカレンダーのオープン日になります。 倉庫カレンダー セットがない場合は、補充グループ カレンダーが使用されます。 
