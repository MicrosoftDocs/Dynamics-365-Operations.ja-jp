---
title: 在庫予測
description: この記事では、Microsoft Dynamics 365 Supply Chain Management で在庫予測を作成するために使用できる供給と需要の予測機能について説明します。
author: t-benebo
ms.date: 06/08/2021
ms.topic: article
...
ms.locfileid: "8844528"
---
# <a name="inventory-forecasts"></a>在庫予測

[!include [banner](../includes/banner.md)]

この記事では、在庫予測を表示および作成する方法について説明します。 品目、品目グループ、品目配賦キー、顧客勘定、顧客グループ、仕入先勘定、仕入先グループの供給予測明細行と需要予測明細行を作成、表示できます。
...
1. 次の表に示したように、予測を作成するエンティティのタイプ、および作成する予測のタイプに応じて、供給、需要、または在庫予測ページを開きます。
...
1. 新しい明細行の **モデル** フィールド で、使用する予測モデルを選択します。 その後、必要に応じて、品目、品目グループ、顧客か仕入先の勘定、またはグループ、品目の数量、またはトランザクションの合計金額などのその他の詳細を入力します。 **供給予測** ページと **需要予測** ページで使用できるフィールドの詳細については、この記事の後のセクションを参照してください。
...
供給予測は、品目、品目グループ、品目配賦キー、仕入先、および仕入先グループ別に入力できます。 さまざまなエンティティやレコードの **供給予測** ページを開く方法の詳細については、この記事の前半の [予測明細行の表示と手動入力](#manual-entry) を参照してください。
...
| コマンド | 説明 |
|---|---|
| 予測の配賦 | 配賦方法を使用する場合は、予測トランザクションの個々のスケジュール行を生成します。 行の数量は、選択した時間間隔、数量、および期間全体の金額に従って、日付別に分配されます。 (この記事で後述の [予測の割り当て](#allocate-forecast) セクションをご覧ください。) |
| 一括更新 | **予測トランザクションの編集** ページを開きます。 (この記事の後の [予測トランザクションの一括更新](#bulk-update) セクションをご覧ください。) |
| 在庫予測 | 選択した品目/モデルの組み合わせに対してフィルター処理される **在庫予測** ページのビューを開きます。 (この記事の後の [在庫予測](#inventory-forecast) セクションをご覧ください。) |
...
| メソッド | <p>予測トランザクションの割り当て方法を選択します。</p><ul><li>**なし** - 割り当ては行われません。</li><li>**期間** - 各期間で同じ数量を予測します。 この値を選択した場合は、**単位あたり** フィールドに数量を指定し、**単位** フィールドに時間の単位を指定します。</li><li>**キー** - 予測数量は、**期間キー** フィールドで指定した期間割り当てキーに基づいて割り当てられます。 この方法は、季節変動を考慮する場合に使用できます。</li></ol>|
...
需要予測は、品目、品目グループ、品目配賦キー、顧客、および顧客グループ別に入力できます。 さまざまなエンティティやレコードの **需要予測** ページを開く方法の詳細については、この記事の前半の [予測明細行の表示と手動入力](#manual-entry) を参照してください。
...
| 売上税グループ | 予測トランザクションの税を指定する場合に使用される税グループ。 |
| 品目消費税グループ | 予測トランザクションの税を指定する場合に使用される品目税グループ。 |
| メソッド | <p>予測トランザクションの割り当て方法を選択します。</p><ul><li>**なし** - 割り当ては行われません。</li><li>**期間** - 各期間で同じ数量を予測します。 この値を選択した場合は、**単位あたり** フィールドに数量を指定し、**単位** フィールドに時間の単位を指定します。</li><li>**キー** - 予測数量は、**期間キー** フィールドで指定した期間割り当てキーに基づいて割り当てられます。 この方法は、季節変動を考慮する場合に使用できます。</li><ul>|
...
品目配賦キーを使用している場合、または1つ以上の将来の期間の品目予測を入力した場合は、**概要** タブのツール バーで **予測の配賦** を選択することで、予測を配賦できます。その後、**配賦** グリッドの明細行によって示される方法で数量が配分されます。 (この記事で後述の [予測の割り当て](#allocate-forecast) セクションをご覧ください。)
...
## <a name="allocate-forecast"></a><a name="allocate-forecast"></a>予測の割り当て

選択した予測トランザクション明細行を処理するには、次の手順を使用します。 予測を割り当てると、**割り当て** グリッドの明細行によって示された数量が配分されます。

1. [予測明細行の表示と手動入力](#manual-entry)で説明しているように、予測を作成するエンティティのタイプ、および作成する予測のタイプに応じて、供給または需要予測ページを開きます。
1. 供給予測明細行または需要予測明細行ページで、予測明細行を選択し、**概要** タブで、ツール バーの **予測の割り当て** を選択します。
1. **予測の割り当て** ダイアログ ボックスで、次の表に示すフィールドを設定します。 (**メソッド** フィールドで選択する値により、使用可能となるその他のフィールドが決まります。)
    ...
    | 単位 | 時間間隔の単位を *日*、*月* または *年* から選択します。 その後割り当ては、**単位あたり** フィールドで指定した日数、月数、または年数に対応します。|
    | 期間キー | 予測の配賦に使用する期間割り当てキーを指定します。 詳細については、[予算計画データ配賦](../../finance/budgeting/budget-planning-data-allocation.md) を参照してください。 |
    | 終了 | **間隔** フィールドおよび **単位** フィールドでの設定に適用される終了日を指定します。 |

1. **OK** を選択して、設定を確定します。
1. 結果は、同じ行の **割り当て** タブで確認できます。
...
## <a name="use-forecasts-with-master-planning"></a>マスター プランを利用した予測の使用

需要予測や供給予測を入力した後、マスター プランの実行において、マスター プラン中の予測を予想される需要や供給に含めることができます。 予測をマスター プランに含める場合、材料と能力の総必要量が計算され、計画オーダーが生成されます。

### <a name="set-up-a-master-plan-to-include-an-inventory-forecast"></a>在庫予測を含めるためのマスター プランの設定

在庫予測が含まれるようにマスター プランを設定するには、次の手順に従います。
...
1. **一般** クイック タブで、次のフィールドを設定します。
    ...
    - **供給予測を含める** – このオプションを *はい* に設定すると、現在のマスター プランに供給予測を含めることができます。 これを *いいえ* に設定すると、供給予測トランザクションはマスター プランに含められません。
    - **需要予測を含める** – このオプションを *はい* に設定すると、現在のマスター プランに需要予測を含めることができます。 これを *いいえ* に設定すると、需要予測トランザクションはマスター プランに含まれません。
    - **予測要件を減らすために使用するメソッド** - 予測要求を減らすために使用するメソッドを選択します。 詳細については、[予測の下方修正キー](planning-optimization/demand-forecast.md#reduction-keys)を参照してください。

1. **タイム フェンス (日数)** クイックタブで、次のフィールドを設定して、需要予測が発生する期間を指定できます。
    ...
    > [!IMPORTANT]
    > **予測計画** オプションは、計画の最適化ではまだサポートされていません。

### <a name="run-a-master-plan-that-includes-an-inventory-forecast"></a>在庫予測を含むマスター プランの実行

在庫予測を含むマスター プランを実行するには、次の手順に従います。

1. **マスター プラン \> ワークスペース \> マスター プラン** に移動します。
1. **マスター プラン** フィールドで、前の手順で設定したマスター プランを入力または選択します。
1. **マスター プラン** タイルで **実行** を選択します。
1. **マスター プラン** ダイアログボックスで、**処理時間の追跡** オプションを *はい* に設定します。
1. **スレッド数** フィールドに数値を入力します。
1. **対象に含めるレコード** クイック タブで、**フィルター** を選択します。
1. 標準のクエリ エディター ダイアログ ボックスが表示されます。 **範囲** タブで、**フィールド** フィールドが *品目番号* に設定されている行を選択します。
1. **基準** フィールドで、計画に含める品目番号を選択します。
1. **OK** を選択します。

計算される要求を表示するには、**総必要量** ページを開きます。 たとえば、**リリースされた製品** ページのアクション ウィンドウにある **計画** タブの **要求** グループで、**総必要条件** を選択します。
...
- [需要予測を伴うマスター プラン](planning-optimization/demand-forecast.md)
