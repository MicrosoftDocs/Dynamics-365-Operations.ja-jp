...
- **優先順位** - このフィールドは、計画の最適化ではサポートされていません。 しかし、非推奨のマスタ プラン エンジンを使用している場合は、リソースや能力の割り当てで **優先順位** フィールドを使用して、リソースの優先順位を定義することができます。 続いて、**スケジュール設定パラメータ** ページの **一次リソース選択** フィールドで *優先順位* が選択されている場合、システムはスケジュール設定時に最も優先度の高い (つまり **優先順位** フィールドの数値が小さい )リソースを最初に選択します。
