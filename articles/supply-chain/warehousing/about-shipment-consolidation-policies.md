---
title: 出荷連結ポリシーの概要
description: この記事では、出荷連結ポリシーの柔軟な構成を実現する機能の概要について説明します。
author: Mirzaab
ms.date: 05/12/2020
ms.topic: article
ms.prod: ''
...
ms.locfileid: "9427931"
---
# <a name="shipment-consolidation-policies-overview"></a>出荷連結ポリシーの概要
...
*出荷連結ポリシー* 機能がシステムで有効になると、以前は **倉庫** 設定ページで使用可能となっていた、**倉庫へのリリース時に出荷を連結** 設定が非表示になります。 新たな出荷連結機能への移行を簡易化する目的で、**出荷連結ポリシー** ページの機能によって、既存の倉庫の古い設定を自動的に含む既定のポリシーを作成できます。 既定のポリシーが作成されると、 **在庫** の設定ページ内の **倉庫にリリースされた出荷を連結する** 設定が反映されなくなります。 詳細については、[出荷連結ポリシーを構成する](configure-shipment-consolidation-policies.md) を参照してください。
