...
出庫重量を記録するための方法は、各 CW 単位 (つまりボックス 1 つあたり) の重量が処理されるか、または重量がピッキングされる数量に基づき記録されるか定義できます (たとえば 3 つのボックス)。 生産ラインのピッキング プロセスに注意し、平均重量は**記録しない**オプションが使用される場合のみ実行します。
...
- 混合ライセンス プレートの受取は、CW 製品をサポートしていません。
...
- 倉庫アプリケーション処理の一部として CW タグがキャプチャされるとき、ユーザーはワークフローからキャンセルできません。
