...
この記事では、`InventSum` と `WHSInventReserve` のテーブルから不要なレコードを削除する、手持在庫エントリのクリーンアップ ジョブについて説明します。 これらのテーブルには、倉庫管理処理が有効になっている品目の手持情報が格納されます。 (これらの品目は WMS 品目と呼ばれます。) これらのレコードを削除すると、手持在庫計算のパフォーマンスが大幅に向上する可能性があります。
