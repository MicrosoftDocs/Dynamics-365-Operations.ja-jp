...
<span data-ttu-id="9aca0-107">このセクションでは、既存の在庫引当階層の要約します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-107">This section summarizes the existing inventory reservation hierarchy.</span></span> <span data-ttu-id="9aca0-108">バッチ追跡品目とシリアル追跡品目の処理方法に焦点を当てます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-108">It focuses on the way that batch-tracked and serial-tracked items are handled.</span></span>
...
### <a name="business-scenario"></a><span data-ttu-id="9aca0-122">ビジネス シナリオ</span><span class="sxs-lookup"><span data-stu-id="9aca0-122">Business scenario</span></span>
...
<span data-ttu-id="9aca0-163">次の例は、エンド ツー エンド フローの例を示しています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-163">The following example shows the end-to-end flow.</span></span>
...
1. <span data-ttu-id="9aca0-177">次の値を使用して、製品の 3 つのマスター データ パラメータを設定します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-177">Set the product's three master data parameters by using these values:</span></span>
    ...
    - <span data-ttu-id="9aca0-178">**保管分析コード グループ** フィールドで、**Ware** を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-178">In the **Storage dimension group** field, select **Ware**.</span></span>
...
7. <span data-ttu-id="9aca0-241">**製品情報管理** \> **製品** \> **リリース済製品**の順に移動します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-241">Go to **Product information management** \> **Products** \> **Released products**.</span></span> <span data-ttu-id="9aca0-242">品目を選択し、**在庫の管理** \> **表示** \> **トランザクション**を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-242">Select your item, and then select **Manage inventory** \> **View** \> **Transactions**.</span></span>
...
1. <span data-ttu-id="9aca0-253">**販売注文明細行**クイック タブで、**倉庫** \> **作業の詳細**を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-253">On the **Sales order lines** FastTab, select **Warehouse** \> **Work details**.</span></span>
    ...
    - <span data-ttu-id="9aca0-260">バッチ番号は、ピッキング行には表示されません。(「バッチ上\[場所\]」の引当階層が関連付けられている品目に対して作成される作業明細行と同様です) 代わりに、「開始」バッチ番号と他のすべての保管分析コードが、関連付けられている在庫トランザクションから参照されている作業明細行の作業在庫トランザクションに表示されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-260">The batch number isn't shown on the pick line (as is the case for the work line that is created for an item that has an associated "Batch-above\[location\]" reservation hierarchy.) Instead, the "from" batch number and all other storage dimensions are shown on the work line's work inventory transaction that is referenced from the associated inventory transactions.</span></span>
...
## <a name="exception-handling-of-warehouse-work-that-has-order-committed-batch-numbers"></a><span data-ttu-id="9aca0-273">注文が確定したバッチ番号を持つ倉庫作業の例外処理</span><span class="sxs-lookup"><span data-stu-id="9aca0-273">Exception handling of warehouse work that has order-committed batch numbers</span></span>
...
<span data-ttu-id="9aca0-289">ピッキング解除アクションの実行結果は次のようになります。</span><span class="sxs-lookup"><span data-stu-id="9aca0-289">Here are the results of the unpicking action:</span></span>
...
#### <a name="full-button--split-a-work-line-because-of-overflow-on-the-user-location"></a><span data-ttu-id="9aca0-324">フル ボタン - ユーザーの場所でオーバーフローしたために作業ラインを分割します</span><span class="sxs-lookup"><span data-stu-id="9aca0-324">Full button – Split a work line because of overflow on the user location</span></span>
...
#### <a name="reduce-the-picked-quantity-of-completed-work-from-a-load"></a><span data-ttu-id="9aca0-337">完了した作業のピッキング済数量をする (積荷から)</span><span class="sxs-lookup"><span data-stu-id="9aca0-337">Reduce the picked quantity of completed work (from a load)</span></span>
...
<td><span data-ttu-id="9aca0-373">前の行を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9aca0-373">See the previous row.</span></span></td>
...
<td><span data-ttu-id="9aca0-391">数量は、同じバッチ、および取消時に数量が残された場所とライセンス プレートに対して再引当されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-391">The quantity is re-reserved for the same batch, and for the location and license plate where the quantity was left upon reversal.</span></span></td>
</tr>
<tr>
...
<td><span data-ttu-id="9aca0-402">数量は、同じバッチ、および取消時に数量が割り当てられた場所とライセンス プレートに対して再引当されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-402">The quantity is re-reserved for the same batch, and for the location and license plate that the quantity was assigned to upon reversal.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-403">はい/いいえ</span><span class="sxs-lookup"><span data-stu-id="9aca0-403">Yes/No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-404"><strong>作業を取消</strong>ページを開きます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-404">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="9aca0-405">要求ページで、<strong>品目をこの場所に移動する</strong>オプションを選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-405">On the request page, select the <strong>Move items to this location</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="9aca0-406">取消はサポートされていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-406">Reversal isn't supported.</span></span></td>
<td><span data-ttu-id="9aca0-407">適用できません</span><span class="sxs-lookup"><span data-stu-id="9aca0-407">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-408">はい/いいえ</span><span class="sxs-lookup"><span data-stu-id="9aca0-408">Yes/No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-409"><strong>作業を取消</strong>ページを開きます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-409">Open the <strong>Reverse work</strong> page.</span></span></li>
<li><span data-ttu-id="9aca0-410">要求ページで、<strong>場所のディレクティブに基づいて品目を移動する</strong>オプションを選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-410">On the request page, select the <strong>Move items based on location directives</strong> option.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="9aca0-411">取消はサポートされていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-411">Reversal isn't supported.</span></span> </td>
<td><span data-ttu-id="9aca0-412">適用できません</span><span class="sxs-lookup"><span data-stu-id="9aca0-412">Not applicable</span></span></td>
</tr>
</tbody>
</table>

#### <a name="short-pick-a-quantity--register-the-quantity-as-physically-not-found-at-the-locationlicense-plate-while-you-perform-picking-work"></a><span data-ttu-id="9aca0-413">数量の選択 - ピッキング作業の実行中に、場所 / ライセンス プレートでは見つからない数量を登録する</span><span class="sxs-lookup"><span data-stu-id="9aca0-413">Short-pick a quantity – Register the quantity as physically not found at the location/license plate while you perform picking work</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="9aca0-414">キー設定パラメーター</span><span class="sxs-lookup"><span data-stu-id="9aca0-414">Key setup parameter</span></span></th>
<th><span data-ttu-id="9aca0-415">バッチ数量が利用可能</span><span class="sxs-lookup"><span data-stu-id="9aca0-415">Batch quantity is available</span></span></th>
<th><span data-ttu-id="9aca0-416">主要なユーザーの手順</span><span class="sxs-lookup"><span data-stu-id="9aca0-416">Key user steps</span></span></th>
<th><span data-ttu-id="9aca0-417">倉庫作業</span><span class="sxs-lookup"><span data-stu-id="9aca0-417">Warehouse work</span></span></th>
<th><span data-ttu-id="9aca0-418">注文が確定したバッチ引当</span><span class="sxs-lookup"><span data-stu-id="9aca0-418">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="9aca0-419"><strong>品目再配賦</strong> = <strong>なし</strong>、<strong>在庫の調整</strong> = <strong>はい</strong>、および<strong>引当の削除</strong> = <strong>いいえ</strong>で、<strong>ショートピック</strong> タイプの作業例外が設定されています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-419">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>None</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No</strong>.</span></span></td>
<td><span data-ttu-id="9aca0-420">有</span><span class="sxs-lookup"><span data-stu-id="9aca0-420">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-421">ピッキング作業のプロセス時に、倉庫アプリのメニュー項目<strong>ショート ピック</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-421">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="9aca0-422"><strong>ピック数量</strong>フィールドに <strong>0</strong> (ゼロ) と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-422">In the <strong>Pick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="9aca0-423"><strong>理由</strong>フィールドに、<strong>非再配賦</strong>と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-423">In the <strong>Reason</strong> field, enter <strong>No reallocation</strong>.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="9aca0-424">現在の作業は終了済で、ピッキング済数量は 0 (ゼロ) です。</span><span class="sxs-lookup"><span data-stu-id="9aca0-424">The current work is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="9aca0-425"><strong>棚卸</strong>タイプの在庫トランザクションおよび<strong>売却</strong>出庫タイプの在庫トランザクションが作成され、調整を表します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-425">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="9aca0-426">この数量は、同じバッチに対して再引当されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-426">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="9aca0-427">システムは、数量が使用可能な場所とライセンス プレート (場所がライセンス プレートで制御されている場合) をランダムに割り当てます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-427">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-428">いいえ</span><span class="sxs-lookup"><span data-stu-id="9aca0-428">No</span></span></td>
<td><span data-ttu-id="9aca0-429">前の行を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9aca0-429">See the previous row.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="9aca0-430">ショートピッキング アクションの実行に失敗し、エラーがスローされます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-430">The short-picking action fails, and an error is thrown.</span></span></li>
<li><span data-ttu-id="9aca0-431">現在の作業は開いたままになります。</span><span class="sxs-lookup"><span data-stu-id="9aca0-431">The current work remains open.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="9aca0-432">適用できません</span><span class="sxs-lookup"><span data-stu-id="9aca0-432">Not applicable</span></span></td>
</tr>
<tr>
<td rowspan='2'><span data-ttu-id="9aca0-433"><strong>品目再配賦</strong> = <strong>なし</strong>、<strong>在庫の調整</strong> = <strong>はい</strong>、および<strong>引当の削除</strong> = <strong>はい</strong>で、<strong>ショートピック</strong> タイプの作業例外が設定されています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-433">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>None</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>Yes</strong>.</span></span></td>
<td><span data-ttu-id="9aca0-434">有</span><span class="sxs-lookup"><span data-stu-id="9aca0-434">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-435">ピッキング作業のプロセス時に、倉庫アプリのメニュー項目<strong>ショート ピック</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-435">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="9aca0-436"><strong>ピック数量</strong>フィールドに <strong>0</strong> (ゼロ) と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-436">In the <strong>Pick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="9aca0-437"><strong>理由</strong>フィールドに、<strong>非再配賦</strong>と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-437">In the <strong>Reason</strong> field, enter <strong>No reallocation</strong>.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="9aca0-438">現在の作業は終了済で、ピッキング済数量は 0 (ゼロ) です。</span><span class="sxs-lookup"><span data-stu-id="9aca0-438">The current work is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="9aca0-439"><strong>棚卸</strong>タイプの在庫トランザクションおよび<strong>売却</strong>出庫タイプの在庫トランザクションが作成され、調整を表します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-439">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="9aca0-440">簡単なピッキング場所からの数量の調整によって影響を受けるすべての既存の引当は、同じバッチに対して再引当されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-440">All existing reservations that are affected by the quantity adjustment in the short-picked location are re-reserved for the same batch.</span></span> <span data-ttu-id="9aca0-441">システムは、数量が使用可能な場所とライセンス プレート (場所がライセンス プレートで制御されている場合) をランダムに割り当てます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-441">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-442">いいえ</span><span class="sxs-lookup"><span data-stu-id="9aca0-442">No</span></span></td>
<td><span data-ttu-id="9aca0-443">前の行を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9aca0-443">See the previous row.</span></span></td>
<td><span data-ttu-id="9aca0-444">前の行を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9aca0-444">See the previous row.</span></span></td>
<td><span data-ttu-id="9aca0-445">簡単なピッキング場所からの数量の調整によって影響を受けるすべての既存の引当は、削除されました。</span><span class="sxs-lookup"><span data-stu-id="9aca0-445">All existing reservations that are affected by the quantity adjustment in the short-picked location are removed.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-446"><strong>品目再配賦</strong> = <strong>手動</strong>、<strong>在庫の調整</strong> = <strong>はい</strong>、および<strong>引当の削除</strong> = <strong>いいえ / はい</strong>で、<strong>ショートピック</strong> タイプの作業例外が設定されています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-446">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No/Yes</strong>.</span></span> <span data-ttu-id="9aca0-447">さらに、作業者で<strong>手動による再配置を許可</strong>オプションが有効になります。</span><span class="sxs-lookup"><span data-stu-id="9aca0-447">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="9aca0-448">有</span><span class="sxs-lookup"><span data-stu-id="9aca0-448">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-449">ピッキング作業のプロセス時に、倉庫アプリのメニュー項目<strong>ショート ピック</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-449">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="9aca0-450"><strong>ショートピック数量</strong>フィールドに <strong>0</strong> (ゼロ) と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-450">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="9aca0-451"><strong>理由</strong>フィールドで、<strong>手動での再配分を使用したショートピッキング</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-451">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
<li><span data-ttu-id="9aca0-452">一覧から場所 / ライセンス プレートを選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-452">Select the location/license plate in the list.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="9aca0-453">現在の作業では、次のアクションが発生します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-453">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="9aca0-454">ピッキング行は終了済で、ピッキング済数量は 0 (ゼロ) です。</span><span class="sxs-lookup"><span data-stu-id="9aca0-454">The pick line is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="9aca0-455">プット明細行はキャンセルされました。</span><span class="sxs-lookup"><span data-stu-id="9aca0-455">The put line is canceled.</span></span></li>
<li><span data-ttu-id="9aca0-456">新しいピック行が作成されました。</span><span class="sxs-lookup"><span data-stu-id="9aca0-456">A new pick line is created.</span></span> <span data-ttu-id="9aca0-457">ユーザーによって選択された場所 / ライセンス プレートを使用します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-457">It uses the location/license plate that the user selected.</span></span></li>
<li><span data-ttu-id="9aca0-458">新しいプット明細行が作成されました。</span><span class="sxs-lookup"><span data-stu-id="9aca0-458">A new put line is created.</span></span></li>
</ul>
</li>
<li><span data-ttu-id="9aca0-459"><strong>棚卸</strong>タイプの在庫トランザクションおよび<strong>売却</strong>出庫タイプの在庫トランザクションが作成され、調整を表します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-459">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="9aca0-460">適用できません</span><span class="sxs-lookup"><span data-stu-id="9aca0-460">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-461"><strong>品目再配賦</strong> = <strong>手動</strong>、<strong>在庫の調整</strong> = <strong>はい</strong>、および<strong>引当の削除</strong> = <strong>いいえ</strong>で、<strong>ショートピック</strong> タイプの作業例外が設定されています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-461">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>No</strong>.</span></span> <span data-ttu-id="9aca0-462">さらに、作業者で<strong>手動による再配置を許可</strong>オプションが有効になります。</span><span class="sxs-lookup"><span data-stu-id="9aca0-462">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="9aca0-463">無</span><span class="sxs-lookup"><span data-stu-id="9aca0-463">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-464">ピッキング作業のプロセス時に、倉庫アプリのメニュー項目<strong>ショート ピック</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-464">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="9aca0-465"><strong>ショートピック数量</strong>フィールドに <strong>0</strong> (ゼロ) と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-465">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="9aca0-466"><strong>理由</strong>フィールドで、<strong>手動での再配分を使用したショートピッキング</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-466">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="9aca0-467">ショートピッキング アクションの実行に失敗し、エラーがスローされます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-467">The short-picking action fails, and an error is thrown.</span></span></td>
<td><span data-ttu-id="9aca0-468">適用できません</span><span class="sxs-lookup"><span data-stu-id="9aca0-468">Not applicable</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-469"><strong>品目再配賦</strong> = <strong>手動</strong>、<strong>在庫の調整</strong> = <strong>はい</strong>、および<strong>引当の削除</strong> = <strong>はい</strong>で、<strong>ショートピック</strong> タイプの作業例外が設定されています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-469">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Manual</strong>, <strong>Adjust inventory</strong> = <strong>Yes</strong>, and <strong>Remove reservations</strong> = <strong>Yes</strong>.</span></span> <span data-ttu-id="9aca0-470">さらに、作業者で<strong>手動による再配置を許可</strong>オプションが有効になります。</span><span class="sxs-lookup"><span data-stu-id="9aca0-470">Additionally, the <strong>Allow manual item reallocation</strong> option is enabled on the worker.</span></span></td>
<td><span data-ttu-id="9aca0-471">無</span><span class="sxs-lookup"><span data-stu-id="9aca0-471">No</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-472">ピッキング作業のプロセス時に、倉庫アプリのメニュー項目<strong>ショート ピック</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-472">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="9aca0-473"><strong>ショートピック数量</strong>フィールドに <strong>0</strong> (ゼロ) と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-473">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="9aca0-474"><strong>理由</strong>フィールドで、<strong>手動での再配分を使用したショートピッキング</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-474">In the <strong>Reason</strong> field, select <strong>Short Picking with manual reallocation</strong>.</span></span></li>
<li><span data-ttu-id="9aca0-475">一覧から場所 / ライセンス プレートを選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-475">Select the location/license plate in the list.</span></span></li>
</ol>
</td>
<td>
<ul>
<li><span data-ttu-id="9aca0-476">現在の作業では、次のアクションが発生します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-476">On the current work, the following actions occur:</span></span>
<ul>
<li><span data-ttu-id="9aca0-477">ピッキング行は終了済で、ピッキング済数量は 0 (ゼロ) です。</span><span class="sxs-lookup"><span data-stu-id="9aca0-477">The pick line is closed, and the picked quantity is 0 (zero).</span></span></li>
<li><span data-ttu-id="9aca0-478">プット明細行はキャンセルされました。</span><span class="sxs-lookup"><span data-stu-id="9aca0-478">The put line is canceled.</span></span></li>
</ul>
</li>
<li><span data-ttu-id="9aca0-479"><strong>棚卸</strong>タイプの在庫トランザクションおよび<strong>売却</strong>出庫タイプの在庫トランザクションが作成され、調整を表します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-479">An inventory transaction of the <strong>Counting</strong> type and the <strong>Sold</strong> issue type is created to represent the adjustment.</span></span></li>
</ul>
</td>
<td><span data-ttu-id="9aca0-480">簡単なピッキング場所 / ライセンス プレートからの数量の調整によって影響を受けるすべての既存の引当は、削除されました。</span><span class="sxs-lookup"><span data-stu-id="9aca0-480">All existing reservations that are affected by the quantity adjustment in the short-picked location/license plate are removed.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-481"><strong>品目再配賦</strong> = <strong>自動</strong>、<strong>在庫の調整</strong> = <strong>はい / いいえ</strong>、および<strong>引当の削除</strong> = <strong>はい / いいえ</strong>で、<strong>ショートピック</strong> タイプの作業例外が設定されています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-481">A work exception of the <strong>Short pick</strong> type is set up, where <strong>Item reallocation</strong> = <strong>Automatic</strong>, <strong>Adjust inventory</strong> = <strong>Yes/No</strong>, and <strong>Remove reservations</strong> = <strong>Yes/No</strong>.</span></span></td>
<td><span data-ttu-id="9aca0-482">該当なし</span><span class="sxs-lookup"><span data-stu-id="9aca0-482">Not applicable</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-483">ピッキング作業のプロセス時に、倉庫アプリのメニュー項目<strong>ショート ピック</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-483">Select the <strong>Shortpick</strong> menu item on the warehouse app when you run picking work.</span></span></li>
<li><span data-ttu-id="9aca0-484"><strong>ショートピック数量</strong>フィールドに <strong>0</strong> (ゼロ) と入力します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-484">In the <strong>Shortpick Quantity</strong> field, enter <strong>0</strong> (zero).</span></span></li>
<li><span data-ttu-id="9aca0-485"><strong>理由</strong>フィールドで、<strong>自動での再配分を使用したショートピッキング</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-485">In the <strong>Reason</strong> field, select <strong>Short Picking with automatic reallocation</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="9aca0-486">自動再割り当てを含むショートピッキングはサポートされていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-486">Short-picking that involves automatic reallocation isn't supported.</span></span></td>
<td><span data-ttu-id="9aca0-487">自動再割り当てを含むショートピッキングはサポートされていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-487">Short-picking that involves automatic reallocation isn't supported.</span></span></td>
</tr>
</tbody>
</table>

#### <a name="change-the-inventory-status"></a><span data-ttu-id="9aca0-488">在庫状態の変更</span><span class="sxs-lookup"><span data-stu-id="9aca0-488">Change the inventory status</span></span>

> [!NOTE]
> <span data-ttu-id="9aca0-489">この倉庫アクションは複数のエントリ ポイントから実行できます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-489">This warehouse action can be performed from multiple entry points.</span></span> <span data-ttu-id="9aca0-490">ここに表示されている例では、**保管場所ごとの手持在庫**ページで**在庫状態の変更**アクションを使用しています。</span><span class="sxs-lookup"><span data-stu-id="9aca0-490">The example that is shown here uses **Inventory status change** action on the **On-hand by location** page.</span></span>

<table>
<thead>
<tr>
<th><span data-ttu-id="9aca0-491">キー設定パラメーター</span><span class="sxs-lookup"><span data-stu-id="9aca0-491">Key setup parameter</span></span></th>
<th><span data-ttu-id="9aca0-492">バッチ数量が利用可能</span><span class="sxs-lookup"><span data-stu-id="9aca0-492">Batch quantity is available</span></span></th>
<th><span data-ttu-id="9aca0-493">主要なユーザーの手順</span><span class="sxs-lookup"><span data-stu-id="9aca0-493">Key user steps</span></span></th>
<th><span data-ttu-id="9aca0-494">倉庫作業</span><span class="sxs-lookup"><span data-stu-id="9aca0-494">Warehouse work</span></span></th>
<th><span data-ttu-id="9aca0-495">注文が確定したバッチ引当</span><span class="sxs-lookup"><span data-stu-id="9aca0-495">Order-committed batch reservation</span></span></th>
</tr>
</thead>
<tbody>
<tr>
<td rowspan='2'><span data-ttu-id="9aca0-496"><strong>倉庫</strong>タブの<strong>倉庫</strong>レコードで、<strong>引当とマーキングの削除</strong>フィールドが<strong>引当</strong>または<strong>マーキングと引当</strong>に設定されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-496">On the <strong>Warehouse</strong> tab, in the <strong>Warehouse</strong> record, the <strong>Remove reservations and markings</strong> field is set to <strong>Reservations</strong> or <strong>Markings and reservations</strong>.</span></span></td>
<td><span data-ttu-id="9aca0-497">はい</span><span class="sxs-lookup"><span data-stu-id="9aca0-497">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-498">特定の場所を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-498">Select a specific location.</span></span></li>
<li><span data-ttu-id="9aca0-499">特定の品目、場所、およびライセンス プレートがある明細行を選択します (場所がライセンス プレートにより制御されている場合)。</span><span class="sxs-lookup"><span data-stu-id="9aca0-499">Select a line that has a specific item, location, and license plate (if the location is license plate–controlled).</span></span></li>
<li><span data-ttu-id="9aca0-500"><strong>在庫状態の変更</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-500">Select <strong>Inventory status change</strong>.</span></span></li>
<li><span data-ttu-id="9aca0-501"><strong>在庫状態</strong>フィールドを<strong>ブロック</strong>に設定します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-501">Set the <strong>Inventory status</strong> field to <strong>Blocking</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="9aca0-502">作業用に引き当てられている数量に対して、在庫状態の変更は許可されていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-502">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="9aca0-503">この数量は、同じバッチに対して再引当されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-503">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="9aca0-504">システムは、数量が使用可能な場所とライセンス プレート (場所がライセンス プレートで制御されている場合) をランダムに割り当てます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-504">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></li>
<li><span data-ttu-id="9aca0-505"><strong>在庫状態の変更</strong>タイプの 2 つの在庫トランザクションを作成して、在庫状態分析コードの変更を表します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-505">Two inventory transactions of the <strong>Inventory status change</strong> type are created to represent the change in the inventory status dimension.</span></span></li>
<li><span data-ttu-id="9aca0-506">ブロックされた数量の引き当てを表すために、<strong>在庫ブロック</strong>タイプおよび<strong>引当済現物</strong>払出タイプの在庫トランザクションが作成されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-506">An inventory transaction of the <strong>Inventory blocking</strong> type and the <strong>Reserved physical</strong> issue type is created to represent the reservation of the blocked quantity.</span></span></li>
</ul>
</td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-507">いいえ</span><span class="sxs-lookup"><span data-stu-id="9aca0-507">No</span></span></td>
<td><span data-ttu-id="9aca0-508">前の行を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9aca0-508">See the previous row.</span></span></td>
<td><span data-ttu-id="9aca0-509">作業用に引き当てられている数量に対して、在庫状態の変更は許可されていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-509">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td>
<ul>
<li><span data-ttu-id="9aca0-510">引当が削除されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-510">The reservation is removed.</span></span></li>
<li><span data-ttu-id="9aca0-511"><strong>在庫状態の変更</strong>タイプの 2 つの在庫トランザクションを作成して、在庫状態分析コードの変更を表します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-511">Two inventory transactions of the <strong>Inventory status change</strong> type are created to represent the change in the inventory status dimension.</span></span></li>
<li><span data-ttu-id="9aca0-512">ブロックされた数量の引き当てを表すために、<strong>在庫ブロック</strong>タイプおよび<strong>引当済現物</strong>払出タイプの在庫トランザクションが作成されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-512">An inventory transaction of the <strong>Inventory blocking</strong> type and the <strong>Reserved physical</strong> issue type is created to represent the reservation of the blocked quantity.</span></span></li>
</ul>
</td>
</tr>
<tr>
<td rowspan='2'><span data-ttu-id="9aca0-513"><strong>倉庫</strong>タブの<strong>倉庫</strong>レコードで、<strong>引当とマーキングの削除</strong>フィールドが<strong>なし</strong>に設定されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-513">On the <strong>Warehouse</strong> tab, in the <strong>Warehouse</strong> record, the <strong>Remove reservations and markings</strong> field is set to <strong>None</strong>.</span></span></td>
<td><span data-ttu-id="9aca0-514">はい</span><span class="sxs-lookup"><span data-stu-id="9aca0-514">Yes</span></span></td>
<td>
<ol>
<li><span data-ttu-id="9aca0-515">特定の場所を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-515">Select a specific location.</span></span></li>
<li><span data-ttu-id="9aca0-516">特定の品目、場所、およびライセンス プレートがある明細行を選択します (場所がライセンス プレートにより制御されている場合)。</span><span class="sxs-lookup"><span data-stu-id="9aca0-516">Select a line that has a specific item, location, and license plate (if the location is license plate–controlled).</span></span></li>
<li><span data-ttu-id="9aca0-517"><strong>在庫状態の変更</strong>を選択します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-517">Select <strong>Inventory status change</strong>.</span></span></li>
<li><span data-ttu-id="9aca0-518"><strong>在庫状態</strong>フィールドを<strong>ブロック</strong>に設定します。</span><span class="sxs-lookup"><span data-stu-id="9aca0-518">Set the <strong>Inventory status</strong> field to <strong>Blocking</strong>.</span></span></li>
</ol>
</td>
<td><span data-ttu-id="9aca0-519">作業用に引き当てられている数量に対して、在庫状態の変更は許可されていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-519">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td><span data-ttu-id="9aca0-520">この数量は、同じバッチに対して再引当されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-520">The quantity is re-reserved for the same batch.</span></span> <span data-ttu-id="9aca0-521">システムは、数量が使用可能な場所とライセンス プレート (場所がライセンス プレートで制御されている場合) をランダムに割り当てます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-521">The system randomly assigns a location and license plate (if the location is license plate–controlled) where the quantity is available.</span></span></td>
</tr>
<tr>
<td><span data-ttu-id="9aca0-522">いいえ</span><span class="sxs-lookup"><span data-stu-id="9aca0-522">No</span></span></td>
<td><span data-ttu-id="9aca0-523">前の行を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9aca0-523">See the previous row.</span></span></td>
<td><span data-ttu-id="9aca0-524">作業用に引き当てられている数量に対して、在庫状態の変更は許可されていません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-524">Inventory status changes aren't allowed for quantities that are reserved for work.</span></span></td>
<td><span data-ttu-id="9aca0-525">在庫状態の変更は許可されません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-525">Inventory status changes aren't allowed.</span></span></td>
</tr>
</tbody>
</table>

## <a name="limitations"></a><span data-ttu-id="9aca0-526">制限</span><span class="sxs-lookup"><span data-stu-id="9aca0-526">Limitations</span></span>

- <span data-ttu-id="9aca0-527">変動倉庫のレベルの分析コード引当機能では、次の機能はサポートされません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-527">The flexible warehouse-level dimension reservation functionality doesn't support the following features:</span></span>

    - <span data-ttu-id="9aca0-528">CW 管理</span><span class="sxs-lookup"><span data-stu-id="9aca0-528">Catch weight management</span></span>
    - <span data-ttu-id="9aca0-529">現物マイナス在庫</span><span class="sxs-lookup"><span data-stu-id="9aca0-529">Physical negative inventory</span></span>
    - <span data-ttu-id="9aca0-530">注文供給に対する引当</span><span class="sxs-lookup"><span data-stu-id="9aca0-530">Reservation against ordered supply</span></span>
    - <span data-ttu-id="9aca0-531">移動オーダーおよび原材料のピッキング</span><span class="sxs-lookup"><span data-stu-id="9aca0-531">Transfer orders and raw material picking</span></span>

- <span data-ttu-id="9aca0-532">ディレクティブ単位での梱包のためのコンテナー連結ルールには、制限があります。</span><span class="sxs-lookup"><span data-stu-id="9aca0-532">The container consolidation rule for packing by directive unit has limitations.</span></span> <span data-ttu-id="9aca0-533">注文が確定された引当では、**ディレクティブ単位別の梱包**フィールドが有効になっているコンテナー構築テンプレートを使用しないことをお勧めします。</span><span class="sxs-lookup"><span data-stu-id="9aca0-533">For order-committed reservations, we recommend that you not use container build templates where the **Pack by directive unit** field is enabled.</span></span> <span data-ttu-id="9aca0-534">現在の設計では、倉庫作業の作成時に場所ディレクティブは使用されません。</span><span class="sxs-lookup"><span data-stu-id="9aca0-534">In the current design, location directives aren't used when warehouse work is created.</span></span> <span data-ttu-id="9aca0-535">したがって、コンテナー詰めのウェーブ ステップ中には、単位順序グループ (在庫単位) の最小単位のみが適用されます。</span><span class="sxs-lookup"><span data-stu-id="9aca0-535">Therefore, only the lowest unit in the unit sequence group (the inventory unit) is applied during the containerization wave step.</span></span>
