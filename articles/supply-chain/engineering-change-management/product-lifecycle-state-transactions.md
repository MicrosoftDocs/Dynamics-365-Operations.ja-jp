...
## <a name="default-product-lifecycle-states"></a>既定の製品ライフサイクルの状態

エンジニアリング バージョンの既定のライフサイクルの状態は、そのエンジニアリング カテゴリによって指定されます。 この状態は、新しいエンジニアリング バージョンを作成するときに行います (新しい製品の最初のバージョンも含む)。

新しい製品またはエンジニアリング製品を作成する場合は、製品に割り当てられているリリース ポリシーのテンプレートリリース製品で指定することにより、既定のライフサイクルの状態を設定することもできます。

この場合、新しいエンジニアリング製品を作成する場合、製品のライフサイクルの状態はバージョンとは異なる可能性があります。
