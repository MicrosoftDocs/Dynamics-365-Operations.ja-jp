---
title: サービス契約グループの作成
description: この記事では、サービス契約グループ作成方法の手順について解説します。
author: sorenva
ms.date: 06/15/2020
ms.topic: article
ms.search.form: SMAAgreementGroup
audience: Application User
ms.reviewer: kamaybac
ms.search.region: Global
ms.author: sorenand
ms.search.validFrom: 2016-02-28
ms.dyn365.ops.version: AX 7.0.0
ms.openlocfilehash: f7cd195f8c8cda0b975f47bc73fc27860eb3a40f
ms.sourcegitcommit: cfe8fbc202c3eb05d894076fdf99e46704f17365
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 06/15/2022
ms.locfileid: "9014593"
---
# <a name="create-service-agreement-groups"></a>サービス契約グループの作成

[!include [banner](../includes/banner.md)]

サービス契約グループは、サービス契約の並べ替えやフィルタ処理に便利です。

1. **サービス管理** \> **設定** \> **サービス契約** \> **サービス契約グループ** の順に移動します。
1. アクション ウィンドウで、**新規** を選択して新しいサービス契約グループを作成します。
1. **サービス契約グループ** フィールドで、サービス契約グループの ID を入力します。
1. **説明** フィールドに、サービス契約グループの説明を入力します。
1. **サービス レベル アグリーメント** フィールドで、グループに適用するアグリーメントを選択します。
1. ページを閉じて、変更を保存します。

> [!NOTE]
> **サービス管理パラメーター** ページの **サービス契約グループ** フィールドでサービス契約グループを定義した場合、新しいサービス契約を作成するときは常に、このグループが既定として表示されます。

## <a name="related-articles"></a>関連記事

[サービス契約の概要](service-agreement-groups.md)


[!INCLUDE[footer-include](../../includes/footer-banner.md)]