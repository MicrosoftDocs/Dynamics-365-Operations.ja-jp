...
コンテキスト メニューを変更するためのプログラミング モデルは、以前のリリースで使用されていたモデルとは異なります。 Dynamics AX 2012 では、**PopupMenu** クラスが使用されていました。 このクラスは、Microsoft Windows アプリケーション プログラミング インターフェイス (API) に依存します。 ただし、これらの API が Web で使用できないため、交換 API が同様の機能を提供するよう作成されました。 詳細については、[コードの移行: コンテキスト メニュー](../migration-upgrade/code-migration-context-menus.md)を参照してください。
