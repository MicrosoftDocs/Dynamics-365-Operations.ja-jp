...
クライアントがサービスと通信する前に、(Azure AD) に登録する必要があります。 これらの手順を使用すると、(Azure AD) でアプリケーションに登録できます。 この手順については、[Azure アプリ登録のトレーニングガイド](https://docs.microsoft.com/en-us/azure/active-directory/develop/app-registrations-training-guide)を参照してください。 このプロセスでの Finance and Operation の構成には、次の追加情報をコンテキストで使用する必要があります。
