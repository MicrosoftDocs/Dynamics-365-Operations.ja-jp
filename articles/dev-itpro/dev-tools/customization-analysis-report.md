...
ms.openlocfilehash: 06d8d354429b0d23b634e7b402e21bb89597fc0d
...
|---------------------|----------------------------------------------------------------------------------------------|
| エラー メッセージ       | メソッド %1 のパラメーター 1% は、メソッド内で変更されます                                     |
| 出庫タイプ/重要度 | コード/警告                                                                                 |
...
|---------------------|-----------------------------------------------------------------------------------|
| エラー メッセージ       | メソッド %1 に含まれる SQL コード                                                       |
| 出庫タイプ/重要度 | コード/警告                                                                      |
...
|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | %1 メソッドに見られる入れ子になったデータのアクセス ループ                                                                                                                                                                                                     |
| 出庫タイプ/重要度 | コード/警告                                                                                                                                                                                                                                   |
...
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | Insert メソッドは、メソッド %1 で RecordInsertList に置き換えることができます。                                                                                                               |
| 出庫タイプ/重要度 | コード/警告                                                                                                                                                                   |
...
|---------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | %1 メソッドで入れ子になった select ステートメントは結合できます。                                                                                                                                                                                                  |
| 出庫タイプ/重要度 | コード/警告                                                                                                                                                                                                                                        |
...
|---------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | メソッド %1 の select ステートメントでの関数呼び出し                                                                                                     |
| 出庫タイプ/重要度 | コード/警告                                                                                                                                             |
| 修正する方法       | **選択**ステートメントを呼び出す前に、関数の戻り値をローカル変数に割り当て、**選択**ステートメントでローカル変数を使用します。 |
...
|---------------------|----------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | フォーム %1 の ExecuteQuery メソッドに super() を追加しないで、範囲を追加します。                                 |
| 出庫タイプ/重要度 | コード/警告                                                                                                   |
| 修正する方法       | このパターンを回避するために、コードをリファクタリングします。                                                                      |

### <a name="bpcheckinvalidinitformmethod"></a>BPCheckInvalidInitFormMethod

| 説明         | このルールは、**super()** を呼び出す前に、フォームのコントロールとデータソースをフォームの **init** メソッドで初期化しないようにします。 **element** または **this** を **super()** に対する呼び出し (**this.aMethod()** または **element.aMethod()**) の前に使用するステートメントが検出された場合に失敗します。 情報メッセージは番号順序の初期化など許可されたいくつかのパターンのみを表示します (**numberSeqPreInit**) 。 |
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | フォーム要素の明細書は、フォーム %1 の init メソッドの super() の前に使用しないでください。                                                                                                                                                                                                                                                                                                                                     |
...
|---------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | 範囲はロックするか、フォーム %1で非表示にする必要があります                                                                                            |
| 出庫タイプ/重要度 | コード/警告                                                                                                                           |
| 修正する方法       | **QueryBuildRange.status(RangeStatus::Locked)** または **QueryBuildRange.status(RangeStatus::Hidden)** after the call to **AddRange** を追加します。 |
...
<tr class="odd">
<td>説明</td>
<td>このルールは、正しく処理されない <strong>tts</strong> ブロック内の <strong>try</strong> ステートメントを検出すると失敗します。</td>
</tr>
<tr class="even">
<td>エラー メッセージ</td>
<td>Try 文の Tts ブロックは、メソッド %1 の例外を明示的にキャッチしません。</td>
</tr>
<tr class="odd">
<td>出庫タイプ/重要度</td>
<td>コード/警告</td>
...
|---------------------|------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | カスタム ジョブはクラス %1 でバッチ対応されていません                                                                            |
| 出庫タイプ/重要度 | コード/警告                                                                                                           |
...
<tr class="even">
<td>エラー メッセージ</td>
<td>キャッシュされていない フォーム %2 に、メソッド %1 を表示</td>
</tr>
<tr class="odd">
<td>出庫タイプ/重要度</td>
<td>コード/警告</td>
...
<tr class="even">
<td>修正する方法</td>
<td>この警告は、次のいずれかのパターンを使用して修正することができます。
<ul>
...
<li>フォームの <strong>init</strong> メソッドで <strong>CacheAddMethod</strong> を使用すると、そのメソッドをキャッシュ済みとしてマークします。</li>
</ul></td>
</tr>
</tbody>
</table>
...
| 説明         | このルールは、フォームの **init** メソッドで **while** ループを持つデータ アクセス クエリが見つかった場合に失敗します。 このパターンは、フォームの初期化時にパフォーマンスの問題を引き起こす可能性があります。 |
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | フォーム %1 の init メソッドでループが検出された SQL クエリ                                                                                                                        |
| 出庫タイプ/重要度 | コード/警告                                                                                                                                                                         |
| 修正する方法       | このパターンを回避するために、コードをリファクタリングします。                                                                                                                                            |
...
|---------------------|-----------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | フォーム メソッド %1 で上書きされたデータ ソース クエリ                                                                  |
| 出庫タイプ/重要度 | コード/警告                                                                                                    |
| 修正する方法       | このパターンを回避するために、コードをリファクタリングします。                                                                       |
...
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | メソッド %1 に実装されていない ForUpdate を選択                                                                                                                                                         |
| 出庫タイプ/重要度 | コード/警告                                                                                                                                                                                          |
...
<tr class="even">
<td>エラー メッセージ</td>
<td>%1 テーブルにテーブル グループ、およびキャッシュ ルックアップの無効な組み合わせがあります</td>
</tr>
<tr class="odd">
<td>出庫タイプ/重要度</td>
<td>メタデータ/警告</td>
...
|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| エラー メッセージ       | リレーション名 %3 のテーブル %2 に関連するテーブル %1 に欠けているアクションを削除する                                                                                                                                                                                                    |
| 出庫タイプ/重要度 | メタデータ/警告                                                                                                                                                                                                                                                                         |
...
<li>データ ソースの<strong>リンク タイプ</strong>プロパティは<strong>パッシブ</strong>に設定されません。</li>
</ul></td>
</tr>
<tr class="even">
<td>エラー メッセージ</td>
<td>新しいメッセージでは、「フォーム %2 のタブ ページ コントロールにバインドされているデータ ソース %1 にパッシブ結合を使用する」と示します</td>
</tr>
<tr class="odd">
<td>出庫タイプ/重要度</td>
<td>メタデータ/警告</td>
...
### <a name="bpcheckaltenatekeyabsent"></a>BPCheckAltenateKeyAbsent
