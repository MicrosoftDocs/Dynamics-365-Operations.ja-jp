...
一時的なウィンドウにメッセージ (テキストまたは選択した結果) を表示するには、**print** ステートメントを使用します。 このウィンドウは、最初の **print** 文が実行されたときに表示されます。 テスト中に、**印刷**明細書は情報ログのテキストを表示する **Global::info** メソッドに代わる便利な手段となります。 次のテーブルは、 **print** ステートメントと **info** メソッドを比較しています。
