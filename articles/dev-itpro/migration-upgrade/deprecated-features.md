...
> [!IMPORTANT]
> Dynamics 365 for Finance and Operations 10.0.1 およびプラットフォーム更新プログラム 25 は、プレビュー リリースの一部として、対象とするユーザーが使用可能です。 コンテンツおよび機能は、変更されることがあります。 プレビュー リリースの詳細については、[標準および最初のリリース サービス更新](https://docs.microsoft.com/en-us/dynamics365/unified-operations/fin-and-ops/get-started/public-preview-releases) を参照してください。
...
|   |  |
|------------|--------------------|
| **廃止 / 削除の理由** | プラットフォーム更新プログラム 25 より前のリリースでは、別のパッケージまたはモジュールが定義する内部クラスまたはテーブルから派生したクラスまたはテーブルを作成することができました。 これは、安全なコーディング プラクティスではありません。 プラットフォーム更新プログラム 25 以降では、これを試行すると、コンパイラは警告メッセージを表示します。|
| **別の機能で置き換えられているか?**   | コンパイラの警告は、次回のプラットフォーム更新プログラムでは、エラーに置き換えられます。 この変更は実行時に下位互換性があり、プラットフォーム更新プログラム 25 以降を実行している場合、すべてのサンドボックスまたはカスタム コードを修正する必要のない実稼働環境上で配置されます。 この変更は開発およびコンパイル時にのみ影響します。 |
| **影響を受ける製品領域**         | Visual Studio 開発ツール。 |
| **配置オプション**              | すべて |
| **ステータス**                         | 非推奨 - 警告は次回のプラットフォーム更新プログラムではエラーになります。 |

#### <a name="overriding-internal-methods-is-deprecated"></a>内部メソッドのオーバーライドは推奨されません

|   |  |
|------------|--------------------|
| **廃止 / 削除の理由** | プラットフォーム更新プログラム 25 より前のリリースでは、別のパッケージまたはモジュールが定義する派生クラスの内部メソッドをオーバーライドすることができました。 これは、安全なコーディング プラクティスではありません。 プラットフォーム更新プログラム 25 以降では、これを試行すると、コンパイラは警告メッセージを表示します。|
| **別の機能で置き換えられているか?**   | この警告は、次回のプラットフォーム更新プログラムでは、コンパイル エラーに置き換えられます。 この変更は実行時に下位互換性があり、プラットフォーム更新プログラム 25 以降を実行している場合、すべてのサンドボックスまたはカスタム コードを修正する必要のない実稼働環境上で配置されます。 この変更は開発およびコンパイル時にのみ影響します。 |
| **影響を受ける製品領域**         | Visual Studio 開発ツール。 |
| **配置オプション**              | すべて |
| **ステータス**                         | 非推奨 - 警告は次回のプラットフォーム更新プログラムではエラーになります。 |
...
### <a name="print-to-screen-functionality"></a>画面に出力機能
顧客は、レポート ビューアー コントールによって提供される**インポート** アクションを使用して、Finance and Operations アプリケーションが生成するドキュメントをダウンロードすることができます。 この HTML ベースのレポート プレゼンテーションによって、ページ設定されていないドキュメント プレビューがユーザーにもたらされます。

|   |  |
|------------|--------------------|
| **廃止 / 削除の理由** | ページ設定されていない種類の HTML ベースのプレビュー エクスペリエンスは、Finance and Operations によって最終的に生成される物理的なドキュメントの再現性は提供**しません**。 PDF を事業運営の標準形式として完全に採用することにより、アプリケーション レポートとのやり取りのユーザー オプションを大幅に簡略化し、ドキュメント レンダリング プロセスを合理化することができます。 |
...
| **配置オプション**              | すべて  |
| **ステータス**                         | 非推奨: この機能の削除日は設定されていません。 アプリケーションのレポートを PDF ドキュメントとして自動的にブラウザーにダウンロードする機能は、2019 年 5 月のプラットフォーム更新プログラムで計画されています。 <br><br>**重要:** 画面への出力機能に依存している既存の顧客は、プラットフォーム更新プログラム 26 にアップグレードする前に、[サポート](../lifecycle-services/lcs-support.md) に通知することをお勧めします。 |
...
| **別の機能での置き換え?**   | [AppSource](https://appsource.microsoft.com) サイトで利用可能な、**原価管理**、**財務パフォーマンス**、および **Retail channel performance** コンテンツ パックは、データベース レベルでソリューション統合を許可する分析アプリケーションにより置き換えられます。 分析アプリケーションの詳細については、[ワークスペースの埋め込み Power BI](../../dev-itpro/analytics/embed-power-bi-workspaces.md) を参照してください。    |
...
|------------|--------------------|
| **廃止 / 削除の理由** | 機能の重複。       |
| **別の機能での置き換え?**   | はい。 この機能は Finance and Operations - Warehousing で置き換えられました。 設定、および前提条件の詳細については、[Microsoft Dynamics 365 for Finance and Operations- Warehousing のインストールと構成](https://docs.microsoft.com/en-us/dynamics365/unified-operations/supply-chain/warehousing/install-configure-warehousing-app) を参照してください。 |
