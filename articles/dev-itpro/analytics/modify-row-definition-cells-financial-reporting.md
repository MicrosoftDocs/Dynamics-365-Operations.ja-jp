...
次の例では、どの現金勘定にも動きがないため、Phyllis はレポートの **合計現金** 行の見出しとアンダースコアを印刷しないようにしようとしています。 したがって、**関連するフォーミュラ / 行 / 単位** セル内の (**---** 書式コードが示すようにフォーマット行である) 行 220 に、表示しない金額行の行コードである**250**を入力します。
