...
このトピックでは、[セルフサービス配置](infrastructure-stack.md)エクスペリエンスを使用して配置した Microsoft Dynamics 365 for Finance and Operations 環境に、更新を適用するプロセスについて説明します。
...
[セルフサービス配置](infrastructure-stack.md)環境を使用して展開されている環境は、バージョン8.1 (2018) またはそれ以降のバージョンでMicrosoft Dynamics 365 for Finance and Operations実行されるため、次の種類のパッケージがサポートされています。
...
運用環境に適用される更新プログラムの信頼性を向上させるために、サンドボックス環境で実行される特定の更新プログラムを**リリース候補**として指定 ("マーク") し、実稼働環境に移動します。 この更新プログラムには、基本製品とカスタマイズの両方が含まれています。 したがって、選択した更新プログラムを実稼働環境に適用したときに、両方が移動します。 この動作は現在のサービスとは異なります。 現時点では、カスタマイズのみを移動できますが、基本製品は変更 **されません** 。 セルフサービス配置環境では、サンドボックス環境でリリース候補としてマークされている更新によって、基本製品バージョンが**変わる場合があります**。
...
5. **ダウンタイム開始** フィールドで、日付と時刻を入力します。 環境は、指定された日の指定された時間に停止されます。 **ダウンタイムの終了** フィールドは、ダウンタイムの開始から数時間に設定されています。
