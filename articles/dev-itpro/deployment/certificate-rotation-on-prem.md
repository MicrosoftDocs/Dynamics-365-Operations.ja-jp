...
有効期限が近づくにつれて、Finance and Operations のオンプレミス環境で使用する証明書を回転することが必要となる場合があります。 このトピックでは、既存の証明書を置換する方法と、新しい証明書を使用するために環境内の参照を更新する方法について説明します。
...
7. 後で LCS で使用できる値を保持するには、次の PowerShell コマンドを実行します。 詳細については、「[LCS から Finance and Operations (オンプレミス) 環境を配置する](setup-deploy-on-premises-pu12.md#deploy)」を参照してください。
