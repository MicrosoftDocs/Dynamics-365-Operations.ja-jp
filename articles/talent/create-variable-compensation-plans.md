...
従業員の特定の値を上書きするには、[**上書き**] タブを使用できます。 たとえば、[**採用ルール**] でプランに [**割合**] が設定されていて、別の採用日付を従業員の採用率の計算時に使用する必要がある場合、[**採用ルールの日付**] フィールドに採用日付を設定できます。 また、プランの設定に応じて、特定の従業員の [**報奨率**] の値または [**単位数**] の値も上書きできます。 これらの値は、さらにプランの採用ルール、業績係数、およびその他の設定に影響されます。 
