...
- <span data-ttu-id="af0b0-106">誤って定期売買が正しくない Azure AD テナントに対して購入されました。</span><span class="sxs-lookup"><span data-stu-id="af0b0-106">Subscriptions were accidentally purchased against the incorrect Azure AD tenant.</span></span>

    > [!NOTE]
    > <span data-ttu-id="af0b0-107">クラウド サービス プロバイダーであり、既存の顧客に Microsoft Dynamics 365 for Finance and Operations のサブスクリプションを販売する場合は、その顧客との再販業者関係を要求して、顧客の既存の Azure AD テナントにサブスクリプションを配置する必要があります。</span><span class="sxs-lookup"><span data-stu-id="af0b0-107">If you're a cloud service provider, and you sell subscriptions for Microsoft Dynamics 365 for Finance and Operations to an existing customer, you must request a reseller relationship with that customer to put the subscriptions on the customer's existing Azure AD tenant.</span></span> <span data-ttu-id="af0b0-108">Microsoft パートナー センターで新しい顧客レコードを作成する場合、顧客の新しい Azure AD テナントを作成します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-108">If you create a new customer record for the customer in Microsoft Partner Center, you create a new Azure AD tenant for the customer.</span></span>

- <span data-ttu-id="af0b0-109">定期売買が購入された後、顧客は Azure AD テナントの構造を変更します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-109">The customer changes the structure of the Azure AD tenant after the subscription is purchased.</span></span>

<span data-ttu-id="af0b0-110">定期購読および関連するすべてのコンポーネントの移動のプロセスには、次の図に示すように、次の 4 つの主要なステップがあります。</span><span class="sxs-lookup"><span data-stu-id="af0b0-110">The process for moving your subscriptions and all related artifacts has four main steps, as shown in the following illustration.</span></span>
...
- <span data-ttu-id="af0b0-127">登録番号</span><span class="sxs-lookup"><span data-stu-id="af0b0-127">Enrollment number</span></span>
- <span data-ttu-id="af0b0-128">現在プロビジョニングされている定期売買の現在のテナント ドメイン</span><span class="sxs-lookup"><span data-stu-id="af0b0-128">The current tenant domain that the subscriptions are currently provisioned on</span></span>
- <span data-ttu-id="af0b0-129">顧客が定期売買のプロビジョニングを希望する宛先テナント ドメイン</span><span class="sxs-lookup"><span data-stu-id="af0b0-129">The destination tenant domain that the customer wants the subscriptions provisioned under</span></span>
- <span data-ttu-id="af0b0-130">顧客が違うテナントに移行したボリューム ライセンス サブスクリプションを必要とする理由の詳細説明</span><span class="sxs-lookup"><span data-stu-id="af0b0-130">A detailed explanation of why the customer must have its Volume Licensing subscriptions migrated to a different tenant</span></span>
- <span data-ttu-id="af0b0-131">新しいテナントに移動しなければならない有料サブスクリプションの総数、サブスクリプション タイプおよび定員数</span><span class="sxs-lookup"><span data-stu-id="af0b0-131">The total number of paid subscriptions that must be moved to the new tenant, together with the subscription type and seat count</span></span>
...
2. <span data-ttu-id="af0b0-137">新しい LCS プロジェクトのすべての非運用環境を配置します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-137">Deploy all non-production environments in the new LCS project.</span></span>
3. <span data-ttu-id="af0b0-138">必要なコード パッケージを環境に適用します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-138">Apply the required code packages to the environments.</span></span>
4. <span data-ttu-id="af0b0-139">環境にデータをアップロードします。</span><span class="sxs-lookup"><span data-stu-id="af0b0-139">Upload data to the environments.</span></span> <span data-ttu-id="af0b0-140">データ パッケージを使用してデータを移動することも、データベースを復元することもできます。</span><span class="sxs-lookup"><span data-stu-id="af0b0-140">You can move the data through data packages or by restoring the database.</span></span> <span data-ttu-id="af0b0-141">データベースを復元する場合は、いくつかのプロパティを新しいテナントに再マップするために追加の手順が必要です。</span><span class="sxs-lookup"><span data-stu-id="af0b0-141">If you restore the database, additional steps are required in order to remap some properties to the new tenant.</span></span>
5. <span data-ttu-id="af0b0-142">ユーザー情報を更新します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-142">Update your user information.</span></span>

    1. <span data-ttu-id="af0b0-143">管理者ユーザーを除くすべてのユーザー アカウントを削除します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-143">Remove all user accounts except the admin user.</span></span>
    2. <span data-ttu-id="af0b0-144">USERINFO の管理者ユーザー レコードを修正します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-144">Fix the admin user record in USERINFO.</span></span>
...
6. <span data-ttu-id="af0b0-145">正しいセキュリティ識別子 (SID) および ID プロバイダーを持つ他のすべてのユーザーを再インポートします。</span><span class="sxs-lookup"><span data-stu-id="af0b0-145">Re-import all other users that have the correct security identifier (SID) and identity provider.</span></span>
7. <span data-ttu-id="af0b0-146">適切なテーブルでテナント ID を更新するには、次のコマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-146">Run the following commands to update the tenant ID in the appropriate tables:</span></span>

    - <span data-ttu-id="af0b0-147">名前 = 'TENANTID' の際は SYSSERVICECONFIGURATIONSETTING から VALUE を選択します</span><span class="sxs-lookup"><span data-stu-id="af0b0-147">select VALUE from SYSSERVICECONFIGURATIONSETTING where name = 'TENANTID'</span></span>
    - <span data-ttu-id="af0b0-148">POWERBICONFIG から TENANTID を選択します</span><span class="sxs-lookup"><span data-stu-id="af0b0-148">select TENANTID from POWERBICONFIG</span></span>
    - <span data-ttu-id="af0b0-149">PROVISIONINGMESSAGETABLE から TENANTID を選択します</span><span class="sxs-lookup"><span data-stu-id="af0b0-149">select TENANTID from PROVISIONINGMESSAGETABLE</span></span>
    - <span data-ttu-id="af0b0-150">B2BINVITATIONCONFIG から TENANTID を選択します</span><span class="sxs-lookup"><span data-stu-id="af0b0-150">select TENANTID from B2BINVITATIONCONFIG</span></span>
    - <span data-ttu-id="af0b0-151">RETAILSHAREDPARAMETERS から TENANTID を選択します</span><span class="sxs-lookup"><span data-stu-id="af0b0-151">select TENANTID from RETAILSHAREDPARAMETERS</span></span>

8. <span data-ttu-id="af0b0-152">環境を完全に構成します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-152">Fully configure the environments.</span></span> <span data-ttu-id="af0b0-153">この手順の一部として、統合のエンドポイントをコンフィギュレーションします。</span><span class="sxs-lookup"><span data-stu-id="af0b0-153">As part of this step, configure the integration endpoints.</span></span>
9. <span data-ttu-id="af0b0-154">新しい LCS プロジェクトのユーザー受け入れテスト (UAT) 環境でスモーク テストを実行します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-154">Perform smoke tests on the user acceptance testing (UAT) environment in the new LCS project.</span></span> <span data-ttu-id="af0b0-155">これらのテストでは、ユーザーのサインイン、統合、ワークフロー、印刷、レポート、および構成やユーザー情報に依存する同様のプロセスに焦点を当てる必要があります。</span><span class="sxs-lookup"><span data-stu-id="af0b0-155">These tests should focus on user sign-in, integrations, workflows, printing, reporting, and similar processes that depend on configuration and user information.</span></span>
10. <span data-ttu-id="af0b0-156">実稼働環境を既に配置している場合は、すべてのサンドボックス環境の移動が終了し、UAT を完了した後に、新しいテナントに移動するためのサポート要求を開く必要があります。</span><span class="sxs-lookup"><span data-stu-id="af0b0-156">If you already had a production environment deployed, you must open a support request to move it to the new tenant after you've finished moving all the sandbox environments and completed UAT.</span></span> <span data-ttu-id="af0b0-157">実稼働環境を新しいテナントに移動するプロセスには、48 ～ 72 時間のダウンタイムの延長が必要です。</span><span class="sxs-lookup"><span data-stu-id="af0b0-157">The process of moving a production environment to a new tenant requires an extended downtime of 48 to 72 hours.</span></span>

<span data-ttu-id="af0b0-158">ソリューションとスコープに応じて、新しい Azure AD テナントで追加の手順を実行する必要があります。</span><span class="sxs-lookup"><span data-stu-id="af0b0-158">Depending on your solution and scope, you might have to perform additional steps on the new Azure AD tenant.</span></span> <span data-ttu-id="af0b0-159">これらの手順には、アプリケーションの登録 (定期的な統合と倉庫管理のため)、ドメインの追加、およびシングル サインオン (SSO) を有効にするディレクトリ同期の設定が含まれます。</span><span class="sxs-lookup"><span data-stu-id="af0b0-159">These steps might include registering applications (for recurring integrations and warehouse management), adding domains, and setting up directory synchronization to enable single sign-on (SSO).</span></span>

<span data-ttu-id="af0b0-160">Web サービスへの呼び出しが、環境の**ホーム**テナントからのみ許可されていることに注意してください。</span><span class="sxs-lookup"><span data-stu-id="af0b0-160">Note that calls to web services are allowed only from the **home** tenant for the environment.</span></span> <span data-ttu-id="af0b0-161">たとえば、元のテナントは companya.com であり、統合は `services@companya.com` として実行されました。</span><span class="sxs-lookup"><span data-stu-id="af0b0-161">For example, the original tenant was companya.com, and integration ran as `services@companya.com`.</span></span> <span data-ttu-id="af0b0-162">この場合、テナントを companyb.com に切り替えると、**userInfo.networkdomain** を `https://sts.windows.net/companyb.com` に更新してもWeb サービスの呼び出しに対して `services@companya.com` を使用できなくなります。</span><span class="sxs-lookup"><span data-stu-id="af0b0-162">In this case, when you switch tenants to companyb.com, you can no longer use `services@companya.com` for web service calls, even if you update **userInfo.networkdomain** to `https://sts.windows.net/companyb.com`.</span></span>
...
1. <span data-ttu-id="af0b0-172">すべての環境が削除されると、必要な LCS コンポーネントを保存し、クラウド サービス プロバイダまたは Volume Licensing Support と協力して、以前の Azure AD テナントのすべてのライセンスを中断します。</span><span class="sxs-lookup"><span data-stu-id="af0b0-172">After all the environments have been deleted, and you've saved the LCS artifacts that you require, work with your cloud service provider or Volume Licensing Support to suspend all the licenses on the old Azure AD tenant.</span></span>
    ...
    - <span data-ttu-id="af0b0-174">**ボリューム ライセンス サポート:** 作業が完了したこと、およびサブスクリプションが古いテナントに対してすぐ中断することを確認するボリューム ライセンス サポート センター。</span><span class="sxs-lookup"><span data-stu-id="af0b0-174">**Volume Licensing Support:** Call the Volume Licensing support center to confirm that you've completed the work and that the subscriptions can now be suspended against the old tenant.</span></span>

2. <span data-ttu-id="af0b0-175">古い LCS プロジェクトを削除するサポート チケットをファイルします。</span><span class="sxs-lookup"><span data-stu-id="af0b0-175">File a support ticket to delete the old LCS project.</span></span>
