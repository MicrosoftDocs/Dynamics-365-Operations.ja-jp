...
- Microsoft Dynamics 365 for Finance and Operations のタスク レコーダー機能と、Microsoft Dynamics Lifecycle Services (LCS) および Microsoft Azure DevOps を組み合わせて使用することで、テストケースの作成、構成、実行、調査、および報告が可能になります。
- 機能ユーザーが Finance and Operations のタスク レコーダーを使用してビジネス タスクを記録し、ソース コードを作成しなくてもタスク記録を一連の自動テストに変換できるようにします。

## <a name="before-you-begin"></a>準備

### <a name="prerequisites"></a>必要条件

- このチュートリアルでは、Microsoft Dynamics 365 for Finance and Operations バージョン 10.0 (2019 年 4 月) またはそれ以降を実行する Finance and Operations 環境が必要です。 Microsoft Dynamics 365 for Finance and Operations、Enterprise Edition 7.3、プラットフォーム更新プログラム 20 (PU20) またはそれ以降を使用している場合もサポートされます。
- ユーザーは、 Finance and Operations 環境に対する管理者権限を持っている必要があります。
...
## <a name="finance-and-operations-environment"></a>Finance and Operations 環境

### <a name="version-requirement"></a>バージョン要求

バージョン 10 を実行する Finance and Operations テスト環境またはサンドボックス環境が必要です。 バージョン 7.3、プラットフォーム更新プログラム 20 またはそれ以降を使用している場合もサポートされます。

> [!NOTE]
> RSAT は、Finance and Operations テスト環境に Web ブラウザーを介してアクセスできる必要があります。
...
このステップは、Finance and Operations クライアントを介して LCS の適切な BPM ライブラリにタスクの記録を保存できるように LCS と接続するために必要です。
...
> [!NOTE]
> すべてのタスクの記録が、Finance and Operations のメイン ダッシュボードから開始していることを確認してください。 個々の記録を短くし、販売注文の作成など、1 つのビジネス タスクに注目します。
...
RSAT は、Windows 10 を実行していて、Web ブラウザー (Internet Explorer または Google Chrome) を介して Finance and Operations 環境に接続できるすべてのコンピューターにインストールできます。
...
> [!NOTE]
> Finance and Operations 環境がレベル 2 以上である場合は、証明書の拇印は環境内の**すべての** AOS コンピューターの wif.config ファイルで更新する必要があります。
...
3. **エクスプローラー**を右クリックして**\<ドライブ\>:\\Aosservice\\WebRoot** フォルダーを開きます。 **wif.config** ファイルを検索します。
...
4. 次の例に示すように、証明書と機関名に対応する新しい機関のエントリを追加して、**wif.config** ファイルを更新します。
    ...
    > [!NOTE]
    > 複数のユーザーが同一の Finance and Operations アプリケーションを使用している場合は、各ユーザーが個別の拇印を生成する必要があります。また、それぞれの拇印は**\<キー\>** セクションで追加する必要があります。
...
レベル 2 以上 (スタンダード承認テスト サンドボックスまたはそれ以上) の Finance and Operations 環境を使用している場合は、RSAT がインストールされているコンピューターで次のレジストリ設定を確認または設定します。 認証または RSAT 接続エラーを回避するのに役立つため、このステップを実行する必要があります。
...
3. 右上隅にある**設定**ボタン (ギヤ記号) を選択し、表示されるダイアログ ボックスで、次のフィールドに情報を入力します。
    ...
    - **ホスト名** ー **\<myaos\>.cloudax.dynamics.com** などの Finance and Operations テスト環境のホスト名を入力します。 **https://** または **http://** の接頭語を含めないでください。
    - **SOAP ホスト名** ー Finance and Operations テスト環境の SOAP ホスト名を入力します。 通常、SOAP ホスト名はホスト名と同じですが、**soap** の接尾語が付いています。 次に例を示します。**\<myaos\>soap.cloudax.dynamics.com**。 **https://** または **http://** の接頭語を含めないでください。
    ...
    - **管理者ユーザー名** ー Finance and Operations テスト環境の管理者ユーザーの電子メール アドレスを入力します。
    ...
    - **会社名** ー Excel パラメーター ファイルを作成するときに、既定の Finance and Operations の会社として使用する会社名を入力します。 後で Excel パラメーター ファイルを編集して会社を変更できます。
...
6. ブラウザー ドライバーがシステムにインストールされていない場合は、「この操作には\<ブラウザー名\>ドライバーが必要です。 今すぐ自動的にダウンロードしてインストールしますか?」という警告メッセージが表示されます。 **はい**を選択します。
...
3. Excel パラメーター ファイルを再度開いて製品番号を毎回手動で更新することなく、実行ごとに一意の製品番号を生成するには、次の Excel の式を使用します。
    ...
    > [!NOTE]
    > **一般**タブに加えて、Excel パラメーター ファイルには、テスト ケースがアクセスするすべての Finance and Operations のフォーム ページのデータ タブが含まれています。
