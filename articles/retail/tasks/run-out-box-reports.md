...
7. ツリーで、「Contoso Retail\Contoso Retail USA\Central\Houston」を選択します。
    * これは、[小売レポート用] の既定の小売組織階層を示します。   組織管理 > 組織 > 組織階層の目的に移動し、小売レポートを選択し、割り当て済の階層で既定の列が選択されている階層名を確認します。 デモ データの一部として (このタスクの記録に使用)、[地域ごとの小売店舗] が [小売レポート用] の既定の組織階層となっています。     
8. [OK] をクリックします。
