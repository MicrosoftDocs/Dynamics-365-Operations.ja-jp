...
# <a name="improvements-to-statement-posting"></a>明細書転記に関する機能強化事項
...
> [!NOTE]
> **小売明細書 (従来)** コンフィギュレーション キーを代わりに使用するやむを得ない理由がない限り、改良された明細書転記機能の**小売明細書**コンフィギュレーション キーを使用することをお勧めします。 Microsoft は新しい改良された明細書転記機能に投資し続けるため、ユーザーが益を得るには早期にそれに切り替えることが重要です。 従来の明細書転記機能は、将来のリリースで削除されます。
...
さらに、**同時に転記する明細書の最大数**フィールドが、**バッチ処理**クイック タブに導入されています。 このフィールドは、同時に実行する必要があるバッチ作業の数を定義します。 現時点では、このフィールドの値を手動で設定する必要があります。

また、新しい転記プロセスについて、**小売パラメーター** ページの**転記**タブ上にある**ギフト カード** クイック タブの**ギフト カード製品**を定義する必要があります。 これは、ギフト カードが組織で使用されていない場合も同様です。 

明細書転記に関連する、さらに小売店舗および**小売パラメーター**ページで定義されるすべての設定およびパラメーターが、改良された明細書転記機能に適用されることに注意してください。
...
- 従来の明細書転記機能では、最初に発生した明細書の失敗が、バッチ全体の失敗の原因となります。 残りの明細書は処理されません。 改良された機能では、一部の明細書が失敗した場合でも、バッチ処理はすべての明細書の処理を続行します。 1 つの利点は、エラーを含む明細書の正確な数への可視性を、ユーザーが取得することです。 したがって、ユーザーはすべての明細書が転記されるまで、エラー修正および明細書転記プロセスを実行する、継続的なループで行き詰る必要はありません。
