...
高度な自動請求が有効にすると、すべて出荷または選択された出荷の顧客注文を作成するときに、ユーザーは POS 端末で手動で出荷費用を入力するよう求められます。 POS 注文諸費用は体系的に計算され、POS トランザクションに追加されます (作成する注文の条件に一致する自動請求テーブルが見つかる場合)。 ユーザーは、POS 画面レイアウトに追加できる新規に追加されたPOS操作を使用して、ヘッダーまたは明細行レベルの請求を手動で追加または維持することもできます。  
