...
-   顧客の電子メールの基本設定を収集し、POS から更新できるようにします。
...
2.  カスタム フィールドを売上票または領収書フォーマットに追加するには、次のコードに示すように **GetSalesTransactionCustomReceiptFieldServiceRequest** を実装します。 このコードは、標準コードが受領書フィールドを認識しないたびに呼び出されます。
...
完全なサンプルコードは、ソフトウェアの開発キット (SDK) の RetailSDK\\SampleExtensions\\CommerceRuntime\\Extensions.ReceiptsSamplefolder フォルダーにあります。 **注記:** カスタム レシート タイプの印刷をクライアントから呼び出す必要があります。 詳細については、[拡張性のパターンおよびベスト プラクティス](https://youtu.be/qQkHFubENIY) を参照してください。
