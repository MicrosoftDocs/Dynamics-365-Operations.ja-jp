...
7. LCSで、ご利用の環境のメインページを開き、 **環境の機能** \> **リテール** \> **管理**を選択します。
8. **拡張機能の適用** を選択し、ライブラリから拡張機能を選択します。
9. 拡張機能が正常に展開された後、 Retail Cloud Scale Unit (RCSU) に対して有効化された Retail Modern POS (MPOS) または Cloud POS (CPOS) のインスタンスを開きます。
