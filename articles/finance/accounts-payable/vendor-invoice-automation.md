...
例外請求書の新しいリスト ページは、**買掛金管理** > **請求書** > **インポートの失敗** > **インポートに失敗した仕入先請求書** にあります。 このページは、仕入先請求書ヘッダーのデータ エンティティのステージング テーブルから、すべての仕入先請求書のヘッダー レコードを表示します。 **データ管理** ワークスペースから同じレコードを表示できることに注意してください。 **データ管理** ワークスペースの例外処理機能で提供されているのと同じアクションを実行することもできます。 例外処理機能は機能ユーザー向けに最適化され、簡単に使用できます。
...
### <a name="vendor-invoices-that-failed-to-import-details-page"></a>明細のインポートに失敗した仕入先請求書ページ

編集モードを開始すると、問題が発生した **明細のインポートに失敗した仕入先請求書** ページが開きます。 添付ファイルのある請求書に問題がある場合、添付ファイルは表示されません。 添付ファイルは請求書に再添付する必要があります。

**明細のインポートに失敗した仕入先請求書** ページでは、保留中の請求書を作成できます。 例外処理の一部として請求書の問題を解決した後、**保留中の請求書の作成** ボタンをクリックして保留中の請求書を作成できます。 保留中の請求書はバックグラウンドで作成されます。 
...
### <a name="vendor-invoice-attachment"></a>仕入先請求書の添付
