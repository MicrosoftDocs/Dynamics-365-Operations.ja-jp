...
複数の勘定科目表を処理する必要がある場合、複数のオプションがあります。 **オンラインで連結する** オプションを使用し、主勘定や連結勘定グループで定義された連結勘定のどちらを使用するかを選択します。 **Financial Reporting** オプションを使用 し、行定義の財務分析コードに複数のリンクを追加し、勘定をマップすることができます。
