...
9. **グローバリゼーション機能** ワークスペースに戻り、**機能** を選択し、**税計算** タイルを選択して、**追加** を選択します。
    ...
    > [!NOTE]
    > バージョン 10.0.26 以降では **DEMF** デモのレガシ エンティティのデモ機能をインポートできます。 詳細については、[Lフィーチャー デモ データをインポートする](tax-calculation-import-export-feature.md) を参照してください。
...
15. **税コード** タブで、**追加** を選択し、税コードと説明を入力します。
16. **税コンポーネント** を選択します。 税コンポーネントは、選択された税コンフィギュレーションの以前のバージョンで定義された一連の方法です。 次の税コンポーネントを使用できます。
...
17. **保存** を選択します。 選択した税コンポーネントに基づいて、使用可能なフィールドが増加します。
18. 次のオプションを使用して、税コードの性質を識別します。
...
19. 手順 15 ～ 18 を繰り返して、必要なすべての税コードを追加します。
20. **税グループ** タブで、**税グループ** 列を選択し、マトリックスに入力条件として追加した後、税グループのマスター データを管理するための明細行を追加します。
...
21. **品目別税グループ** タブで、**品目別税グループ** 列を選択し、マトリックスに入力条件として追加した後、品目別税グループのマスター データを管理するための明細行を追加します。
...
22. **税グループの適用性** タブで、正しい税グループを決定するために必要な列を選択し、**追加** を選択します。 各列の値を入力または選択します。 **税グループ** フィールドはこのマトリックスの出力となります。 このタブが構成されていない場合、トランザクション行の売上税グループが使用されます。
...
23. **品目別税グループの適用性** タブで、正しい税コードを決定するために必要な列を選択し、**追加** を選択します。 各列の値を入力または選択します。 **品目別税グループ** フィールドはこのマトリックスの出力です。 このタブに何も設定されていない場合は、トランザクション行の品目別売上税グループが使用されます。
...
24. ビジネス ニーズに基づいて顧客税登録番号、仕入先税登録番号、およびリスト コードの適用方法を設定します。
25. **保存** を選択し、ページを閉じます。
26. **ステータスの変更** \> **完了** を選択します。 ステータスが **完了** に変更された後は、バージョンを編集できなくなります。
27. **状態の変更** \> **公開** を選択します。 このバージョンの税機能の設定は、グローバル リポジトリにプッシュされ、Finance の各法人に表示されます。
