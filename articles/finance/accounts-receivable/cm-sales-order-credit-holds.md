...
6. 注文が与信管理保留に入れられる前に、選択したブロック ルールに対して**与信限度額の有効期限**を入力します。 期限超過日数は、与信限度額が期限切れになった日数に追加された支払猶予日数を表します。
...
6. 使用されている与信限度額のテストにも使用する値タイプを定義する**値タイプ**を選択します。 ブロック ルールには割合が必要ですが、除外には固定金額または割合を使用できます。
しきい値。 しきい値は、与信限度額に関連します。
...
4. 設定するルール タイプを選択します。  
  - **ブロック**を選択すると、注文をブロックするルールが作成されます。 
  - **除外**を選択すると、注文のブロックから別のルールを除外するルールが作成されます。 
6. 注文が与信管理保留に入れられる前に、選択したブロック ルールに対して**販売注文金額**を入力します。 
...
- **すべての顧客 > 顧客を選択 > 与信および取立のクイック タブ**にある**与信管理から顧客を除外する**の設定を、**はい**に変更した場合は、この顧客の注文は処理されません
