...
description: この記事では、収益および経費繰延の繰延スケジュールについて解説します。
...
## <a name="apply-or-unapply-a-credit-note"></a>訂正票の適用または適用の解除

訂正票を適用するには、次の手順に従います。

> [!NOTE]
> **販売注文トランザクションの繰延** ページから訂正票を作成し、**既存のスケジュールの調整** オプションを **はい** に設定すると、訂正票を転記するときに、その訂正票がスケジュールに自動的に適用されます。

1. **すべての繰延スケジュール** ページで、延期スケジュールを選択します。
2. **貸方調整** リストで 行を選択し、**適用** を選択します。
3. **訂正票の適用 (繰延)** ページで、**再計算日** フィールドに再計算日を指定し、**終了日** フィールドで終了日を指定します。
4. **ヘッダー** の一覧で、訂正票を含む **販売注文** を選択します。
5. **明細行** の一覧で明細を選択します。
6.  **OK** を選択します。
7. **はい** を選択します。

> [!NOTE]
> 異なる請求書の複数の個別の品目に訂正票を適用するには、上の手順を繰り返す必要があります。
...
| 適用日 | 貸方調整が適用された日付です。 |
| **調整** | 調整値は、繰延スケジュールに対して貸方のメモが処理された場合にのみ表示されます。 訂正票が処理された場合、これらの値は非表示になります。 |
