...
**自由書式の請求書** ページで次のボタンを使用して、自由書式の請求の各金額の勘定配布を表示すること、また場合によっては変更することができます。

-   **金額の配分**—税または請求金額などの明細行と子明細行の勘定配布を表示または変更します。 また、**消費税のトランザクション** ページや **料金のトランザクション** ページから、子行の勘定配布を直接確認、変更することができます。
...
税金の勘定配布は、税金が計算されるまで作成できません。 売上税を計算するには、**自由書式の請求書** ページで次の作業のいずれかを完了する必要があります:
-   売上税を表示する。
-   請求金額の合計を表示する。
-   キャッシュ フローを表示する。
