---
title: 販売注文請求書の作成
description: この記事では、請求書の結合やバッチ処理を含む、販売注文の請求方法について説明します。
...
ms.locfileid: "9778387"
---
# <a name="create-sales-order-invoices"></a>販売注文請求書の作成

[!include [banner](../../includes/banner.md)]

この記事では、請求書の結合やバッチ処理を含む、販売注文の請求方法について説明します。 この手順では、USMF というデモ会社を使用します。
...
4. **パラメーター** セクションを展開します。
    - 請求書に転記するには、転記を **はい** に設定する必要があります。 また、転記をオフにし、請求書の印刷のみを行うことができます。 ただし、請求書のほかに、見積請求書の作成でも同じ操作を実行できます。  
    - このオプションはバッチ ジョブに使用されます。 クエリは、バッチ ジョブの実行時に実行されます。
5. **印刷** フィールドで **変更後** を選択します。
6. **請求書の印刷** に対して **オン** を選択します。 印刷管理では、請求書を複数印刷でき、請求書を PDF ファイルとしてメールで送信することもできます。  
7. **請求金額の印刷** フィールドで **集計** を選択します。
8. **与信限度額の確認** フィールドで **残高** を選択します。
9. **キャンセル** をクリックします。
