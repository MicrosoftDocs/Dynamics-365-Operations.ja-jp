...
収益計上枠の設定は、**一般会計パラメーター** ページの **収益認識** タブで構成します (**収益認識 \> 設定 \> 一般会計パラメーター**)。 次の設定を実行できます。
...
リリースされた製品の設定のコンフィギュレーションが完了したら、**収益価格** ページ (**収益認識 \> 設 \> 在庫設定 \> リリースされた製品** に移動し、操作ウィンドウ、**販売** タブ、**収益認識** グループで **収益価格** を選択)で公正価値価格または価格の中央値 (価格の中央値メソッドを使用している場合) を入力して、収益価格を手動で定義する必要があります。
