...
**開始日** と **終了日** 値では、再評価された外貨残高勘定を計算する日付間隔を定義します。 損益勘定を再評価すると、日付範囲内に行われるすべてのトランザクションの合計が再評価されます。 貸借対照表勘定を再評価すると、**開始日** は無視されます。 代わりに、再評価される残高は、会計年度の初めから **終了日** までに決定されます。 

**レートの日付** は、為替レートが既定になる日付を定義するために使用できます。 たとえば、1 月 1 日から 1 月 31 日の期間の残高を再評価することはできますが、2 月 1 日に定義された為替レートを使用します。 

再評価する主勘定を選択します: すべて、貸借対照表、または損益。 再評価用にマークされた主勘定 (**主勘定** ページ上) のみが再評価されます。 さらに主勘定の範囲を制限する場合は、**対象に含めるレコード** タブを使用して、主勘定の範囲または個々の主勘定の範囲を定義します。 
...
再評価プロセスから **レポート通貨の調整仕訳帳** を使用して転記された調整を除外する場合は、**レポート通貨の調整を除外する** を **はい** に設定します。 既定では、レポート通貨の調整が再評価に含まれます。 
