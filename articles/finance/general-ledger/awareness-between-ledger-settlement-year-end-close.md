...
description: この記事では、元帳決済と一般会計の年度末決算に影響する拡張に関する情報を提供します。
author: kweekley
ms.date: 04/06/2022
ms.topic: article
ms.prod: ''
...
- 期首残高に転送されるには決済済トランザクションのみであるため、前会計年度のトランザクションで決済された現在の会計年度からのトランザクションの決済を解除する必要があります。 トランザクションは、現在の会計年度内のトランザクションに対して決済を解除する必要があります。 これは、現在の会計年度の調整エントリを使用して実行できます。 調整は、集計された期首残高を取り消し、今年の元帳エントリを決済するために必要な詳細なトランザクションで相殺されます。 

  > [!IMPORTANT]
  > これを行わない場合、現在の会計年度の年度末決算を実行するときに、**不均衡** エラーが発生します。 同じ会計年度の元帳トランザクションの決済および決済解除を実行できない場合、年度末決算が完了するまで、この機能を有効にしないでください。 年度末決算が完了した直後、かつ新しい元帳トランザクションが次の会計年度で決済される前に、この機能を有効にします。 
