...
元帳配賦ルールを使用すると、自動的に配賦仕訳帳と勘定項目の計算と生成を行い、元帳残高や固定金額を配賦できます。 配賦方法には、変動的または固定があります。 配賦は、トランザクション通貨の値に基づいて行います。 たとえば、外貨の損益勘定項目は転記され、会計およびレポートの通貨金額を調整します。 トランザクション通貨の値は 0.00 なので、これらのエントリは配賦ルールの対象にはなりません。 次の配賦方法が、元帳配賦ルールに使用できます。
