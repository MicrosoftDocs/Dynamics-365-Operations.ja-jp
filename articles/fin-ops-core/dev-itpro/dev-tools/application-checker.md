...
Application チェッカー ツールは、アプリケーション コード (ソースとメタデータの両方) を、これまではできなかった方法で把握できるようにする一連のテクノロジです。 このテクノロジは、XML のソース コードとメタデータの両方の表現に基づいており、XQuery 言語を使用してソースコードに対する宣言クエリを表すことによって、豊富な検索機能を提供します。 現在の実装は、BaseX リポジトリ内で実行され、開発者のボックスでローカルに実行されます。 
