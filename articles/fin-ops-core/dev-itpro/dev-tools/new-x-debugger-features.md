...
## <a name="declare-anywhere"></a><span data-ttu-id="c7d6d-112">任意の場所で宣言</span><span class="sxs-lookup"><span data-stu-id="c7d6d-112">Declare anywhere</span></span>
...
### <a name="example-with-new-features"></a><span data-ttu-id="c7d6d-137">新しい機能の使用例</span><span class="sxs-lookup"><span data-stu-id="c7d6d-137">Example with new features</span></span>
...
<span data-ttu-id="c7d6d-157">単一は、クラスのインスタンスが 1 つしか呼び出されないことを保証します。これは、以下で消費されます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-157">The singleton will guarantee that only one instance of the class will ever be called, which is consumed by the following.</span></span>
...
<span data-ttu-id="c7d6d-163">インライン割り当ては、静的メンバーとインスタンス メンバーの両方で機能します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-163">The inline assignments work for both static and instance members.</span></span>
...
<span data-ttu-id="c7d6d-181">次に、定数を二重コロン構文を使用して参照することができます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-181">The constants can then be referenced by using the double-colon syntax.</span></span>
...
## <a name="var"></a><span data-ttu-id="c7d6d-192">Var</span><span class="sxs-lookup"><span data-stu-id="c7d6d-192">Var</span></span>
...
- <span data-ttu-id="c7d6d-206">プライベート メンバーは、定義されているクラス内でのみ使用できます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-206">A private member can only be used within the class where it's defined.</span></span>
...
<span data-ttu-id="c7d6d-219">try/catch 文にオプションの finally 句を追加できるようになりました。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-219">Try/catch statements can now include an optional finally clause.</span></span> <span data-ttu-id="c7d6d-220">セマンティクスは C\# やその他のマネージド言語と同じです。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-220">The semantics are the same as they are in C\# and other managed languages.</span></span> <span data-ttu-id="c7d6d-221">finally 句のステートメントは、通常または例外を介してコントロールが try ブロックを離れるときに実行されます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-221">The statements in the finally clause are executed when control leaves the try block, either normally or through an exception.</span></span>
...
- <span data-ttu-id="c7d6d-238">拡張クラスは静的でなければなりません。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-238">The extension class must be static.</span></span>
...
### <a name="example-of-using-clause"></a><span data-ttu-id="c7d6d-271">Using 句の例</span><span class="sxs-lookup"><span data-stu-id="c7d6d-271">Example of using clause</span></span>
...
### <a name="exercise"></a><span data-ttu-id="c7d6d-303">練習</span><span class="sxs-lookup"><span data-stu-id="c7d6d-303">Exercise</span></span>
...
### <a name="example---casting"></a><span data-ttu-id="c7d6d-331">例 - キャスト</span><span class="sxs-lookup"><span data-stu-id="c7d6d-331">Example - casting</span></span>
...
## <a name="the-ignore-list"></a><span data-ttu-id="c7d6d-365">Ignore リスト</span><span class="sxs-lookup"><span data-stu-id="c7d6d-365">The Ignore list</span></span>
...
### <a name="identifying-unselected-fields"></a><span data-ttu-id="c7d6d-381">選択されていないフィールドの識別</span><span class="sxs-lookup"><span data-stu-id="c7d6d-381">Identifying unselected fields</span></span>
...
<span data-ttu-id="c7d6d-397">デバッガーを使用すると、アプリケーションの状態の特定の部分にアクセスできます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-397">The debugger will allow you to easily access certain parts of the state of the application.</span></span> <span data-ttu-id="c7d6d-398">この情報は、現在の会社、パーティション、トランザクション レベル、および現在のユーザー ID が一覧表示される [自動] ウィンドウで使用できます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-398">This information is available in the autos window, where the current company, the partition, the transaction level, and the current user ID are listed.</span></span>

![自動ウィンドウ](./media/autos_debugfeatures.png)

<span data-ttu-id="c7d6d-400">また、Infolog に書き込まれるデータを表示するウィンドウもあります。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-400">There is also a window showing the data that is written to the Infolog.</span></span>

![情報ログ ウィンドウ](./media/infolog_debugfeatures.png)

### <a name="new-breakpoint-features"></a><span data-ttu-id="c7d6d-402">新しいブレークポイント機能</span><span class="sxs-lookup"><span data-stu-id="c7d6d-402">New breakpoint features</span></span>

<span data-ttu-id="c7d6d-403">Visual Studio デバッガーでは、条件付きブレークポイントと、ヒット カウントによってトリガーされたブレークポイントをサポートします。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-403">The Visual Studio debugger supports conditional breakpoints and breakpoints that are triggered by hit count.</span></span> <span data-ttu-id="c7d6d-404">また、ブレークポイントをヒットするとき、ユーザーのためにシステムが特定のアクションを実行させることができます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-404">You can also have the system perform specific actions for you as you hit the breakpoint.</span></span> <span data-ttu-id="c7d6d-405">これらの機能のいずれも従来のデバッガーでは使用できませんでした。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-405">None of these features were available in the legacy debugger.</span></span> <span data-ttu-id="c7d6d-406">これらについて以下に詳しく説明します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-406">These are explained below:</span></span>

- <span data-ttu-id="c7d6d-407">ヒット カウントを使用することで、デバッガーが実行を中断する前にブレークポイントがヒットする回数を決定できます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-407">Hit count enables you to determine how many times the breakpoint is hit before the debugger breaks execution.</span></span> <span data-ttu-id="c7d6d-408">既定では、デバッガーはブレークポイントがヒットするたびに実行を中断します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-408">By default, the debugger breaks execution every time that the breakpoint is hit.</span></span> <span data-ttu-id="c7d6d-409">ブレークポイントが 2 回ヒットするたび、または 10 回ヒットするたび、または 512 回ヒットするたび、またはユーザーが選択した回数ヒットするたびにデバッガーが中断するよう、ヒット カウントを設定することができます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-409">You can set a hit count to tell the debugger to break every 2 times the breakpoint is hit, or every 10 times, or every 512 times, or any other number you choose.</span></span> <span data-ttu-id="c7d6d-410">一部のバグは、最初にユーザー プログラムがループを実行し、関数を呼び出し、または変数へアクセスする際に現れないため、ヒット数は役に立ちます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-410">Hit counts can be useful because some bugs don't appear the first time your program executes a loop, calls a function, or accesses a variable.</span></span> <span data-ttu-id="c7d6d-411">場合によっては、100 回または 1000 回繰り返すまでバグが表示されない可能性があります、</span><span class="sxs-lookup"><span data-stu-id="c7d6d-411">Sometimes, the bug might not appear until the 100th or the 1000th iteration.</span></span> <span data-ttu-id="c7d6d-412">このような問題をデバッグするには、ヒット数が 100 または 1000 のブレークポイントを設定します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-412">To debug such a problem, you can set a breakpoint with a hit count of 100 or 1000.</span></span>
- <span data-ttu-id="c7d6d-413">条件は、ブレークポイントがヒットするかスキップするかを決定する式です。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-413">Condition is an expression that determines whether the breakpoint is hit or skipped.</span></span> <span data-ttu-id="c7d6d-414">デバッガーは、ブレークポイントに到達すると、条件を評価します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-414">When the debugger reaches the breakpoint, it'll evaluate the condition.</span></span> <span data-ttu-id="c7d6d-415">条件が満たされた場合にのみ、ブレークポイントにヒットします。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-415">The breakpoint will be hit only if the condition is satisfied.</span></span> <span data-ttu-id="c7d6d-416">場所ブレークポイントを持つ条件を使用して、特定の条件が true の時にのみ指定された場所で停止することができます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-416">You can use a condition with a location breakpoint to stop at a specified location only when a certain condition is true.</span></span> <span data-ttu-id="c7d6d-417">たとえば、勘定残高がマイナスにならないように、銀行決済プログラムをデバッグしているとします。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-417">For example, suppose you're debugging a banking program where the account balance is never allowed to go below zero.</span></span> <span data-ttu-id="c7d6d-418">コード内の特定の場所でブレークポイントを設定し、残高 &lt; 0 などの条件をそれぞれに添付する場合があります。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-418">You might set breakpoints at certain locations in the code and attach a condition such as balance &lt; 0 to each one.</span></span> <span data-ttu-id="c7d6d-419">プログラムを実行するとき、残高が 0 より小さい場合にのみこれらの場所で実行が中断されます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-419">When you run the program, execution will break at those locations only when the balance is less than zero.</span></span> <span data-ttu-id="c7d6d-420">最初のブレークポイントの位置で変数およびプログラムの状態を調べ、2 つ目のブレークポイントの位置まで実行を続行、などを実行することができます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-420">You can examine variables and program state at the first breakpoint location, and then continue execution to the second breakpoint location, and so on.</span></span>
- <span data-ttu-id="c7d6d-421">アクションは、ブレークポイントがヒットした場合に発生する必要のあるものを指定します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-421">Action specifies something that should occur when the breakpoint is hit.</span></span> <span data-ttu-id="c7d6d-422">既定では、デバッガーは実行を中断しますが、代わりにメッセージを印刷するか Visual Studio マクロを実行するかを選択できます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-422">By default, the debugger breaks execution, but you can choose to print a message or run a Visual Studio macro instead.</span></span> <span data-ttu-id="c7d6d-423">中断するのではなく、メッセージを印刷する場合は、ブレークポイントは Trace ステートメントと非常に類似します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-423">If you decide to print a message instead of breaking, the breakpoint has an effect very similar to a Trace statement.</span></span> <span data-ttu-id="c7d6d-424">このブレークポイントを使用するメソッドはトレース ポイントと呼ばれます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-424">This method of using breakpoints is called trace points.</span></span>

#### <a name="using-breakpoints-with-conditions"></a><span data-ttu-id="c7d6d-425">条件を伴うブレークポイントの使用</span><span class="sxs-lookup"><span data-stu-id="c7d6d-425">Using breakpoints with conditions</span></span>

<span data-ttu-id="c7d6d-426">次のコードを考慮してください。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-426">Consider the following code:</span></span>
...
<span data-ttu-id="c7d6d-427">そのステートメントが選択されているときに F9 キーを押すことで、印刷明細書にブレークポイントを設定します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-427">Put a breakpoint on the print statements by pressing F9 while that statement is selected.</span></span> <span data-ttu-id="c7d6d-428">これにより、通常の無条件ブレークポイントが作成されます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-428">This will create a normal, unconditional breakpoint.</span></span> <span data-ttu-id="c7d6d-429">ここで、マウスを使用して、ブレークポイントのコンテキスト メニューを開き、**条件** を選択します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-429">Now, use the mouse to open the context menu for the breakpoint and select **Condition**.</span></span> <span data-ttu-id="c7d6d-430">"i" 変数が 5 を超えたときにブレークポイントが発生することを示す条件を配置します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-430">Put in a condition that indicates that the breakpoint should happen when the value of the 'i' variable exceeds 5.</span></span> <span data-ttu-id="c7d6d-431">スタートアップ プロジェクトとしてクラスを設定し、プロジェクト内のスタートアップ項目としてコードを含むクラスを設定します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-431">Set the class as a startup project, and the class containing the code as the startup item in the project.</span></span> <span data-ttu-id="c7d6d-432">コードを実行します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-432">Run the code.</span></span> <span data-ttu-id="c7d6d-433">デバッガーを使用して「i」の値を自由に変更してください。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-433">Feel free to modify the value of 'i' using the debugger.</span></span> <span data-ttu-id="c7d6d-434">ここで、このブレークポイントを削除し、ヒット カウント機能を使用して同じことを実現します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-434">Now, remove this breakpoint, and use the Hit count feature to accomplish the same thing.</span></span>

> [!NOTE]
> <span data-ttu-id="c7d6d-435">ブレークポイントには、いくつかの条件があります。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-435">A breakpoint can have several conditions.</span></span> <span data-ttu-id="c7d6d-436">多くの場合、ブレークポイントにカーソルを置くと役立つツールヒントが表示されれば便利です。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-436">It's often helpful to hover the cursor over the breakpoint, causing an informative tooltip to appear.</span></span> <span data-ttu-id="c7d6d-437">トレース ポイントは、しばしばトレースの実行に役立ちます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-437">Trace points are often useful tot race execution.</span></span> <span data-ttu-id="c7d6d-438">対象の行に追跡ポイントを挿入し、変数の値を記録します。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-438">Insert a trace point on the line in question and log the value of the variable.</span></span> <span data-ttu-id="c7d6d-439">トレース出力がデバッガーの出力ウィンドウに表示されます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-439">The trace output will appear in the output window in the debugger.</span></span>

### <a name="the-immediate-window"></a><span data-ttu-id="c7d6d-440">直接ウィンドウ</span><span class="sxs-lookup"><span data-stu-id="c7d6d-440">The immediate window</span></span>

<span data-ttu-id="c7d6d-441">直接ウィンドウは VS デバッガーの便利な機能で、ユーザーはいつでも評価するために式と明細書を入力できます。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-441">The immediate window is a useful feature in the VS debugger that allows the user to enter expression and statements to evaluate at any given time.</span></span> <span data-ttu-id="c7d6d-442">この機能は現在、X++スタックには実装されていません。多くの言語、特に \# の場合と同様です。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-442">This feature isn't currently implemented in the X++ stack, as is the case for many other languages, notably F\#.</span></span> <span data-ttu-id="c7d6d-443">ただし、それは精通したユーザーが直接ウィンドウから益を得られないという意味ではありません。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-443">However, that doesn't mean that the savvy user can't benefit from the immediate window.</span></span> <span data-ttu-id="c7d6d-444">スニペットは X++ ではなく C\# で表す必要があることを示すだけです。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-444">It just means that snippets must be expressed in C\#, not in X++.</span></span> <span data-ttu-id="c7d6d-445">これがどのように大きな効果を発揮するかについての詳細を記述した、別のドキュメントがあります。</span><span class="sxs-lookup"><span data-stu-id="c7d6d-445">There's a separate document that describes the details of how this can be done to great effect.</span></span>
