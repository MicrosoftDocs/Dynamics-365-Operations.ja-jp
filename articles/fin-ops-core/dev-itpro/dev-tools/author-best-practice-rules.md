...
-   ベスト プラクティス コンフィギュレーション ダイアログのボタンを使用します。 **拡張機能をインストール** ボタンをクリックします。 自分のルール (つまり、ルール作成時に生成される DLL) を含むアセンブリ ファイルを指さすように求められます。 [OK] を押すと、システムが必要な DLL をコピーします (以下を参照)。
