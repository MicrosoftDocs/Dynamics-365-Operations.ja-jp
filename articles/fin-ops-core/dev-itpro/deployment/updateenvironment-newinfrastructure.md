...
運用環境に適用される更新プログラムの信頼性を向上させるために、サンドボックス環境で実行される特定の更新プログラムを **リリース候補** として指定 ("マーク") し、実稼働環境に移動します。 この更新プログラムには、基本製品とカスタマイズの両方が含まれています。 したがって、選択した更新プログラムを実稼働環境に適用したときに、両方が移動します。 この動作は現在のサービスとは異なります。 現時点では、カスタマイズのみを移動できますが、基本製品は変更 **されません** 。 セルフサービス配置環境では、サンドボックス環境でリリース候補としてマークされている更新によって、基本製品バージョンが **変わる場合があります**。 セルフサービス リリース候補者がマークされると、リリース候補者として個別にマークされているかどうかに関係なく、以前に適用されたすべての更新も含まれます。 
