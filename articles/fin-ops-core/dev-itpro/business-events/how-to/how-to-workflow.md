...
Microsoft Flow では、アプリケーションを自動的にコンフィギュレーションできます。 フローを保存すると、Microsoft Flow でエンドポイントが作成され、ビジネス イベントが有効となります。 その他のコンフィギュレーションを完了する必要はありません。 エンドポイントが正しく構成されてたことを確認してからイベントをトリガーする必要があります。
