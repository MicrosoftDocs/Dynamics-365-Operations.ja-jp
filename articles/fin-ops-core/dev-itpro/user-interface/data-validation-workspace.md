...
この記事では、**データ検証チェックリスト ワークスペース** と関連するコンフィギュレーションの概要を示します。
...
最後のタブは、詳細なタスクのリストのためにあります。 この一覧は、すべてのタスク リストを表示します。 複数の方法でタスク リストをフィルター処理できます。 **タスクの編集** をクリックしてタスクの状態を変更するかタスクを割り当てます。 **添付ファイル** をクリックしてタスクの添付ファイルを表示します。

タスク名はユーザーが作業を完了するために閲覧する必要があるページへのハイパーリンクです。 **データ検証プロジェクトのコンフィギュレーション** フォームからタスクを編集または作成する際に **メニュー項目名** フィールド を使用してこのハイパーリンクを設定できます。
