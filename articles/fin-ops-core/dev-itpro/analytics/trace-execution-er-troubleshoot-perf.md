...
### <a id='export-trace'></a> アプリケーションから生成された追跡のエクスポート
...
このトピックの先のセクション [ER コンフィギュレーションを RCS から Finance and Operations にインポートする](#import-configuration) の手順を繰り返して、**パフォーマンス追跡マッピング** コンフィギュレーションのバージョン 1.2 をインポートします。
