...
1.  **組織管理** \> **ワークスペース** \> **電子レポート**に移動します。
2.  **電子レポート** ワークスペースで、 **電子レポート パラメーター**を選択します。
3.  **電子レポートパラメーター** のページの、**全般** タブで、**デザイン モードを有効化する**オプションを**はい**に設定します。
4.  **添付** タブに、以下のパラメータを設定します :
...
2. **名前** フィールドに、 **Litware, Inc.** と入力します。
...
新しいロジックを開発するには、継続的なビルドに対応したトポロジをデプロイする必要があります。 詳細については、 [継続的なビルドとテストの自動化に対応したトポロジをデプロイする](../perf-test/continuous-build-test-automation.md)を参照してください。 また、このトポロジの開発環境にもアクセスできる必要があります。 利用可能な ER の API については、[ER フレームワークの API](er-apis-app73.md) を参照してください。
