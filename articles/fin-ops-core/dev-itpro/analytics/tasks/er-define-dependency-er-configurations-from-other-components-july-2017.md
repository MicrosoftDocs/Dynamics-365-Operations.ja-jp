...
8. [バージョン] フィールドで、「[7.1.1541.3036,8]」を入力します。
    * [7.1.1541.3036,8]  
    * 入力した依存関係は、このコンフィギュレーションが ER リポジトリからダウンロードされるときに評価されます。 この設定バージョンは、「サンプル データ モデル」設定のバージョン 1 が既に存在するか、事前にダウンロードされている場合に、ER リポジトリからダウンロードされます。 事前にダウンロードされている場合は、財務と運用のバージョン 7.1.1541.3036 か、それ以降のバージョンで完了する必要があります。但し、メジャー バージョン 8 を超えてはいけません。   
9. [保存] をクリックします。
10. ページを閉じます。
11. [状態の変更] をクリックします。
12. [完了] をクリックします。
...
26. ツリーで、「サンプル データ モデル」を展開します。
    ...
    * 財務と運用にログインする場合は、同じプロバイダーを登録し、同じ LCS プロジェクトへアクセスし、同じデータ モデル構成をダウンロードし、「サンプル マッピング (代替)」構成がダウンロードされますが、「サンプル マッピング」構成はスキップされます。  
