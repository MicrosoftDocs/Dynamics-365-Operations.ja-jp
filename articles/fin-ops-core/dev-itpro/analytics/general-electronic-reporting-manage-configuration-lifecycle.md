...
この記事では、Dynamics 365 Finance の [電子申告](general-electronic-reporting.md) (ER) [構成](general-electronic-reporting.md#Configuration) のライフサイクルを管理する方法について説明します。
...
## <a name="dependencies-on-other-components"></a>他のコンポーネントへの依存

ER コンフィギュレーションは、他のコンフィギュレーションに [依存](er-download-configurations-global-repo.md#import-filtered-configurations) するように構成できます。 たとえば、グローバル リポジトリから [Microsoft Regulatory Configuration Services (RCS)](../../../finance/localizations/rcs-overview.md) または Dynamics 365 Finance インスタンスに ER [データ モデル](er-overview-components.md#data-model-component) コンフィギュレーションを [インポート](er-download-configurations-global-repo.md) し、インポートされた ER データ モデル コンフィギュレーションから [派生](er-quick-start2-customize-report.md#DeriveProvidedFormat) する新しい ER [形式](er-overview-components.md#format-component) コンフィギュレーションを作成できます。 派生した ER 形式のコンフィギュレーションは基本となる ER データ モデルのコンフィギュレーションに依存します。
...
![コンフィギュレーション ページでの ER 形式のコンフィギュレーション バージョンの XML としてのエクスポート。](./media/ger-configuration-lifecycle-img3.png)
...
> 未解決の参照が残っています<br>
オブジェクト '\<imported configuration name\>' からオブジェクト 'Base' (\<globally unique identifier of the missed base configuration\>、\<version of the missed base configuration\>) への参照を確立できません

![コンフィギュレーション リポジトリ ページでの ER 形式のコンフィギュレーション バージョンのインポート。](./media/ger-configuration-lifecycle-img4.gif)
...
> 未解決の参照が残っています<br>
オブジェクト '\<imported configuration name\>' からオブジェクト 'Base' (\<name of the missed base configuration\> \<globally unique identifier of the missed base configuration\>、\<version of the missed base configuration\>) への参照を確立できません

![基本コンフィギュレーションが見つからない場合のコンフィギュレーション リポジトリ ページの例外。](./media/ger-configuration-lifecycle-img5.png)

指定した名前を使用して基本コンフィギュレーションを検索し、手動でインポートできます。

> [!NOTE]
> この新しいオプションは、少なくとも 1 人のユーザーが、[コンフィギュレーション リポジトリ](er-download-configurations-global-repo.md#open-configurations-repository) ページまたは現在の Finance インスタンスのグローバル リポジトリ [ルックアップ](er-extended-format-lookup.md) フィールドのいずれかを使用してグローバル リポジトリにすでにサインインしていて、グローバル リポジトリのコンテンツがキャッシュされている場合にのみ機能します。
