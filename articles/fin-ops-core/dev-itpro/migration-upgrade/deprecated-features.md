...
| **別の機能で置き換えられているか?**   | コンパイラの警告は、プラットフォーム更新プログラム 26 では、エラーに置き換えられます。 この変更は実行時に下位互換性があます。つまり、プラットフォーム更新プログラム 25 または新しいプログラムを実行している場合、すべてのサンドボックスまたはカスタム コードを修正する必要のない運用環境上で配置されます。 この変更は開発およびコンパイル時にのみ影響します。|
...
| **別の機能で置き換えられているか?**   | 警告は、プラットフォーム更新プログラム 26 でコンパイル エラーに置き換えられます。 この変更は実行時に下位互換性があます。つまり、プラットフォーム更新プログラム 25 または新しいプログラムを実行している場合、すべてのサンドボックスまたはカスタム コードを修正する必要のない運用環境上で配置されます。 この変更は開発およびコンパイル時にのみ影響します。 |
| **影響を受ける製品領域**         | Visual Studio 開発ツール |
| **配置オプション**              | すべて |
| **ステータス**                         | 非推奨: 警告はプラットフォーム更新プログラム 26 ではエラーになります。 |
