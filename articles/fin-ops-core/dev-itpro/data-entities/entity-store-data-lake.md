...
## <a name="entity-store-data-in-azure-data-lake-full-push-and-trickle-feed"></a>Azure Data Lake (フル プッシュおよびトリクル フィード) のエンティティ格納データ
...
1. **ストレージ アカウントを作成する。** 環境がプロビジョニングされているのと同じデータ センター内のストレージ アカウントのプロビジョニング。 その後で指定する必要があるため、ストレージ アカウントの接続文字列をメモします。
