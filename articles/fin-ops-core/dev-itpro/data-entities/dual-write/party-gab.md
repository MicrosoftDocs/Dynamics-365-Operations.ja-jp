...
4. Finance and Operations アプリを開きます。 データ管理モジュールに移動し、[二重書き込み] タブを選択します。二重書き込み管理ページが開きます。
...
11. 既存の二重書き込みユーザーである場合は、[当事者およびグローバル アドレス帳モデルへのアップグレード](upgrade-party-gab.md)の手順に従って、データをアップグレードしてください。

12. 次の順序でマップを実行します。 次のようなエラーの状態、「プロジェクトの検証に失敗しました。 出力先フィールドがありません...」が発生したら、マップを開き、**テーブルの更新** を選択します 。 次に、マップを実行します。
