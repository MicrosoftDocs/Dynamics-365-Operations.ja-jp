...
description: この記事では、デュアル書き込みでサポートされている通貨の小数点以下の桁数を変更する方法について説明します。
...
この財務と運用アプリと Dataverse は、通貨値の小数点以下の桁数が同じであることをサポートする必要があります。 これを行わないと、この情報がアプリ間で同期されている場合にデータが失われる可能性があります。 移行プロセスでは、通貨と為替レートの値の格納方法が再設定されますが、データは変更されません。 移行が完了した後、通貨コードと価格の小数桁数を増やすことができ、ユーザーが入力および表示するデータの小数点以下の精度を高めることができます。
...
同時に、為替レートが変更され、現在の 10 回の制限ではなく 12 桁までの小数位をサポートするようになります。 この変更は、小数点以下の桁数が財務と運用アプリと Dataverse の両方で同じになるようにする必要があります。
...
### <a name="default-currency-decimal-precision"></a>既定の通貨に使用する小数点以下の精度
移行と非移行のシナリオで、既定の通貨に使用する小数点以下の精度で予想される動作については、次の表を参照してください。 

| 作成日  | 通貨の小数点以下フィールド    | 既存の組織 (通貨フィールドを移行しない) | 既存の組織 (通貨フィールドを移行する) | ビルド 9.2.21062.00134 より後に作成した新しい組織 |
|---------------------------------------------------------|-------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|------------------------------------------------|
| ビルド 9.2.21111.00146 より前に作成した通貨フィールド  |     |  |       |
|    | UI に表示する最大精度   | 4 桁    | 10 桁    | 在庫切れ    |
| | データベースと DB クエリの結果 UI に表示する最大精度         | 4 桁   | 10 桁   | 在庫切れ    |
| ビルド 9.2.21111.00146 以降で作成した通貨フィールド |    |  |     |   |
|   | UI に表示する小数点以下の最大精度     | 4 桁   | 10 桁   | 10 桁     |
|          | データベースと DB クエリの結果 UI に表示する小数点以下の最大精度 | 10 桁。 ただし、影響するのは 4 桁だけで、小数点以下 4 桁の後はすべてゼロです。 これにより必要に応じて、組織の移行がより簡単かつ迅速になります。 | 10 桁      | 10 桁     |
