...
4. 次の例に示すように、証明書と機関名に対応する新しい機関のエントリを追加して、**wif.config** ファイルを更新します。
    ...
    > [!NOTE]
    > 複数のユーザーが同一のアプリケーションを使用している場合は、各ユーザーが個別の拇印を生成する必要があります。また、それぞれの拇印は**\<キー\>** セクションで追加する必要があります。
...
3. 右上隅にある**設定**ボタン (ギヤ記号) を選択し、表示されるダイアログ ボックスで、次のフィールドに情報を入力します。
    ...
    - **ホスト名** ー **\<myaos\>.cloudax.dynamics.com** などのテスト環境のホスト名を入力します。 **https://** または **http://** の接頭語を含めないでください。
