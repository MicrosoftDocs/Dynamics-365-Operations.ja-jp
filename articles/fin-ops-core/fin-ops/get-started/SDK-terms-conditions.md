...
**c) 制限。**  以下を行うことはできません (i) カスタマイズ パッケージを Dynamics 365 for Finance and Operations のライブ運用環境にデプロイすること; (ii) 本ソフトウェアまたはカスタマイズ パッケージをサード パーティに配布すること; または (iii) Microsoft が別の契約に基づいて許可する場合を除き、サード パーティが本ソフトウェアを使用して作成した Web サイトをインターネット経由でアクセスできるようにすること。 Microsoft は本ソフトウェアに対しコンポーネントを随時追加または削除する場合があります。
