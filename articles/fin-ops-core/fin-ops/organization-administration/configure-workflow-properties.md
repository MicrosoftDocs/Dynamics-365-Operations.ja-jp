...
description: この記事では、ワークフローの各種プロパティをコンフィギュレーションする方法について説明します。
author: ChrisGarty
ms.date: 07/07/2020
ms.topic: article
ms.prod: ''
...
この記事では、ワークフローの各種プロパティをコンフィギュレーションする方法について説明します。
...
5. テキストをカスタマイズするには、プレースホルダーを挿入します。 プレースホルダーは、テキストがユーザーに表示されるときに、適切なデータに置き換えられます。 プレースホルダーを挿入するには、次の手順に従います。
    ...
    3. 表示されるリストで、挿入するプレースホルダーを選択します。
    4. **挿入** をクリックします。
    5. 対象に含める共通の **通知テキスト** プレースホルダーは、前の手順のコメントを表示する "Last Notes: %Workflow.Last note%" です。
...
7. **受信者** タブで、次のオプションを使用して、通知の受信者を指定します。
    ...
    <li><strong>ユーザー</strong>タブの<strong>利用可能なユーザー</strong>一覧には、すべての Dynamics AX ユーザーが含まれます。 通知の送信先のユーザーを選択して、それらのユーザーを <strong>選択されたユーザー</strong> リストに移動します。</li>
    </ol>
    </td>
    </tr>
    </tbody>
