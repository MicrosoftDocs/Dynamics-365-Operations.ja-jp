---
title: 分散型注文管理 (DOM)
description: この記事では、Microsoft Dynamics 365 Commerce の分散型注文管理 (DOM) の機能について説明します。
author: josaw1
ms.date: 11/16/2022
ms.topic: index-page
...
DOM では、混合整数計画法 (MIP) と予測分析モデルを使用して、バッチ レベルと個々の注文レベルの両方で最適化を実行します。 この能力により、小売業者は定義済みのルールを使用して、競合する注文フルフィルメントの多くのニーズのバランスを取ることができます。 製品のフルフィルメントを複数のチャネルから提供できる最新の供給ネットワークでは、組織は注文の変更、仕入先の可用性の問題、需要のスパイクに迅速に対応する必要があります。 DOM を使用すると、最も近いソースから注文を履行することでコストを最小化するなど、ビジネスの制約と目標に基づいて、注文フルフィルメントを最大化し、製品の配送に適したソースを探すことができます。 DOM では、製品のフルフィルメント ソースと出荷先間の距離、最適化の目的として定義されている原価係数、およびフルフィルメント ノードの在庫などの、制約として定義されているルールを使用して、注文フルフィルメントを最適化します。 DOM では、ビジネスの種類や消費者セグメントに応じて企業がさまざまな最適化戦略を実行できるように、複数のプロファイルを定義できます。 

次の図は、DOM システムにおける販売注文のライフサイクルを示しています。

![DOM のコンテキストにおける販売注文のライフサイクル。](./media/flow.png "DOM のコンテキストにおける販売注文のライフサイクル")

次のビデオでは、Dynamics 365 Commerce における DOM の機能の概要を示します。


> [!VIDEO https://www.microsoft.com/videoplayer/embed/RE5bRYl]

## <a name="set-up-dom"></a>DOM の設定
...
5. **ソルバー** タブで、次の値を設定します。
    ...
    - **ソルバー タイプ** – 値を選択します。 Commerce では、**本番ソルバー** と **簡易ソルバー** の 2 種類のソルバー タイプがあります。 DOM を実行するすべてのマシン (DOMBatch グループに含まれるすべてのサーバー) で、**本番ソルバー** を選択する必要があります。 本番ソルバーでは、運用環境に既定でライセンスされ、配置される特別なライセンス キーが必要です。 新しいレベル 2+ 環境では、本番ソルバーが最初から有効になります。 非運用環境では、このライセンス キーは手動で配置する必要があります。 ライセンス キーを手動で配置するには、次の手順を実行します。
...
8. DOM 機能では、さまざまなタイプの DOM ルールの定義がサポートされており、企業のビジネス ニーズに応じて、複数のルールを構成することができます。 DOM ルールは、場所のグループまたは個々の場所ごと、および、特定の製品カテゴリ、製品、バリアントに対して定義することができます。 DOM ルールに使用する必要がある場所のグループを作成するには、次の手順を実行します。
    ...
    > [!NOTE]
    > Commerce バージョン 10.0.12 以上では、**フルフィルメント グループ内で、場所を出荷または集荷と指定する機能** が **機能管理** ワークスペースで有効になっている必要があります。
    >
    > この機能は、**フルフィルメント グループ** ページに新しいコンフィギュレーションを追加して、倉庫を出荷に使用できるかどうか、倉庫/店舗の組み合わせを出荷、集荷、またはその両方に使用できるかどうかを定義できます。 
    >
...
9. ルールを定義するには、**Retail と Commerce \> 配分済み注文の管理 \> 設定 \> ルールを管理する** の順にクリックします。 次の DOM ルールが現在サポートされています。
    ...
    - **最小在庫ルール** – このルール タイプでは、一定量の製品を注文のフルフィルメント以外の目的で確保しておくことができます。 たとえば、店舗にある在庫のすべてを、DOM で注文のフルフィルメントに使用されると困る場合があります。 一定の在庫は、来店する顧客用に確保しておく必要があるためです。 このルール タイプを使用すると、1 つの場所または場所のグループごとに、製品のカテゴリ、個別の製品、または製品バリアントについて、確保しておく必要がある最小限の在庫を定義できます。 また、補足カテゴリ階層を使用して最小限の在庫を定義することもできます。 製品が複数のカテゴリに分類される場合は、カテゴリを使用できるすべてのルールに対し、最も高い重要度が補足カテゴリに与えられます。
    - **フルフィルメント場所優先順位ルール** – このルール タイプでは、場所の階層を定義して、DOM エンジンが特定の製品についてフルフィルメント場所を特定する際の優先順位を指定できます。 優先順位の有効な範囲は 1 ～ 10 で、1 が最も優先度が高く、10 が最も優先度が低くなります。 優先順位の高い場所が、優先順位の低い場所よりも先に検討対象となります。 ルールが厳格な制約として定義されている場合、注文は、優先順位が定義されている場所にのみ仲介されます。 DOM では、1 か所からすべての注文を出荷することを優先させます。 したがって、優先順位が 1 の場所で注文全体とその明細行に対応できない場合、DOM では優先順位が 2 の場所から注文を履行しようとします。
    - **部分注文ルール** – Retail バージョン 10.0.5 では、**1 つの場所からのみ注文をフルフィルメント** パラメーターは、**最大のフルフィルメント場所** に変更されました。 古いパラメーターでは、ユーザーは注文を 1 つの場所からのみ履行するか、できる限り多くの場所から履行するかを構成できました。 新しいパラメーターでは、ユーザーはフルフィルメントを明確な場所セット (最大 5 か所) から行うか、できる限り多くの場所から行うかを指定できます。 [1 つの場所からのみ注文をフルフィルメント] を除くすべてのオプションに対し、DOM では明細行を分割します。注文の処理は明細行ごとに行われるためです。 このルールは、本番ソルバーでのみ有効です。
    ...
    - **最大拒否回数ルール** – このルールにより、組織は拒否のしきい値を定義できます。 このしきい値に達すると、DOM プロセッサは注文または注文明細行に例外のマークを付け、その後の処理から除外します。 パフォーマンスを維持するために、DOM ではすべての拒否の履歴は確認しません。 
    ...
    - **最大注文数ルール** – このルールでは、場所または場所のグループで処理できる注文の最大数を定義できます。 最適化プロセス中に、システムではこれらの場所からまだ出荷されていない注文を確認します。 このチェックはプロファイルをまたがって行われます。 したがって、同じ場所に対して重複する注文の最大数がプロファイル間で定義されている場合は、すべてのプロファイルで定義されている注文の最大数が考慮されます。 

    以下に、共通の属性をいくつか示します。ここまでに記載したすべての種類のルールで、これらの属性を定義できます。

    - **開始日** と **終了日** – これらのフィールドを使用して、各ルールの有効日を設定できます。
...
10. フルフィルメント プロファイルは、ルール、法人、販売注文元、配送モードのコレクションをグループ化するために使用されます。 1 つのフルフィルメント プロファイルに対して 1 回の DOM が実行されます。 このようにして、一連の法人に対する、特定の販売注文元および配送モードが設定されている注文についてのルールのセットを定義し、実行することができます。 そのため、販売注文元または配送モードが異なる別のセットについて、別のルールのセットを実行する必要がある場合は、それに応じてフルフィルメント プロファイルを定義することができます。 フルフィルメント プロファイルを設定するには、次の手順を実行します。  
    ...
    5. 販売注文元が未定義の注文なども、すべての販売注文元を持つ注文に対して、DOM プロファイルを実行するには、**販売元が指定されていない注文を処理する** オプションを **はい** に設定します。 一部の販売注文元についてのみプロファイルを実行するには、後で説明するように、**販売元** ページでその販売注文元を定義することができます。
        ...
        > [!NOTE]
        > Commerce バージョン 10.0.12 以上のリリースでは、**フルフィルメント グループをフルフィルメント プロファイルに割り当てる機能** が **機能管理** ワークスペースで有効になっている必要があり ます。 この機能を使用すると、フルフィルメント プロファイルを使用して最適化を実行するときに DOM が考慮する必要がある倉庫の一覧を指定できます。 この倉庫の一覧が指定されていない場合、DOM はプロファイルに定義されている法人のすべての倉庫を参照します。
        >
    ...
    9. **保存** を選択します。
    10. 操作ウィンドウの **設定** タブで、**配送モード** を選択します。
    ...
    15. **配送モード** ページを閉じます。
    16. 操作ウィンドウの **設定** タブで、**販売注文元** を選択します。
...
ルール、在庫の制約、最適化の適用後、DOM は、顧客の配送先住所に最も近い場所を選択します。 DOM では、**配送** タイプの住所を緯度と経度の値に変換します。 次に、販売注文の配送先住所を緯度と経度の値に変換し、将来の使用のために、その住所の緯度と経度の値を更新します。 DOM は Bing 地図に依存して、市区町村、番地、郵便番号の情報に基づいて緯度と経度の値を判断します。

DOM では Bing 地図 API を使用して、設定に応じて空路または陸路の距離を計算します。 次に、この情報を使用して配送費用を判断します。 最適化モデルでは、1 つの場所からの完全な注文のフルフィルメントを優先させます。 注文の一部が同じ市区町村または郵便番号の地域で利用可能な場合でも、このモデルは出荷数を減らすように最適化されています。 

DOM は、倉庫 V2 エンティティで手持在庫を表示することにより、利用可能な在庫を検索します。 各バッチの実行時、DOM ではプロファイルに定義されているタスクの **DOM プロセッサ** パラメーター値に応じて、注文をバッチに分割します。 このパラメータの既定値は **2000** です。 たとえば、10,000 の注文明細行が 1 回の実行で最適化され、**DOM プロセッサ** パラメーターの既定値が **2000** に設定されている場合は、同時に処理される 5 つのバッチが作成されます。 その後、フルフィルメント計画がオプティマイザから取得され、その明細行に適用されます。 注文明細行を 2 つの場所に分割する必要がある場合、DOM では価格と税金がこれらの明細行間で適切に分散されるようにします。
...
- マイクロソフトでは、DOM での高度な倉庫管理機能の使用については、テストを行っていません。 したがって、お客様およびパートナー様には、関連のある高度な倉庫管理の機能およびプロセスとの互換性が DOM にあるかどうかを、注意して判断していただく必要があります。 高度な倉庫管理では、使用可能な在庫を正確に示さない、在庫状態などの構成可能な分析コードを有効にします。 DOM は、高度な倉庫管理を使用する実装に対し、使用可能な在庫を設定するための拡張手段を提供します。 これは、在庫状態や他の分析コードのカスタマイズ値と一緒に使用できます。

    DOM の機能拡張には制限があります。最適化は、最適化とその制約を考慮する、構築済みの MIP モデルで行われるためです。 在庫および後処理の最適化のために、いくつかの拡張ポイントが既に用意されています。 DOM プロファイルは、販売元と配送モードによって異なる場合があります。 販売注文元は注文の消化時に設定でき、これらの値に基づいて、異なる最適化戦略を使用できます。 DOM では、DOM プロセッサ タスクを入力として受け取り、プロファイルをパラメーターとして渡すことのできるカスタム バッチ ジョブの作成もサポートしています。 したがって、さまざまなビジネス シナリオをサポートするために、1 つの最適化を相次いで実行できます。

- DOM は、クラウド バージョンの Commerce でのみ利用できます。 オンプレミス展開ではサポートされません。
