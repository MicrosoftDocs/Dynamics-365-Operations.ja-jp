...
以前に説明した 2 つの APIs を使用する前に、コマース本社が**製品の可用性**ジョブを計算し、正しいテーブルにデータを入力した在庫値のスナップショットを確認するには、コマース本社でパラメータ変更を行う必要があります。

パラメーターを設定するには、これらの手順に従います。

1. **Retail とコマース \> 本社の設定 \> パラメーター \> コマース共有パラメーター**の順に移動します。
1. **在庫**タブの**製品の可用性ジョブ**セクションで、**製品の可用性ジョブの最適化されたプロセスを使用**を選択します。 この設定により、最適な機能セットを使用して、チャネルの利用可能な在庫の計算にコマース サーバーを使用できるようになります。
...
在庫状態が **GetEstimatedAvailabilty** または **ProductWarehouseInventoryAvailabilities** API から依頼されると、計算が実行され、製品在庫の可能な限り最良の見積を取得しようとします。 この計算は、チャンネル データベースに含まれている E コマースの顧客注文を参照しますが、1130 ジョブによって提供されたスナップショット データには含まれていませんでした。 このロジックは、コマース本社から最後に処理された在庫トランザクションを追跡して、それをチャネル データベース内のトランザクションと比較することによって実行されます。 これは、チャネル側の計算ロジックのベースラインを提供します。これにより、E コマースのチャネル データベースの顧客注文販売トランザクションで発生した追加の在庫移動を、API によって予測される在庫値として考慮することができます。
...
チャネル側計算のロジックを使用し、POS アプリケーションからの在庫検索リアルタイム サービス コールを無効にするには、最初に 2 つのパラメータを変更する必要があります。 次に、配送スケジュールのプロセスを使用して、チャネルに変更を同期する必要があります。

最初のパラメーターを設定するには、これらの手順に従います。

1. **Retail とコマース \> 本社の設定 \> パラメーター \> コマース共有パラメーター**の順に移動します。
1. **在庫**タブの**製品の可用性ジョブ**セクションで、**製品の可用性ジョブの最適化されたプロセスを使用**を選択します。 この設定により、最適な機能セットを使用して、チャネルの利用可能な在庫の計算にコマース サーバーを使用できるようになります。

次のパラメーターを設定するには、これらの手順に従います。
