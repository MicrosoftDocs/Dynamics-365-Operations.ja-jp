...
description: この記事では、会社が Microsoft Dynamics 365 Commerce を使用して、オンラインおよび店舗チャネルの製品の見積済手持在庫を表示する方法について説明します。
author: hhainesms
ms.date: 09/01/2021
ms.topic: article
ms.prod: ''
...
ms.locfileid: "9279684"
---
# <a name="calculate-inventory-availability-for-retail-channels"></a>小売チャンネルの引当可能在庫数量の計算

[!include [banner](../includes/banner.md)]

この記事では、会社が Microsoft Dynamics 365 Commerce を使用して、オンラインおよび店舗チャネルの製品の見積済手持在庫を表示する方法について説明します。
...
チャンネル側の在庫計算を使用するには、**最適化された製品の使用可能な計算** 機能を有効にする必用があります。

Commerce 環境がリリース **10.0.8 から 10.0.11** の場合は、次の手順に従います。

1. Commerce 本社で、**Retail と Commerce** \> **Commerce 共有パラメーター** へ移動します。
1. **在庫** タブの **製品の可用性ジョブ** フィールドで、**製品の可用性ジョブの最適化されたプロセスを使用** を選択します。

Commerce 環境がリリース **10.0.12 またはそれ以降** の場合は、次の手順に従います。

1. Commerce 本社で、**ワークスペース \> 機能管理** へ移動し、**最適化された製品の利用可能な計算** 機能を有効にします。
1. オンラインおよび店舗のチャンネルで同じフルフィルメント倉庫を使用する場合は、**拡張 e-Commerce チャンネル側の在庫計算ロジック** 機能も有効にする必用があります。 つまり、チャンネル側の計算ロジックでは、店舗のチャンネルで作成される未転記トランザクションが考慮されます。 (トランザクションには、現金および持ち帰りトランザクション、顧客注文、および返品トランザクションがあります。)
1. **1070** (**チャネル コンフィギュレーション**) ジョブを実行します。

Commerce 環境が Commerce バージョン 10.0.8 よりも前のリリースからアップグレードされていた場合は、**最適化された製品の使用可能計算** 機能を有効にした後、機能の **コマース スケジューラを初期化** も実行し有効にする必用があります。 初期化を実行するには、**Retail と Commerce** \> **本社の手順** \> **Commerce スケジューラ** へ移動します。
