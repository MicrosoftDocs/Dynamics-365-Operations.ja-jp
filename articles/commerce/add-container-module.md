...
コンテナー モジュールは、その中の他のモジュールをホストするモジュールです。 Dynamics 365 Commerce で使用される最も一般的なコンテナーです。 コンテナー モジュールの主な目的は、それに対して設定されているプロパティを通じて、内部に含まれるモジュールのレイアウトを定義することです。 たとえば、それらのモジュールは 2 列、3 列、4 列、または 6 列のレイアウトで左右に並んで表示できます。 また、コンテナーの幅に限定することも、画面全体に表示することもできます。 ヘッダーは、各コンテナー モジュールに追加することもできます。

コンテナー モジュールには 3 つの標準タイプがあります: コンテナー、2 スロットのコンテナー、3 スロットのコンテナー。 任意のモジュール タイプのモジュールを、これらのコンテナー内に配置できます。 またカルーセル、コンテンツ リッチ ブロック、コンテンツ配置、カート、チェックアウト、購入ボックス、ヘッダー、およびフッターなど特別なタイプのコンテナー モジュールもあります。 これらのコンテナーには特定の目的があります。サポートされる特定のタイプのモジュールのみを中に配置できます。

コンテナーの幅に限定できるよう、モジュールをコンテナー内に配置することをお勧めします。
...
1. コンテナ― モジュールで、機能モジュールを追加します。
1. テンプレートをチェックインし、公開します。
1. 作成したコンテナ― テンプレートを使用して、**コンテナ― ページ**という名前のページを作成します。
1. 新しいページの**メイン** スロットで、コンテナ― モジュールを追加します。
1. コンテナー モジュールのプロパティ ウィンドウで、**列数**プロパティを **1** に設定し、**幅**プロパティを**コンテナーに適合**に設定します。
1. コンテナ― モジュールで、機能モジュールを追加します。
1. 機能モジュールのプロパティ ウィンドウで、ヘッダーをコンフィギュレーションします。
1. ページを保存してプレビューします。 コンテナー モジュールの幅内に適合する 1 つの機能モジュールが表示されます。
1. コンテナー モジュールのプロパティ ウィンドウで**列数**プロパティの値を **3** に変更します。
1. コンテナ― モジュールで、機能モジュールをもう 2 つ追加します。
1. ページを保存してプレビューします。 並んで表示される 3 つの機能モジュールが確認できるはずです。
1. 目的のレイアウトが達成されたら、ページをチェックインして発行します。
