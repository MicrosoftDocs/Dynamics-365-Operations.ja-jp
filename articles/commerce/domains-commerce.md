...
- **オプション 2** - DNS プロバイダーが ALIAS レコードをサポートしている場合、前もって選択したエンドポイントにドメインをポイントできます。 これにより、フロント パブリック エンドポイントの IP 変更が確実に反映されます。
  
- **オプション 3** - DNS プロバイダが ALIAS レコードをサポートしない場合は、何も指定したドメインをホストするために、何か年に 1 つの DNS レコードまたはフロント ホテル インスタンスを自分で設定する必要があります。
