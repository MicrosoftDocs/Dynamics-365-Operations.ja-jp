...
1. **Commerce パラメーター \> 構成パラメーター** に移動、**ProductSearch.UseAzureSearch** のエントリがあり、値が **true** に設定されていることを確認します。 このエントリがない場合は、追加して、値を **true** に設定してから、eコマース Web サイトに関連付けられた Commerce Scale Unit で **チャネル データベース \> 完全データ同期** を選択します。
...
1. Commerce Scale Unit にチャネルを追加するには、**小売とコマース \> 本社の設定 \> コマース スケジューラ \>チャネル データベース** に移動し、左ペインで Commerce Scale Unit を選択します。 **小売チャネル** FastTab で、**AW オンライン ストア**、**AW Business オンライン ストア**、および **Fabrikam 拡張オンライン ストア** のチャンネルを追加します。 必要に応じて、POS を使用する場合 (たとえば、**シアトル**、**サンフランシスコ**、**サンホセ**) は、小売店舗を追加することもできます。
...
1. **AdventureWorks** サイト (**AW オンライン ストア** チャンネルにマップ) と **AdventureWorks Business** サイト (**AW Business オンライン ストア** チャンネルにマップ) に対して、手順 2-7 を繰り返します。 Fabrikam サイトの **パス** フィールドが空の場合は、2 つの AdventureWorks サイトのパス (例えば、"aw" や "awbusiness") を追加する必要があります。
