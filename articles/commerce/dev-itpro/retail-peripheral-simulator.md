...
## <a name="install-the-peripheral-simulator"></a><span data-ttu-id="9d91a-108">周辺機器シミュレーターのインストール</span><span class="sxs-lookup"><span data-stu-id="9d91a-108">Install the peripheral simulator</span></span>
...
- <span data-ttu-id="9d91a-124">**プリンター** – 仮想周辺機器シミュレーターは POS プリンターに対して設定されている入庫を表示できます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-124">**Printer** – The virtual peripheral simulator can show receipts that are configured for a POS printer.</span></span>
...
### <a name="key-scenarios--virtual-peripheral-simulator"></a><span data-ttu-id="9d91a-139">重要なシナリオ – 仮想周辺機器シミュレーター</span><span class="sxs-lookup"><span data-stu-id="9d91a-139">Key scenarios – Virtual peripheral simulator</span></span>
...
### <a name="set-up-the-virtual-peripheral-simulator"></a><span data-ttu-id="9d91a-154">仮想周辺機器シミュレーターの設定</span><span class="sxs-lookup"><span data-stu-id="9d91a-154">Set up the virtual peripheral simulator</span></span>
...
### <a name="line-display-capabilities"></a><span data-ttu-id="9d91a-213">ライン ディスプレイ機能</span><span class="sxs-lookup"><span data-stu-id="9d91a-213">Line display capabilities</span></span>
...
#### <a name="testing-gift-and-payment-card-swipes"></a><span data-ttu-id="9d91a-245">ギフト カードや支払カードの読み取りのテスト</span><span class="sxs-lookup"><span data-stu-id="9d91a-245">Testing gift and payment card swipes</span></span>
...
1. <span data-ttu-id="9d91a-261">**Retail と Commerce** &gt; **チャネル設定** &gt; **POS 設定** &gt; **POS プロファイル** &gt; **ハードウェア プロファイル** の順に移動します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-261">Go to **Retail and Commerce** &gt; **Channel setup** &gt; **POS setup** &gt; **POS profiles** &gt; **Hardware profiles**.</span></span>
...
##### <a name="synchronize-changes-to-the-channel-database"></a><span data-ttu-id="9d91a-278">チャンネル データベースへの変更の同期</span><span class="sxs-lookup"><span data-stu-id="9d91a-278">Synchronize changes to the channel database</span></span>
...
##### <a name="synchronize-changes-to-the-channel-database"></a><span data-ttu-id="9d91a-297">チャンネル データベースへの変更の同期</span><span class="sxs-lookup"><span data-stu-id="9d91a-297">Synchronize changes to the channel database</span></span>
...
#### <a name="configure-an-opos-device"></a><span data-ttu-id="9d91a-361">OPOS デバイスのコンフィギュレーション</span><span class="sxs-lookup"><span data-stu-id="9d91a-361">Configure an OPOS device</span></span>
...
- <span data-ttu-id="9d91a-413">**バイナリ変換** - ライン ディスプレイによって、テキストをバイナリ形式に変換する必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-413">**Binary conversion** – Some line displays require that text be converted into binary format.</span></span> <span data-ttu-id="9d91a-414">この変換が必要かどうかを確認するのには、デバイスのマニュアルを参照してください。</span><span class="sxs-lookup"><span data-stu-id="9d91a-414">To determine whether this conversion is required, see the device’s documentation.</span></span>
- <span data-ttu-id="9d91a-415">**文字セット** - デバイスに送信される文字のコード ページ。</span><span class="sxs-lookup"><span data-stu-id="9d91a-415">**Character set** – The code page for the characters that are sent to the device.</span></span> <span data-ttu-id="9d91a-416">特定のコード ページの識別子については、<https://msdn.microsoft.com/library/windows/desktop/dd317756(v=vs.85).aspx> を参照してください。</span><span class="sxs-lookup"><span data-stu-id="9d91a-416">For the identifiers of specific code pages, see <https://msdn.microsoft.com/library/windows/desktop/dd317756(v=vs.85).aspx>.</span></span>

#### <a name="msr"></a><span data-ttu-id="9d91a-417">MSR</span><span class="sxs-lookup"><span data-stu-id="9d91a-417">MSR</span></span>

- <span data-ttu-id="9d91a-418">**MSR を開いて要求** – MSR からデータを受信するように POS シミュレーターを準備します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-418">**Open and claim MSR** – Prepare the POS simulator to receive data from the MSR.</span></span>
- <span data-ttu-id="9d91a-419">**MSR をリリースして閉じる** – テストが完了したときに、MSR デバイスを閉じます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-419">**Release and close MSR** – Close the MSR device when testing is completed.</span></span>
- <span data-ttu-id="9d91a-420">**カード情報** - MSR デバイスでスキャンしたカードからのデータ。</span><span class="sxs-lookup"><span data-stu-id="9d91a-420">**Card info** – Data from the card that was scanned on the MSR device.</span></span>

    > [!IMPORTANT]
    > <span data-ttu-id="9d91a-421">実際のクレジット カードは、デバイス テストに使用しないでください。</span><span class="sxs-lookup"><span data-stu-id="9d91a-421">Actual credit cards should never be used for device testing.</span></span> <span data-ttu-id="9d91a-422">期限切れのクレジットカードであっても、テストには使用しないでください。</span><span class="sxs-lookup"><span data-stu-id="9d91a-422">Even expired credit cards should not be used for testing.</span></span>

#### <a name="pin-pad"></a><span data-ttu-id="9d91a-423">PIN パッド</span><span class="sxs-lookup"><span data-stu-id="9d91a-423">PIN pad</span></span>

<span data-ttu-id="9d91a-424">**両方のタブに表示される設定**</span><span class="sxs-lookup"><span data-stu-id="9d91a-424">**Settings that appear on both tabs**</span></span>

- <span data-ttu-id="9d91a-425">**ロック** – PIN パッド デバイスをロックして、POS シミュレーターでのみ使用できるようにします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-425">**Lock** – Lock the PIN pad device so that it can be used only with the POS simulator.</span></span>
- <span data-ttu-id="9d91a-426">**エントリを取得** – PIN データを受信する POS シミュレーションを有効にします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-426">**Get entry** – Enable the POS simulator to receive PIN data.</span></span>
- <span data-ttu-id="9d91a-427">**操作のキャンセル** - PIN パッドに送信された要求をキャンセルします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-427">**Cancel operation** – Cancel the request that was sent to the PIN pad.</span></span>
- <span data-ttu-id="9d91a-428">**リリース** – PIN パッド デバイスをリリースします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-428">**Release** – Release the PIN pad device.</span></span>

<span data-ttu-id="9d91a-429">**設定タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-429">**Settings tab**</span></span>

- <span data-ttu-id="9d91a-430">**金額** - 顧客の受入のために PIN パッド デバイスに送信する金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-430">**Amount** – The amount to send to the PIN pad device for customer acceptance.</span></span>
- <span data-ttu-id="9d91a-431">**勘定番号** - 必要な場合は、感情番号を指定します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-431">**Account number** – Specify the account number, if it's required.</span></span>
- <span data-ttu-id="9d91a-432">**暗号化された PIN** - デバイスから受信した暗号化された PIN。</span><span class="sxs-lookup"><span data-stu-id="9d91a-432">**Encrypted PIN** – The encrypted PIN that is received from the device.</span></span>
- <span data-ttu-id="9d91a-433">**追加のセキュリティ データ** - 暗号化された PIN に使用される暗号を指定します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-433">**Additional security data** – Specify the cryptography that is used for the encrypted PIN.</span></span>

<span data-ttu-id="9d91a-434">**詳細タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-434">**Advanced tab**</span></span>

- <span data-ttu-id="9d91a-435">**タイムアウト** – デバイスからの応答に待機する秒数を指定します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-435">**Timeout** – Specify the number of seconds to wait for a response from the device.</span></span>
- <span data-ttu-id="9d91a-436">**排他** - 有効にするには、PIN パッド デバイスが要求されている必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-436">**Exclusive** – Require that the PIN pad device be claimed before it can be enabled.</span></span>
- <span data-ttu-id="9d91a-437">**上書き** – デバイスが応答していない場合に、デバイスに送信された前のコマンドを上書きします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-437">**Override** – Override previous commands that were sent to device when the device isn't responding.</span></span>

#### <a name="scale"></a><span data-ttu-id="9d91a-438">スケール</span><span class="sxs-lookup"><span data-stu-id="9d91a-438">Scale</span></span>

- <span data-ttu-id="9d91a-439">**タイムアウト** – タイムアウトの間隔。</span><span class="sxs-lookup"><span data-stu-id="9d91a-439">**Timeout** – The time-out interval.</span></span> <span data-ttu-id="9d91a-440">重量が読み取られる前に、スケールに製品を配置する必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-440">Product should be put on the scale before the weight is read.</span></span> <span data-ttu-id="9d91a-441">スケールが指定した期間内に応答されない場合、要求がキャンセルされます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-441">If the scale doesn't respond within the specified interval, the request is canceled.</span></span>

#### <a name="signature-capture"></a><span data-ttu-id="9d91a-442">署名キャプチャ</span><span class="sxs-lookup"><span data-stu-id="9d91a-442">Signature capture</span></span>

<span data-ttu-id="9d91a-443">**設定タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-443">**Settings tab**</span></span>

- <span data-ttu-id="9d91a-444">**フォーム名** – 署名要求の送信時に、一部の署名キャプチャ デバイスではフォーム名が必要です。</span><span class="sxs-lookup"><span data-stu-id="9d91a-444">**Form name** – Some signature capture devices require a form name when the signature request is sent.</span></span>
- <span data-ttu-id="9d91a-445">**(16 進数での) 署名** – デバイスから受信した署名のデータの 16 進数の値。</span><span class="sxs-lookup"><span data-stu-id="9d91a-445">**Signature (in HEX)** – The hex value for the signature data that is received from the device.</span></span>
- <span data-ttu-id="9d91a-446">**表示する署名** – デバイスから受信した署名のイメージ。</span><span class="sxs-lookup"><span data-stu-id="9d91a-446">**Rendered signature** – The image of the signature that is received from the device.</span></span>

<span data-ttu-id="9d91a-447">**詳細タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-447">**Advanced tab**</span></span>

- <span data-ttu-id="9d91a-448">**タイムアウト** – タイムアウトの間隔。</span><span class="sxs-lookup"><span data-stu-id="9d91a-448">**Timeout** – The time-out interval.</span></span> <span data-ttu-id="9d91a-449">署名キャプチャ デバイスが指定した期間内に応答されない場合、要求がキャンセルされます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-449">If the signature capture device doesn't respond within the specified interval, the request is canceled.</span></span>
- <span data-ttu-id="9d91a-450">**排他** - 有効にするには、PIN パッド デバイスが要求されている必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-450">**Exclusive** – Require that the PIN pad device be claimed before it can be enabled.</span></span>
- <span data-ttu-id="9d91a-451">**上書き** – デバイスが応答していない場合に、デバイスに送信された前のコマンドを上書きします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-451">**Override** – Override previous commands that were sent to device when the device isn't responding.</span></span>
- <span data-ttu-id="9d91a-452">**ロック** – デバイスをロックして、POS シミュレーターで使用するようにデバイスを要求します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-452">**Lock** – Lock the device and claim it for use by the POS simulator.</span></span>
- <span data-ttu-id="9d91a-453">**エントリを取得** – デバイスから署名を要求します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-453">**Get entry** – Request the signature from the device.</span></span>
- <span data-ttu-id="9d91a-454">**操作のキャンセル** - 署名要求をキャンセルします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-454">**Cancel operation** – Cancel the signature request.</span></span>
- <span data-ttu-id="9d91a-455">**リリース** – 他のプロセスで使用できるようにデバイスをリリースします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-455">**Release** – Release the device so that it can be used by other processes.</span></span>

#### <a name="bar-code-scanner"></a><span data-ttu-id="9d91a-456">バーコード スキャナー</span><span class="sxs-lookup"><span data-stu-id="9d91a-456">Bar code scanner</span></span>

- <span data-ttu-id="9d91a-457">**スキャナーを開いて要求** – スキャナーを開いて要求します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-457">**Open and claim scanner** – Open and claim the scanner.</span></span> <span data-ttu-id="9d91a-458">POS シミュレーターは、この操作が正常に完了した後にスキャン イベントを受け取ることができます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-458">The POS simulator can receive scan events after this operation is completed successfully.</span></span>
- <span data-ttu-id="9d91a-459">**スキャナーをリリースして閉じる** – スキャナーを他のプロセスで使用可能にします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-459">**Release and close the scanner** – Make the scanner available for other processes.</span></span>
- <span data-ttu-id="9d91a-460">**スキャンした情報** – バーコード スキャナーから受信したデータ。</span><span class="sxs-lookup"><span data-stu-id="9d91a-460">**Scanned information** – The data that was received from the bar code scanner.</span></span>

#### <a name="payment-terminal"></a><span data-ttu-id="9d91a-461">支払ターミナル</span><span class="sxs-lookup"><span data-stu-id="9d91a-461">Payment terminal</span></span>

<span data-ttu-id="9d91a-462">**それぞれのタブに表示される設定**</span><span class="sxs-lookup"><span data-stu-id="9d91a-462">**Settings that appear on every tab**</span></span>

- <span data-ttu-id="9d91a-463">**操作を選択** – デバイス上で実行する特定の操作を選択します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-463">**Select an operation** – Select a specific operation to perform on the device.</span></span> <span data-ttu-id="9d91a-464">オプションには、**すべて**、**カードで支払う**、**カードへの返金**、および **支払の無効化** があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-464">Options include **All**, **Pay by card**, **Refund by card**, and **Void payment**.</span></span>
- <span data-ttu-id="9d91a-465">**ロックと要求** – POS シミュレーターで使えるようにデバイスを準備します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-465">**Lock and claim** – Prepare the device so that it can be used by the POS simulator.</span></span>
- <span data-ttu-id="9d91a-466">**明細行品目の更新** – 指定した明細行品目の詳細をデバイスに送信してください。</span><span class="sxs-lookup"><span data-stu-id="9d91a-466">**Update line items** – Send specified line item details to the device.</span></span>
- <span data-ttu-id="9d91a-467">**支払の承認** - 支払の承認を要求する支払コネクタに指示します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-467">**Authorize payment** – Instruct the payment connector to request payment authorization.</span></span>
- <span data-ttu-id="9d91a-468">**支払のキャプチャ** - 以前の承認をキャプチャするよう支払コネクタに指示します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-468">**Capture payment** – Instruct the payment connector to capture the previous authorization.</span></span>
- <span data-ttu-id="9d91a-469">**リリース** – デバイスをリリースします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-469">**Release** – Release the device.</span></span>

<span data-ttu-id="9d91a-470">**設定タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-470">**Settings tab**</span></span>

<span data-ttu-id="9d91a-471">**設定** タブには、支払の承認が要求されたときにデバイスに送信されるプロパティが含まれています。</span><span class="sxs-lookup"><span data-stu-id="9d91a-471">The **Settings** tab contains properties that are sent to the device when a payment authorization is requested.</span></span>

- <span data-ttu-id="9d91a-472">**請求書番号** – POS で生成される請求書番号を指定します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-472">**Invoice number** – Specify the invoice number that is generated at the POS.</span></span>
- <span data-ttu-id="9d91a-473">**テスト モード** – テスト トランザクションが実行されていることを示す値。</span><span class="sxs-lookup"><span data-stu-id="9d91a-473">**Test mode** – A value that indicates that a test transaction is being performed.</span></span>
- <span data-ttu-id="9d91a-474">**支払コネクタ** – 使用する支払コネクタの名前。</span><span class="sxs-lookup"><span data-stu-id="9d91a-474">**Payment connector** – The name of the payment connector to use.</span></span>
- <span data-ttu-id="9d91a-475">**借方キャッシュ バック限度** - 借方トランザクションが実行されるときにデバイス上で要求できるキャッシュ バックの最大金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-475">**Debit cashback limit** – The maximum amount of cash back that can be requested on the device when a debit transaction is performed.</span></span>
- <span data-ttu-id="9d91a-476">**ロケール** – 使用されるロケール。</span><span class="sxs-lookup"><span data-stu-id="9d91a-476">**Locale** – The locale that is used.</span></span> <span data-ttu-id="9d91a-477">ロケールは、POS シミュレーター用の構成ファイルで定義されます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-477">Locales are defined in the configuration file for the POS simulator.</span></span>
- <span data-ttu-id="9d91a-478">**最大許容金額** – 指定されたトランザクションで処理できる最大金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-478">**Maximum amount allowed** – The maximum amount that can be processed for a given transaction.</span></span>
- <span data-ttu-id="9d91a-479">**最小許容金額** – 指定されたトランザクションで処理できる最小金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-479">**Minimum amount allowed** – The minimum amount that can be processed for a given transaction.</span></span>
- <span data-ttu-id="9d91a-480">**署名キャプチャの最小量** – 顧客が署名を提供するように促される最低額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-480">**Signature capture minimum amount** – The lowest amount that the customer is prompted to provide a signature for.</span></span>
- <span data-ttu-id="9d91a-481">**ターミナル ID** – プロセッサが提供したトランザクション プロパティに含まれるターミナル ID。</span><span class="sxs-lookup"><span data-stu-id="9d91a-481">**Terminal ID** – The terminal ID that is included in the transaction properties that the processor provided.</span></span>

<span data-ttu-id="9d91a-482">**行タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-482">**Lines tab**</span></span>

<span data-ttu-id="9d91a-483">**明細行** タブには、ライン ディスプレイとして使用されている場合に、デバイスに送信できるデータが含まれています。</span><span class="sxs-lookup"><span data-stu-id="9d91a-483">The **Lines** tab contains data that can be sent to the device when it's used as a line display.</span></span>

- <span data-ttu-id="9d91a-484">**説明** - デバイスに表示されるトランザクション明細行の製品説明。</span><span class="sxs-lookup"><span data-stu-id="9d91a-484">**Description** – The product description for the transaction line to show on the device.</span></span>
- <span data-ttu-id="9d91a-485">**割引** - トランザクション明細行に適用される割引金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-485">**Discount** – The discount amount that is applied to the transaction line.</span></span>
- <span data-ttu-id="9d91a-486">**拡張された税込明細行** - 製品明細行の金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-486">**Extended line with tax** – The product line amount.</span></span> <span data-ttu-id="9d91a-487">この金額には税金が含まれています。</span><span class="sxs-lookup"><span data-stu-id="9d91a-487">This amount includes tax.</span></span>
- <span data-ttu-id="9d91a-488">**行項目 ID** – トランザクション明細行の製品 ID。</span><span class="sxs-lookup"><span data-stu-id="9d91a-488">**Line item ID** – The product ID for the transaction line.</span></span>
- <span data-ttu-id="9d91a-489">**数量** – トランザクション明細行の数量。</span><span class="sxs-lookup"><span data-stu-id="9d91a-489">**Quantity** – The quantity for the transaction line.</span></span>
- <span data-ttu-id="9d91a-490">**最小在庫管理単位** – デバイスに表示する測定単位。</span><span class="sxs-lookup"><span data-stu-id="9d91a-490">**Stock keeping unit** – The unit of measure to show on the device.</span></span>
- <span data-ttu-id="9d91a-491">**単価** – トランザクション明細行の販売価格。</span><span class="sxs-lookup"><span data-stu-id="9d91a-491">**Unit price** – The selling price for the transaction line.</span></span>
- <span data-ttu-id="9d91a-492">**統一商品コード** – トランザクション明細行のためにスキャンされたバーコード。</span><span class="sxs-lookup"><span data-stu-id="9d91a-492">**Universal product code** – The bar code that was scanned for the transaction line.</span></span>
- <span data-ttu-id="9d91a-493">**無効** – トランザクションからトランザクション行が削除されたことを示す値。</span><span class="sxs-lookup"><span data-stu-id="9d91a-493">**Is voided** – A value that indicates that the transaction line has been voided from the transaction.</span></span>

<span data-ttu-id="9d91a-494">**詳細タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-494">**Advanced tab**</span></span>

<span data-ttu-id="9d91a-495">**詳細** タブには、デバイスに表示できる小計の情報が含まれます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-495">The **Advanced** tab contains subtotal information that can be shown on the device.</span></span>

- <span data-ttu-id="9d91a-496">**割引金額** - トランザクション上の割引の合計金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-496">**Discount amount** – The total amount for discounts on the transaction.</span></span>
- <span data-ttu-id="9d91a-497">**小計金額** – トランザクションの小計。</span><span class="sxs-lookup"><span data-stu-id="9d91a-497">**Subtotal amount** – The subtotal for the transaction.</span></span> <span data-ttu-id="9d91a-498">この金額には、税金が含まれていません。</span><span class="sxs-lookup"><span data-stu-id="9d91a-498">This amount doesn't include tax.</span></span>
- <span data-ttu-id="9d91a-499">**税額** – トランザクションの税額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-499">**Tax amount** – The tax amount for the transaction.</span></span>
- <span data-ttu-id="9d91a-500">**合計金額** – 予定されている合計金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-500">**Total amount** – The total amount that is due.</span></span> <span data-ttu-id="9d91a-501">この金額には税金と割引が含まれます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-501">This amount includes tax and discounts.</span></span>
- <span data-ttu-id="9d91a-502">**通貨** - トランザクションに使用される通貨。</span><span class="sxs-lookup"><span data-stu-id="9d91a-502">**Currency** – The currency that is used for the transaction.</span></span>

<span data-ttu-id="9d91a-503">**支払情報タブ**</span><span class="sxs-lookup"><span data-stu-id="9d91a-503">**Payment info tab**</span></span>

<span data-ttu-id="9d91a-504">**支払情報** タブには、承認が処理された後に、支払コネクタから受信したデータが含まれています。</span><span class="sxs-lookup"><span data-stu-id="9d91a-504">The **Payment info** tab contains data that is received from the payment connector after an authorization has been processed.</span></span>

- <span data-ttu-id="9d91a-505">**承認** – 承認要求が承認されているかどうかを示す値。</span><span class="sxs-lookup"><span data-stu-id="9d91a-505">**Is approved** – A value that indicates whether the authorization request is approved.</span></span>
- <span data-ttu-id="9d91a-506">**マスクされたカード番号** - デバイスによって提供されるマスクされたカード番号です。</span><span class="sxs-lookup"><span data-stu-id="9d91a-506">**Card number masked** – The masked card number that is provided by the device.</span></span> <span data-ttu-id="9d91a-507">通常、カード番号の最後の 4 桁のみが表示されます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-507">Typically, only the last four digits of the card number are shown.</span></span>
- <span data-ttu-id="9d91a-508">**カード タイプ** - トランザクションに使用されるカードの問題です。</span><span class="sxs-lookup"><span data-stu-id="9d91a-508">**Card type** – The issue for the card that is used in the transaction.</span></span>
- <span data-ttu-id="9d91a-509">**承認済金額** - カードの支払に対して承認されている金額。</span><span class="sxs-lookup"><span data-stu-id="9d91a-509">**Approved amount** – The amount that is authorized for the card payment.</span></span>
- <span data-ttu-id="9d91a-510">**支払 SDK データ** – 認証コードや支払プロセッサが提供するその他のデータなど、支払ソフトウェア開発キット (SDK) が提供する追加データ。</span><span class="sxs-lookup"><span data-stu-id="9d91a-510">**Payment SDK data** – Additional data that the payment software development kit (SDK) can provide, such as the authorization code and other data that the payment processor provides.</span></span>
- <span data-ttu-id="9d91a-511">**署名データ** – デバイスによって提供される 16 進署名データ。</span><span class="sxs-lookup"><span data-stu-id="9d91a-511">**Signature data** – Hexadecimal signature data that is provided by the device.</span></span>
- <span data-ttu-id="9d91a-512">**支払エラー** – 認証要求中に発生したエラー。</span><span class="sxs-lookup"><span data-stu-id="9d91a-512">**Payment errors** – Any errors that occurred during the authorization request.</span></span>

## <a name="instructions-for-device-manufacturers"></a><span data-ttu-id="9d91a-513">デバイス メーカーに対する指示</span><span class="sxs-lookup"><span data-stu-id="9d91a-513">Instructions for device manufacturers</span></span>

<span data-ttu-id="9d91a-514">コマースとの互換性を一覧表示できるようにデバイスを送信するには、次の手順を実行します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-514">To submit a device so that it can be listed for compatibility with Commerce,  follow these steps.</span></span>

1. <span data-ttu-id="9d91a-515">**設定**タブで、要求されたすべての連絡先情報を指定します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-515">On the **Settings** tab, specify all contact information that is requested.</span></span> <span data-ttu-id="9d91a-516">全フィールドが必須です。</span><span class="sxs-lookup"><span data-stu-id="9d91a-516">All fields are required.</span></span> <span data-ttu-id="9d91a-517">個人の連絡先情報は、Microsoft の社内用です。</span><span class="sxs-lookup"><span data-stu-id="9d91a-517">The personal contact information is for Microsoft internal use.</span></span> <span data-ttu-id="9d91a-518">互換性のあると提出されたデバイスに問題がある場合にのみ使用されます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-518">It will be used only if there are issues with devices that have been submitted as compatible.</span></span>
2. <span data-ttu-id="9d91a-519">各デバイス用に、**デバイス**タブで、デバイスを設定し、すべてのフィールドの値を指定します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-519">For each device, on the **Devices** tab, set up the device, and provide values for all fields.</span></span> <span data-ttu-id="9d91a-520">各順列で、新しいデバイスを指定する必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-520">For each permutation, a new device must be specified.</span></span> <span data-ttu-id="9d91a-521">たとえば、USB およびシリアル用に、指定されたデバイス モードがテストされる場合、2 つの互換性レポートが送信される必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-521">For example, if a given device mode is tested for USB and serial, two compatibility reports must be submitted.</span></span> <span data-ttu-id="9d91a-522">1 つのデバイスから次のデバイスへの差異を示す必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-522">Any variance from one device to the next should be indicated.</span></span> <span data-ttu-id="9d91a-523">次に、互換性のためにデバイスが記載されている場合、そのデバイスを実装するユーザーに非常に限定された情報を提供できます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-523">Then, when devices are listed for compatibility, very specific information can be provided to people who implement the devices.</span></span> <span data-ttu-id="9d91a-524">デバイスの互換性に関する情報が多く提供されるほど、実装の成功率が高くなり、サポートの問題が少なくなります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-524">The more information that can be provided about device compatibility, the higher the success rate for implementations and the fewer support issues will be raised.</span></span>
3. <span data-ttu-id="9d91a-525">デバイスおよび設定に必要な情報を入力した後に、デバイスをテストします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-525">After you've entered the required information for devices and settings, test the device.</span></span>

    1. <span data-ttu-id="9d91a-526">テストしているデバイスのログを消去します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-526">Clear the log for the device that you're testing.</span></span>
    2. <span data-ttu-id="9d91a-527">**自己テスト**をクリックします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-527">Click **Self-test**.</span></span>
    3. <span data-ttu-id="9d91a-528">テストが正常に完了したときは、テストの結果をエクスポートして保存します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-528">When a test has been completed successfully, export the test results, and save them.</span></span> <span data-ttu-id="9d91a-529">ファイルに、テストを容易に識別する名前を付けます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-529">Give the file a name that will make the test easy to identify.</span></span> <span data-ttu-id="9d91a-530">ファイル名として、メーカーの名前とデバイス モデルの省略形を使用することをお勧めします。</span><span class="sxs-lookup"><span data-stu-id="9d91a-530">We suggest that you use an abbreviated form of the manufacturer's name and the device model as the file name.</span></span>

<span data-ttu-id="9d91a-531">次の例は、エクスポートされたファイルに含まれるデータを示しています。</span><span class="sxs-lookup"><span data-stu-id="9d91a-531">The following example shows the data that is included in the exported file.</span></span>
...
<span data-ttu-id="9d91a-532">最終的には、レポート (連絡先の個人情報を除く) に含まれているデータがデバイス互換性 Web サイトに表示されます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-532">Eventually, the data that is included in the report (except personal contact information) will be listed on a device compatibility website.</span></span> <span data-ttu-id="9d91a-533">顧客環境の管理に使用される設計および配置のツールでも表示されます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-533">It will also be listed in design and deployment tools that are used to manage customer environments.</span></span>

<span data-ttu-id="9d91a-534">正常なログを、drpc@microsoft.com に送信する必要があります。</span><span class="sxs-lookup"><span data-stu-id="9d91a-534">Successful logs should be sent to drpc@microsoft.com.</span></span> <span data-ttu-id="9d91a-535">件名行に、メーカーの名前とデバイス モデルを含めます。</span><span class="sxs-lookup"><span data-stu-id="9d91a-535">Include the manufacturer’s name and the device model in the subject line.</span></span>

<span data-ttu-id="9d91a-536">互換性のテストを実行しているサポート、およびその他の照会については、drpc@microsoft.com に電子メールを送信します。</span><span class="sxs-lookup"><span data-stu-id="9d91a-536">For support if you're performing compatibility tests, and for other inquiries, send an email to drpc@microsoft.com.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="9d91a-537">追加リソース</span><span class="sxs-lookup"><span data-stu-id="9d91a-537">Additional resources</span></span>

[<span data-ttu-id="9d91a-538">周辺機器</span><span class="sxs-lookup"><span data-stu-id="9d91a-538">Peripherals</span></span>](../retail-peripherals-overview.md)
