...
- **Commerce Runtime (CRT) と Retail サーバー (RS) 拡張:** CRT および拡張機能プロジェクトすべてを **Retail SDK** フォルダーの下にコピーします。 msbuild 中にすべての拡張プロジェクトがビルドされ、プロジェクト アセンブリの出力パスが *RetailSDK\\Reference* フォルダーに設定されるように、必ず、CRT および RS 拡張ソリューション ファイルの詳細を **RetailSDK フォルダー**にある dirs.proj ファイルに含めてください。
...
前のセクション **7.3 からそれ以上のバージョンに Retail チャネル拡張機能をアップグレードする**で説明した手順はコマース プロキシを除くすべてのコンポーネントでも同じです。 RS 拡張機能を持つ CRT があり、**customization.settings** ファイルに基づいて TypeScript セッションが自動生成された場合、7.2 では、プロキシ プロジェクトでインライン変更を完了している必要があります。
