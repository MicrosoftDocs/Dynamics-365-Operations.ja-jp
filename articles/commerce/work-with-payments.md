...
さらに、ユーザーが Retail コマースのコール センターのユーザーとして返品注文を処理していても、返品を元の販売にリンクできない場合、**返品** の支払方法をコール センター パラメータで定義する必要があります。 **Retail とコマース** \> **チャネルの設定** \> **コール センターの設定** \> **コール センター パラメーター** の順に移動し、**支払方法** フィールドの **RMA/返品** タブで、支払方法が定義されていることを確認します。 支払方法は、払戻に使用される支払方法になります。 通常、チェック メソッドまたは顧客勘定方法のいずれかとして定義されます。
