...
description: この記事では、Dynamics 365 Commerce のユーザー サイト認証のために Azure Active Directory (Azure AD) の企業と顧客間 (B2C) テナントを設定する方法について説明します。
author: BrianShook
ms.date: 05/05/2022
ms.topic: article
ms.prod: ''
...
必要に応じて、**ログイン カスタム ドメイン** フィールドは、Azure AD B2C テナントのカスタム ドメインを設定する場合にのみ使用します。 **ログイン カスタム ドメイン** フィールドの使用に関する詳細および考慮事項については、以下の [追加の B2C 情報](#additional-b2c-information) を参照してください。
