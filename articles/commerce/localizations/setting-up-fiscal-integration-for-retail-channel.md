...
1. 次の機能を検索し、有効にします。
    ...
    - **会計統合技術プロファイルの上書き** – この機能は、会計統合コンフィギュレーションの拡張を有効化して、技術プロファイルのパラメーターをオーバーライドする機能を追加します。 たとえば、個々の POS レジスタ レベルで会計デバイス接続文字列を指定できます。 この機能は、POS レジスタの **設定** ページで、接続パラメーターを確認する機能も追加します。 
    - **POS レジスターの会計登録状態** – この機能を有効にすると、特定の POS レジスターの会計登録プロセスを無効にできます。 POS レジスターで会計登録が無効になっている場合は、そのレジスターで販売トランザクションを完了することはできません。
    - **会計統合データのローカル ストレージ バックアップ** – この機能は、会計登録データの自動バックアップを有効化して、会計統合フレームワークのエラー処理機能を拡張します。これにより、デバイスをアクティブ化する間に、ローカル ストレージが含むデータを復元できます。
    - **ドキュメントの登録の延期** - この機能は、会計登録に失敗した場合に会計登録を延期するオプションを有効化することで、会計統合フレームワークのエラー処理機能を拡張し、さらにバックアップ会計登録オプションを使用するか、または会計統合フレームワーク以外の方法により、会計登録を後で完了します。
...
## <a name="enable-manual-execution-of-deferred-fiscal-registration"></a>繰り延べした会計登録の手動実行を有効化する

繰り延べした会計登録の手動実行を有効化する際は、POS レイアウトに新しいボタンを追加する必要があります。
