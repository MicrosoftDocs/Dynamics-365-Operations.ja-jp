...
> [!NOTE]
> 構成パラメータの完全な一覧については、[Commerce チャット モジュールのプロアクティブなチャット パラメータ](chat-proactive-chat-parameters.md) を参照してください。
...
> [!NOTE]
> ワークストリームの既定のセッション テンプレートを変更できません。 そのため、新しいテンプレートを作成したり、既存のテンプレートを複製して更新したりすることもできます。 詳細については、[テンプレートをワークストリームで関連付ける](/dynamics365/app-profile-manager/associate-templates) を参照してください。
