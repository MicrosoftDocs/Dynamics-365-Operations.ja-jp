...
分割払い注文に影響する別のプロパティは、顧客レコードの **クレジットと回収** クイック タブにある **Mandatory credit limit** プロパティです。 このプロパティを特定の顧客に対して **はい** に設定すると、**Credit limit type** プロパティが **なし** に設定されている場合でも、顧客に対して与信限度額を確認する必要はありません。

現在、**Mandatory credit limit** プロパティが有効になっている B2B サイトには追加機能があります。 このプロパティが顧客レコードで有効になっている場合、顧客が注文を行う際に、B2B サイトでは、未払の支払方法を使用して残りのクレジット残高よりも多い支払を行うのを防止できます。 たとえば、顧客の残りのクレジット残高が $1,000 である一方で、注文が $1,200 の場合、顧客は、分割払い方法を使用して、$1,000 のみの支払を行います。 この残高の支払には、他の支払方法を使用する必要があります。 顧客レコードで **Mandatory credit limit** プロパティが無効になっている場合、顧客は任意の金額を、分割支払方法を使用して支払することができます。 ただし、顧客が注文を行い得る場合でも、与信限度額を超えた顧客注文はシステムによって履行されません。 分割支払を受け取る資格があるすべての顧客の与信限度額をチェックする必要がある場合は、**Credit limit type** プロパティを **残高 + 梱包明細または製品明細** に、 **Mandatory credit limit** プロパティを **いいえ** に設定することをお勧めします。
...
Commerce の販売注文の保留を妨げる **Exclude from credit management** プロパティは、販売注文ヘッダー (**Retail と Commerce \> 顧客 \> すべての販売注文**) にあります。 このプロパティが Commerce 販売注文に対して **はい** (既定値) に設定されている場合、注文はクレジット管理の保留ワークフローから除外されます。 ただし、このプロパティに **クレジット管理から除外** という名前が付いた場合でも、注文履行の際には定義された与信限度額が使用されます。 この注文は保留にはなりません。
...
ただし、**CredManExcludeSalesOrder** フラグが **いいえ** に設定され、B2B 顧客が POS アプリケーションを使用して店舗から購入できる場合は、現金の転記とキャリー トランザクションの転記が失敗する可能性があります。 たとえば、現金支払タイプに対するブロック ルールが設定され、B2B 顧客が店舗で一部の品目を現金で購入しました。 この場合、受注残の販売注文は保留にされ、正常に請求されません。 したがって、転記は正常におこなわれません。 このようなカスタマイズを実装した後は、最初から最後までテストすることをお勧めします。
