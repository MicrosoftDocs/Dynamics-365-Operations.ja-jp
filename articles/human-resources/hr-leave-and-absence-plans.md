...
1. **プレビュー機能: 単一の休暇および欠勤計画に対する複数の休暇タイプを構成します**。 見越計上スケジュール テーブルの各レコードに対して、休暇タイプを定義できます。

   > [!IMPORTANT]
   > この機能を有効にしたら、オフにすることはできません。

2. **プレビュー機能: フルタイム相当を使用します**。 このプレビュー機能を有効にした場合、Human Resources は、職位に対して定義されたフルタイム相当 (FTE) を使用して従業員の見越計上の比例配分を行います。 たとえば、FTE が 0.5 で、見越計上量が 10 の場合、従業員は 5 を見越計上します。 この機能は、複数の休暇タイプを有効にした場合にのみ使用できます。
