...
# <a name="authorize-an-adjusted-forecast"></a>調整された予測を承認します。
...
予測が生成されるよう指定した開始日と終了日はバケットの開始日と終了日に対応する必要があります。 システムは調整が必要な場合は、この制限を適用し、自動的に日付を調整します。 
...
[**ベースライン需要予測に対して行われた手動調整の保存**] チェック ボックスの機能を理解することは非常に重要です。 統計的な基準の予測に手動調整がある場合、調整された値は使用する場合は、このチェック ボックスを取り除かれても、承認されます。 ただし、変更は承認後に破棄されます。 したがって、次に予測が生成される時、[**手動調整を需要予測に転送**] が選択されている場合でも、その予測は統計予測のみで手動上書きはありません。 したがって、[**ベースライン需要予測に対して行われた手動調整の保存**] チェック ボックスを、すべて手動による変更で維持または破棄するメカニズムと仮定できます。
