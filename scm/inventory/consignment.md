...
委託販売在庫は、仕入先によって所有されているが、サイトで保管されている在庫です。 在庫を消費するか、または使用する準備ができたら、在庫の所有権を引き継ぎます。 このトピックでは、方法を物理的に総勘定元帳トランザクションの仕入先所有の在庫を物理的に引当は、生産プロセスを開始する方法を作成しないで[仕入先の所有在庫の入荷が含まれます。 [原材料の所有権を製造オーダーの処理中に消費を処理できるに変更する方法。 また、仕入先が仕入先コラボレーション インターフェイスを使用して在庫の消費を監視する方法についても説明されます。 入庫委託販売プロセスを有効にしてコンフィギュレーションする方法にの詳細については、「[委託販売の設定](set-up-consignment.md)」を参照してください。
...
委託販売補充注文は、仕入先が注文済在庫トランザクションを作成して特定の日付範囲内で配送する予定の製品在庫数量を要求および追跡するために使用されるドキュメントです。 通常は、これは特定の製品の予測と実需に基づいています。 委託販売補充注文に対して入庫する在庫は仕入先に所有権があります。 現物入庫の更新に関連する製品の財産のみ記録されるため、総勘定元帳トランザクションの更新は発生しません。 **所有者**分析コードは、仕入先が所有する在庫および受領する法人によって所有される在庫に関する情報を区別するために使用されます。 委託の補充注文明細行に行の全数量が行われていない、または取り消されなかった、**オープン注文**ステータスです。 数量全体を受け取るか、またはキャンセルされた場合は、ステータス変更されますはに**完了した**。 委託販売補充注文に関連する現物手持在庫は、製品受領更新プロセスと共に登録プロセスを使用して記録できます。 登録は品目到着プロセスの一部として、または手動で注文明細行を更新することができます。 製品受領更新プロセスを使用すると、レコードは仕入先に商品の受領を確認するのに使用できる製品受領書仕訳帳で作成されます。 

[補充]![委託販売注文 (。/media/consignment補充Order.png) ] (。/media/consignment補充order.png) 
...
-   **発注書** **消費の委託販売用店舗在庫- **委託販売プロセスの所有権の変更に関連する特定ごとに詳細化された発注書情報。
