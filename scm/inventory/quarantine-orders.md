...
検査指示は、在庫をブロックするのに使用できます。 たとえば、 品質テストの理由で品目を検査する場合があります。 在庫して、検査倉庫に移動される検査された。 **注記:** 高度な倉庫管理プロセス (倉庫管理で) を使用している場合、検査指示の処理は返品販売注文に対してのみ使用されます。

## <a name="quarantine-onhand-inventory-items"></a>onhandの在庫品目を検査します。
