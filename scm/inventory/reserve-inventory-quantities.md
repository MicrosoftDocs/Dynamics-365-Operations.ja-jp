...
| 品目販売引当  | 品目の引当を手動または自動で行うかを指定します。 引当が自動の場合は、注文明細行の作成時に在庫の引当を行います。 BOM (**自動** オプション) のレベルの品目番号で、または BOM (**展開** オプション) の個々の要素で引当を行うことができます。 既定値の**品目の販売引当** **売掛金勘定パラメータから継承された場合がある。** このページの値は**販売既定値** **セクションの引当]フィールドに**設定されています** **概要]タブで表示します。 |
