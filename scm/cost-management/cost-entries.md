...
振替仕訳帳イベントが登録されます。 イベントはサイト1 ~ 2.のサイトを指定する品目Aの一つを移動します。 サイト在庫分析コードは原価のオブジェクトの一部と見なされます。 したがって、イベントは 2 個の在庫トランザクションを作成し、2 個の原価エントリを作成します。
...
| 伝票番号 | 日       | 参照      | 番号 | ロット ID  | 参照ロット | 返品ロット ID | 件数 | 金額  |
|---------|------------|----------------|--------|---------|---------------|---------------|----------|---------|
...
[**原価エントリ**] ページは、ドキュメント ID とドキュメントの日付によるフィルター処理を有効にします。 **注記: **のエントリは次の目的でのみ[原価のオブジェクト] (原価object.md) またはリリース済製品。
