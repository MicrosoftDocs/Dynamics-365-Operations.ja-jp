...
# <a name="kanban-job-scheduling-for-lean-manufacturing"></a>細い製造のKanbanジョブのスケジューリング
...
スケジューリングはkanbanルールの自動**計画数量**パラメータを設定すると自動的に発生することができます。 **自動計画数量** ** ** 1に設定した場合に、kanbanジョブが作成した場合、計画されます。 結果は、一連の最初のプル、最初のサービス操作です。 **自動計画数量**に 1 を超える値をセットした場合、かんばん作業は計画前にグループ化されます。 このコンセプトにより、かんばんのサイズを実際の経済的なバッチサイズより低くすることができます。 たとえば、特定の品目 (または品目ファミリ) の経済的なバッチ サイズは30です。 製品の数量を使用するkanbansを作成する代わりに、30に10の製品の数量があると**自動**値計画数量のできるようにkanbanルール設定** 3 **。 3 つの予定外のジョブがある場合にのみ自動計画は、作業セルにかんばん作業をスケジュールしますが、スケジューリングをする人と作業現場の監督者には、2 つの予定外のジョブが実行待ちかもしれないことははっきりと分かります。 スケジューリングをする人やマネージャーは、手動でその作業をスケジュールするか、追加のかんばんを作成することでそれら 2 つの作業の生産を開始することができます。
