---
title: "生産プロセスの概要"
description: "この記事では、生産プロセスの概要が示されます。 これは注文の作成からの財務期間の原価計算に製造オーダー、バッチ オーダーのさまざまなステージおよびkanbansついて説明します。"
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
この記事では、生産プロセスの概要が示されます。 これは注文の作成からの財務期間の原価計算に製造オーダー、バッチ オーダーのさまざまなステージおよびkanbansついて説明します。 
...
** **生産管理モジュールは、他のモジュールに、**などの製品情報管理**、**在庫管理**、**総勘定元帳**、**倉庫管理** **、プロジェクト会計**、リンクされ**組織管理**。 この統合では、完成品目の製造に必要な情報フローをサポートしています。  

生産プロセスは、通常、特定の生産プロセスに対して選択された原価計算と在庫評価方法からの影響を受けます。 Dynamics 365 for Operationsは、実績原価をサポートします (先入れ先出し \[先入れ先出し法\]; 後入れ先出し (LIFO) \[LIFO\]; 移動平均; [定期的な加重平均) と標準原価方法。 リーン生産は、一括引き落とし原価計算原則に基づいて実行されます。  
...
さまざまな製品と生産のトポロジーで、さまざまな注文タイプの適用が要求されます。 Dynamics 365 for Operationsは、生産モードのさまざまな注文タイプを適用できます。 つまり、すべての注文タイプが、1 つの製品を製造するエンド ツー エンドのプロセス中に使用することができます。
...
2.  **見積済** – 材料とリソースの消費の見積を計算できます。 見積では、原材料の在庫トランザクションが**注文中**ステータスで生成されます 。 主要な製品、連産品および副産物の入庫は、製造オーダーまたはバッチ オーダーが生成"見積済"になります。 BOMが含まれる場合の行は、製造オーダーまたはバッチ オーダーに**釘付けに提供された**、材料の発注書または使用される工程サービス生成され、釘付けになります入力します。 品目や注文は、製造オーダーの引当の方針に従って引当され、完成品の価格はパラメータ設定に基づいて計算されます。
...
5.  **準備される**/**選択した**すべての材料とリソースが生産の場所に許可された場合–、生産BOM明細行またはkanban行のステータスに更新されます**選択した**。 ペギングされた供給の注文および関連する倉庫の作業は、通常このステージで完成します。 生産の進捗状況を報告するために必要な、かんばんカードまたはジョブ カードが割り当てられ、印刷されます。
