...
活動または照会のメニュー項目を作成するには、**モード**フィールドを設定します。**間接**。 **活動コード**オプションの一覧は、メニュー項目は、の対象の照会または活動のタイプを選択できるように、使用可能になります。 メニュー項目を倉庫作業を生成するために作成するには**モード**フィールドを設定します。**作業**。 **作業作成プロセス**オプションの一覧は、使用可能になります。 既存の倉庫の作業を処理するメニュー項目を作成するには、**モード** フィールドを**作業**に設定し、**既存の作業を使用**オプションを**はい**に設定します 。 **注記: **追加のフィールドがメニュー項目に使用できる、メニュー項目を選択し、かによって既存の作業を実行する場合でも、メニュー項目が使用されるモードかもしがあります。 追加のフィールドの選択方法の詳細については、この記事の追加「メニュー項目オプション」を参照してください。
...
倉庫の作業を作成するメニュー項目の設定に加えて、既に作成されている作業を処理するメニュー項目を設定できます。 **モード** フィールドを**作業**に設定し、**既存の作業を使用**オプションを選択します。 ある追加オプションは、** **概要]タブで使用できるようになります。 **作業クラス** FastTabの作業クラスの割り当てによってメニュー項目へのアクセスを制御することができます。 作業クラスは、メニュー項目により処理することができる作業を定義します。 また、作業クラスは、特定のユーザー ロールへのアクセス許可、またはさまざまな工程の異なる処理へのアクセスを許可を付与するのに使用できます。 次の表に、使用できるオプションを示します。
...
<tr class="even">
<td>システム主導</td>
<td>、Microsoft Dynamics 365は員が、作業を実行するため、注文に割り当てられている作業のタイプを制御します。 このオプションを選択 <strong>システム指示された作業</strong> した場合、作業の並べ替え基準を設定できるページを開くにアクション ウィンドウを付けることができます。 並べ替え基準はための作業を実行する順序を制御します。 必要なな数の条件を追加できます。</td>
</tr>
<tr class="odd">
<td>ユーザー主導</td>
...
<tr class="even">
<td>検証済ユーザー主導</td>
<td>作業者は、作業が積荷または出荷などのより大きいエンティティに関連付けられるときに実行する作業を選択します。 ワーカー、品目がピッキングされる順序が決まります。 このオプションを選択する場合、次のフィールドが要求されます。
<ul>
...
<tr class="even">
<td>固定</td>
<td>提示されているステージングまたは積載場所を上書きする場所を作業者が指定できるようにするには、このオプションを選択します。 残りのプット アウェイ作業のすべてが、新しい場所に振り分けられます。 このオプションは、たとえば、作業者がオーダー 1 の品目をドック 1 のステージング場所に配置しようとしたとき、前の積荷がその場所から取り除かれていないときに役立ちます。 1、ドックのステージングの場所を待機するのではなくワーカーを使用できるようにするために、ドック2.にステージングの場所を使用するように設定できます。 この場合、ワーカーには、提案されたステージングの場所を上書きします。 ワーク オーダーの残りのすべての品目の収納場所が、ドック 2 のステージングの場所に更新されます。 このオプションを選択した場合、<strong>固定担当者</strong>フィールドを設定する必要があります。</td>
</tr>
<tr class="odd">
<td>固定担当者</td>
...
<tr class="odd">
<td>作業テンプレートのコード</td>
<td>プロセスの作業を作成する作業テンプレートを選択します。 たとえば、発注書の品目を受け取った場合は、作業を生成する作業テンプレートに基づいて設定します。 作業テンプレートを選択しない場合、Microsoft Dynamics 365はクエリ基準に基づいてテンプレートを割り当てます。 作業のテンプレートの詳細については、<a href="control-warehouse-location-directives.md">作業テンプレートと場所ディレクティブでの倉庫作業の制御</a> を参照してください。</td>
</tr>
</tbody>
</table>
...
[設定タイプの発注書 (作業ガイド) 完了するためのモバイル デバイスのメニュー項目の作業を] (https://ax.help.dynamics.com/en/wiki/set-up-a-mobile-device-menu-item-for-completing-work-of-type-purchase-order/)

[設定入庫済品目 (作業ガイドを登録する) モバイル デバイスのメニュー項目] (https://ax.help.dynamics.com/en/wiki/set-up-a-mobile-device-menu-item-to-register-received-items/)
