...
輸送管理では、会社の配送の管理および送受信オーダーのベンダーとルート指定の解決策の識別を使用できるようになります。 たとえば、出荷用の最速のルートまたは最低費用レートを識別できます。 次の表では、Microsoft Dynamics 365 for Finance および Operations で輸送管理を使用する主要なシナリオについて説明します。
