...
1 つの積荷に複数の出荷がある場合、主船荷証券を生成します。 これは船荷証券と同じレイアウトおよび情報がありますが、すべての出荷の集計されたコンテンツが含まれます。 [**輸送管理パラメーター**] ページの [**1 つの積荷に複数の出荷がある場合は主船荷証券を作成**] オプションで [**あり**] を設定した場合、複数の出荷があるときに [**積荷計画ワークベンチ**] から主船荷証券を作成すると、主船荷証券が自動的に生成されます。 [**Related information**] &gt; [**船荷証券**] の順にクリックして船荷証券のリストを取得できます。 船荷証券を手動で作成する場合は、[**船荷証券**] ページで主船荷証券を作成できます。
