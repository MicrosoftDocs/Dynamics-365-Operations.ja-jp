...
# <a name="set-up-vendor-accounts"></a>仕入先を設定します。
...
仕入先を作成するプロセスの処理速度を上げるために、テンプレートを作成できます。 テンプレートを作成、**仕入先**、アクション ウィンドウで、[**オプション**に** &gt; 記録してヒントを**]ページでします。 次に、[**会社コード テンプレート**] をクリックします。 会社コード テンプレートは、他のユーザーと共有されます。  
...
の仕入先の連絡先を追加するには**すべての仕入先**で、**仕入先**記録、**設定**グループに、[** ** ** &gt; 連絡先を追加する連絡先を**]ページでします。  
...
Microsoft Office 365の個人的な連絡先に仕入先の連絡先を追加できます。 ただし、まずMicrosoft Exchange Serverの両方同期、Microsoft Outlookセットアップ ウィザードの工程と会社365 for Operations 365の間で同期を設定する必要があります。
