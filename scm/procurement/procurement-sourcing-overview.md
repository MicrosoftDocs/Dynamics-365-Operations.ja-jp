...
発注書は、より包括的な見積依頼プロセスの代替として仕入先に送信する*購買照会*オプションを含みます。 注文の価格、割引、出荷日などの条件の設定を支援する場合にも、購買照会を使用できます。 **仕入先**ポータルを使用するように仕入先が設定されて**発注の照会機能は無効です。 代わりに、順序は**仕入先**ポータルで共有され、*確認要求*が送信されると、仕入先は直接注文を確認できます。  
...
-   購買を必要とする要求を識別するマスタ プラン結果として。 このプロセスでは、計画発注書を生成し、これらがリリースされると、発注書が生成されます。
-   購買要求の処理によって、調達されます。
...
発注書を作成すると、フィールドの多くは**仕入先**ページの仕入先の保存された情報から取得される値とprepopulated。 これは既定値の上書きを選択できますが、発注書で入力する必要があるフィールドの数に制限があることを意味します。
