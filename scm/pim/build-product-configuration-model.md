...
# <a name="build-a-product-configuration-model"></a>製品モデル コンフィギュレーションを作成します。
...
ルール ベース、分析コードベースおよび制約ベースモデルなどのようなモデルとなる複数の製品コンフィギュレーションの原則があります。 研究によれば、制約ベース方法は他のモデルの原則と比較してモデルの明細行を 50% 削減できることを示しています。 したがって、この方法により総保有コスト (TCO) を削減できます。 制約ベースのモデルにX++コードに基づいているルールに基づいてモデルから移動すると、この製品モデルを管理するために開発者のライセンスが必要はありません。
...
テーブル制約は、ユーザー定義します。またはシステムで定義します。  

ユーザー定義のテーブル制約は、ユーザーによって構築されます。 ユーザーは属性タイプの組み合わせを選択して、テーブルの列を表し、選択した属性タイプのドメインから値を入力して、テーブル制約の行を作ります。  

システムで定義されたテーブル制約は、このテーブルの参照と制約の列を形成するために選択]フィールドとして使用する、テーブルのMicrosoft Dynamics 365選択によって定義されます。 テーブル制約の行は、コンフィギュレーション時にあるアクション テーブル365 for Operationsの行です。  
...
モデルをテストすると、実際のコンフィギュレーション セッションの実行に似ています。 ユーザーは、ページを使用歩き、モデル構造がコンフィギュレーション プロセスをサポートすることを確認できます。 ユーザーは、属性値が正しいこと、属性の説明がユーザーに正しい値を選択させられることを確認できます。 最後に、テストのセッションが完了した後、システムは BOM および選択した属性値に対応する工順の作成を試み、何か間違いがあればエラー メッセージが表示されます。
...
開発者ライセンスがあるパートナーと他のユーザーが製品コンフィギュレーション モデルの処理能力を拡張できるように、専用のアプリケーション プログラミング インターフェイス (API) が実装されてきました。 主な目的は、パートナーを有効にする既存のプロダクト ビルダを使用して、顧客がAPIへのプロダクト ビルダ モデルに組み入れるコードを移行する機能を、設定します。 これにより、モデルを [プロダクト ビルダー] から製品コンフィギュレーションに移行することができます。 ただし、新しいパートナーや顧客は、新しい製品コンフィギュレーション モデルを拡張して API を使用して利益を得ることもできます。

### <a name="pcadaptor-class"></a>PCAdaptor クラス

製品コンフィギュレーション モデルのデータ構造を公開する一連の [**PCAdaptor**] クラスの使用により API　が実装されます。 ** PCAdaptor **クラス インスタンスを拡張する各モデルで作成する必要があります。 コンフィギュレーション セッションが完了した後、見つけたらこのクラスの実行のインスタンスのシステムのチェック。  

次のフロー ダイアグラムは、プロセスの概要を示します。  

[![の流れ図] (。/media/product_configuration_2.png) ] (。/media/product_configuration_2.png)   
