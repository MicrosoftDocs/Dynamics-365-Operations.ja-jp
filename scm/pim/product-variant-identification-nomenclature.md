---
title: "製品番号の職務語"
description: "このトピックでは、固定長フォームを交換するに製品番号の職務語を設定するか[製品のマスタ数-コンフィギュレーション]を[サイズ]色のスタイル]ついて、選択した製品のマスタ番号、有効な製品と分析コード テキストの区切り記号が含まれる目標があるフォームに。 また、制約ベースの製品コンフィギュレーターによって作成されるコンフィギュレーションを識別するための分類も構築できます。 これらの分類は、選択した属性を含めることができます。"
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="product-number-nomenclature"></a>製品番号の職務語

このトピックでは、固定長フォームを交換するに製品番号の職務語を設定するか[製品のマスタ数-コンフィギュレーション]を[サイズ]色のスタイル]ついて、選択した製品のマスタ番号、有効な製品と分析コード テキストの区切り記号が含まれる目標があるフォームに。 また、制約ベースの製品コンフィギュレーターによって作成されるコンフィギュレーションを識別するための分類も構築できます。 これらの分類は、選択した属性を含めることができます。
...
## <a name="nomenclature-of-constraintbased-configurations"></a>constraintbased組織の職務語
制約ベースのコンフィギュレーションに対して、専用の専門語は、製品の分析コードに作成できます。 [**製品の分類**] ページで次の区分を選択できます。
...
## <a name="nomenclature-of-dimensionbased-configurations"></a>dimensionbased組織の職務語
分析コードベースのコンフィギュレーションについては、専用の専門語は、製品の分析コードに作成できます。 [**製品の分類**] ページで次の区分を選択できます。
...
製品のバリアント番号があります: M0099\_PlasticAAA12。
...
分類によっては、システムはコンフィギュレーションに対して一意ではない製品のバリアント番号を割り当てようとする可能性があります。 この場合、製品のバリアント番号としてコンフィギュレーション分析コードに番号順序を代わりに使用されます。 こうした場合は、警告が発生します。 この問題を避け、一意性を確保するために、十分な属性を含め、コンポーネントに対して [**再利用**] オプションが有効とされていることを確認します。
...
[事前に定義された製品のバリアント (作業ガイド作成) ]の製品番号の職務の語 (http://ax.help.dynamics.com/en/wiki/create-a-product-number-nomenclature-for-predefined-product-variants/)

[設定された製品のバリアント (作業ガイド作成) ]の製品番号の職務の語 (http://ax.help.dynamics.com/en/wiki/create-a-product-number-nomenclature-for-configured-product-variants/)
