---
title: "製品コンフィギュレーションの概要をシミュレーションできます。"
description: "この記事では、製品コンフィギュレーション モデルに対するについて、配送条件を定義します。 製品モデル コンフィギュレーションは一つの製品の複数の製品のバリアントをコンフィギュレーションするのに使用できるノーブランド商品の構造を作成することができます。"
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="product-configuration-models-overview"></a>製品コンフィギュレーションの概要をシミュレーションできます。

この記事では、製品コンフィギュレーション モデルに対するについて、配送条件を定義します。 製品モデル コンフィギュレーションは一つの製品の複数の製品のバリアントをコンフィギュレーションするのに使用できるノーブランド商品の構造を作成することができます。
...
また、属性の条件も指定できます。 条件を満たした場合は、必須属性に値を入力する必要があります。 条件は、製品コンフィギュレーション モデルに含める属性、BOM 明細行、および工順工程に対して満たす必要のある式です。 条件で参照する属性はすべて必須となります。 [<strong>属性</strong>] タブで属性を必須として選択することをお勧めします。 これにより、必須属性の識別が容易なります。 属性値は、コンフィギュレーションの再利用に重要な部分です。 システムは、コンフィギュレーション セッション中にユーザーが行った選択と一致するコンフィギュレーションがあるかどうかを、属性値を使用して判断します。</td>
</tr>
<tr class="odd">
<td>属性タイプ</td>
...
属性タイプが範囲のある<strong>ブール型</strong>、<strong>整数</strong>、または固定リストのある<strong>テキスト</strong>の場合、値のセットは製品コンフィギュレーション モデルが設定されている場合に使用できます。 <strong>注:</strong> 製品コンフィギュレーションの解答者は次の属性の型を特定します: <strong>ブール</strong>[ <strong>テキスト</strong> を使用して固定リストにと範囲。 したがって、式の制約と条件ではこれらの属性タイプのみ使用できます。</td>
</tr>
<tr class="even">
<td>制約</td>
<td>制約はプロダクト モデルのコンフィギュレーションの制限を表します。 制約を使用することで、製品がコンフィギュレーションされている時に、必ず有効値のみが選択されるようになります。 制約は式の制約やテーブルの制約のいずれかです。
<ul>
<li>式の制約は関連付けられたコンポーネントにのみ使用できます。 コンポーネントの式の制約は、コンポーネントのサブコンポーネントの属性を参照できます。 製品 Solver configuration は制約の決済に使用されます。制約を書き込む際には Solver の構文を使用する必要があります。 細については、式の制約およびテーブルの制約に関する Wiki のリンクを参照してください。</li>
<li>テーブル制約は、製品コンフィギュレーション モデルのコンポーネントに適用する前に定義する必要があります。 テーブル制約は、ユーザー定義します。またはシステムで定義されます。 ユーザー定義のテーブル制約は、属性タイプで定義されている属性値の組み合わせセットの説明に使用できるマトリックス タイプです。 たとえば、スピーカー生産の場合、ユーザー定義のテーブル制約のマトリックスには、スピーカーの表面処理とグリルの列を含めることができます。</li>
</ul>
<strong>例</strong> スピーカーの表面処理には黒、カシ、シタン、白色の 4 つが使用可能です。 スピーカーは3つの前部グリルの一つを使用できます: 黒、金属、または白色。 破線は、すべてのグリルに使用できますが、他の実行、特定のグリルに限定されます。 次の表には、<strong>テーブルの制約の編集</strong>ページの<strong>許可済み組み合わせ</strong>タブに表示される情報の例が表示されます。
<table>
<thead>
<tr class="header">
<th>キャビネットの仕上げ</th>
...
システムで定義されたテーブル制約は、テーブル365 for Operationsの属性の型フィールドとの間のマッピングを表します。 システムで定義されたテーブル フィールドに制約は動的に属性の型にリンクされます。 リンクは、製品コンフィギュレーション モデルの属性が、テーブル365 for Operationsのフィールドのデータを反映することができます。</td>
</tr>
<tr class="odd">
<td>計算</td>
<td>計算は、制約に提供品を表します。 とタイプの属性の算術演算を実行する場合 <strong>小数点</strong> でも、固定 <strong>整数</strong> リストと型の属性を含む算出の <strong>テキスト</strong> 値を論理的な工程を使用 <strong>ブール</strong> できます。 計算には計算式の結果を保持するターゲット属性があります。 式エディタを使用して計算式を構築します。</td>
</tr>
<tr class="even">
<td>サブコンポーネント</td>
