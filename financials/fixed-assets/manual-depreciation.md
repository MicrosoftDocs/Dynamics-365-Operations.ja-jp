...
**半年ごとまたは** ****の期間頻度** **フィールドでは、二つの手動スケジュール間隔を設定します。 次の表にはこれら 2 間隔の減価償却量が表示されます。
...
合計すべての範囲の割合が100である必要があります。 **ただし固定資産減価償却プロファイルは、スケジュールと**ページの**累積割合**フィールドの値がない場合は、メッセージを受け取ります** 100 **。
