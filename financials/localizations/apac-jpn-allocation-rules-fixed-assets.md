...
この例では、固定資産の減価償却費の金額が 170 通貨単位です。 自分の法人の 100 種類の分析コードごとに減価償却費の 1% を割り当てる配賦ルールを設定します。 また、である僅かの通貨単位は最も近い整数通貨単位により中途半端としてのすべての通貨単位の間に四捨五入することを示す四捨五入のルールを設定します。 このシナリオでの分析コードごとの減価償却費の計算方法をここに示します。
