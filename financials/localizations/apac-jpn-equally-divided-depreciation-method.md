...
## <a name="how-is-depreciation-calculated-using-the-equally-divided-depreciation-method"></a>減価償却はどのようにこれらに分けられた減価償却方法で計算されるか。
**減価償却プロファイル** ページの [**方式**] フィールドで、減価償却方法として [**均等**] を選択して、減価償却プロファイルを設定できます。 固定資産の減価償却で均等に分けられた減価償却プロファイルを使用すると、Microsoft Dynamics 365は次の式を使用して、減価償却金額を計算します: 減価償却は= ×残余額 (残りの年の一つ÷の番号) (今年の一つ÷の期間の数)、次の式を四捨五入する×を四捨五入する残余額を計算するのに使用されます: 減価償却金額を計算するで均等に分けられた減価償却方法を使用すると、残余額=取得原価–の減価償却累計額は、**帳簿**ページで指定した減価償却期間と耐用年数ではなく**減価償却プロファイル**ページの**均等に減価償却金額の分割年数**フィールドの値固定資産の耐用年数を表示します。
...
一連番号 減価償却金額は月間に計算する場合は、によって変更されません。 たとえば、以下の一括比例配分の固定資産を取得します。
...
## <a name="how-is-equally-divided-depreciation-calculated-for-a-lowvalue-asset-that-is-acquired-midyear"></a>区切り減価償却はどのようにこれらは年度の半ば取得lowvalue資産に計算するか。
会計年度の途中に、値の固定資産を購入する場合は、Microsoft Dynamics 365は、現在の会計年度の減価償却計算の最初の会計年度が保持されます。 たとえば、会計年度の中ごろに、低価額の固定資産を取得します。
...
## <a name="how-is-equally-divided-depreciation-calculated-for-a-lumpsum-asset-that-is-acquired-midyear"></a>区切り減価償却はどのようにこれらは年度の半ば派生し金額の資産に計算するか。
会計年度の途中にし金額の固定資産を購入する場合は、Microsoft Dynamics 365は固定資産の購買の日付から現在の会計年度が計算されます。 たとえば、会計年度の中ごろに、一括比例配分の固定資産を取得します。
...
## <a name="how-is-depreciation-calculated-using-the-equally-divided-depreciation-method-if-the-asset-calendar-is-changed-during-the-asset-life-cycle"></a>減価償却はどのようにこれらに分けられた減価償却方法を使用して資産カレンダーは資産のライフサイクル中に変更するかまたは計算されます。
、Microsoft Dynamics 365が均等に分けられた減価償却方法を使用して減価償却を計算する場合は、計算が**帳簿**ページで**カレンダー**フィールドに使用できる会計選択したカレンダーに基づいています。 資産カレンダーが資産の有効期間中に変更された場合、現在のカレンダーに基づいて、会計年度内の月または四半期などの会計年度期間の数を計算します。 たとえば、以下の一括比例配分の固定資産を取得します。

-   会計カレンダー = 4 月 1 日から 3 月 31 日
-   一括比例配分の固定資産の取得日付 = 2012 年 4 月 1 日
-   均等減価償却量に対する年数 = 3 年

2013年12月4で、2番目の会計年度中に、会計カレンダーから4年1月1日から3年1月31日から1年1月1日から12年1月31日を変更すると、会計年度の減価償却期間数は次の表に示すように計算されます。
...
一連番号 これらの区切り減価償却方法を使用して固定資産の減価償却時に、減価償却金額は、固定資産の耐用年数内の会計期間に均等に分割されます。 したがって、固定資産の有効期間中は減価償却方法を変更できません。

## <a name="can-i-change-the-depreciation-period-for-an-equally-divided-depreciation-profile"></a>均等減価償却法プロファイルの減価償却期間を変更できますか。
一連番号 **帳簿**ページで均等に分けられた減価償却プロファイルを選択した場合、減価償却プロファイルの**減価償却プロファイル**ページで**均等に分割の減価償却金額に対する年数**フィールドに正の値を変更できません。

<a name="see-also"></a>参照
--------

[固定資産減価償却 (FAQ] apac jpn固定資産depreciation.md) 
