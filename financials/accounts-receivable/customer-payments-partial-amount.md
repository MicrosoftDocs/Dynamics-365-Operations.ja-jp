...
顧客は全額請求書を決済するために追加の一部支払を作成するつもりで、一部支払を行う場合があります。 一部支払の現金割引を実行するには、** **売掛金勘定パラメータ**ページで、[**一部支払の現金割引を計算し、**オプションを**はい設定する必要があります。 
...
顧客は残余額が支払われる場合に、一部支払を行うではありませんが、かもしがあります。**勘定の買掛金勘定パラメータ**ページで指定される過少支払金額よりも大きい場合。 全額決済されるように請求書をマークする場合は、**決済のトランザクション**ページの**全額決済**オプションを使用できます。 (コンフィギュレーション キーを使用して全額決済機能を有効にできます)。 たとえば、請求書に 1,000.00 が転記され、顧客は 990.00 の支払を行います。 顧客が残りの10.00を支払う必要がないことに合意します。 決済の請求書をマークした後、[を指定できます**全額決済**。 請求書は、完全に決済されたとみなされます。 差額 10.00 が、追加の現金割引量として現金割引口座に転記されます。
