...
**販売注文の顧客請求書**は、売上に関連し、組織が顧客に提供する請求書です。 この種類の顧客請求書は、注文明細行および品目番号を含む販売注文に基づいて作成されます。 品目番号が指定され、元帳に転記されます。 Subledgerの仕訳帳エントリは、販売注文の顧客請求書は使用できません。 
...
**販売注文**リスト ページの複数の請求書を選択し、[**請求書の生成**] を使用して連結します。 **転記の請求**ページでは、注文番号に集計する (単一の請求元仕入先の複数の販売注文が存在する場合) **集計注文**設定を変更できます (一つの販売注文に対して複数の梱包明細が存在する場合) または請求元仕入先します。 **構成します。** **集計注文**設定に基づいて単一の請求書に販売注文を、連結するボタンを使用します。
