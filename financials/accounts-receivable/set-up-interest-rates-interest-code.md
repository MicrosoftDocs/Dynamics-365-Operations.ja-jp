...
-   利息金額はすべての通貨に適用されます。
-   オプションの利子の限度額を入力できます。
-   **割合**選択** **の**対象を、**のフィールドについて、**設定利子コード**ページ計算します。

たとえば、請求書の支払がトランザクションの期日を超えること2か月ごとの対象に5を評価する利子コードを設定するには、で、を**、利息を計算します。**フィールドにつき[月** **入力します。

## <a name="interest-rates-based-on-amounts"></a>金額に基づいた利率
通貨ごとの指定金額を計算する利率を設定できます。
-   利息金額は、利息コードで通貨ごとに指定されます。
-   オプションの利子の限度額を入力できます。
-   **金額**で[**対象を、**のフィールドについて、**設定利子コード**ページ計算します。

たとえば、請求書の支払がトランザクションの期日を超えること20日ごとに25.00の対象を評価する利子コードを設定するには、で20を**、利息を計算します。**フィールドにつき、[**日**入力します。
