---
title: "予算管理の概要"
description: "この記事では、超過予算を管理できるように管理をよって、Microsoft Dynamics 365の予算管理をコンフィギュレーションする際に役立つ情報を提供します。"
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
この記事では、超過予算を管理できるように管理をよって、Microsoft Dynamics 365の予算管理をコンフィギュレーションする際に役立つ情報を提供します。

<a name="overview"></a>概要
--------

、Microsoft Dynamics 365の予算管理が使用可能な資金の勘定科目表を使用して組織の管理の超過、作業の現在、ユーザー グループ、ドキュメント ソース、仕訳帳、コンフィギュレーション可能な計算、予算期間、しきい値サポートされます。 適切に操作される場合、組織は会計年度中の財務資源の計画、測定、管理、予測が可能になります。 

予算が365 for Operationsで、承認された後、組織の支出の予算を記録する予算の登録エントリを生成する場合でも、予算の計画を使用できます。 また、サード パーティ製プログラムの予算の計画の機能を使用する代わりに、予算に登録エントリを作成するか、またはインポートできます。 
...
-   **ソース ドキュメント**、ソース ドキュメントが予算管理の評価する必要があるか–か。 ドキュメントは行またはドキュメントごとに設定する必要がありますか。
...
予算管理は、Dynamics 365 for Operationsで十分に統合されます。 したがって、計画発注書と実際の購買の両方に使用できる予算を評価できます。 予算の照会およびレポートを使用できます。 したがって、予算のリビジョンや移動の形式で、ユーザーが予算サイクル中の予算を評価し、必要に応じて調整を行うことができます。 予算マネージャーは、予算と実績を Microsoft Excel にエクスポートすることにより、必要に応じてより良い分析および予測を行うことも可能です。
...
組織の次の手順は、**予算管理コンフィギュレーション**ページのさまざまなタブで入力します。
...
> [!NOTE] 
> 計算の予算期間中に変更した場合、変更は前の予算管理のチェックに合格した転記済、または完了してドキュメントを影響しません。
...
> [!Important]
> 予算管理が**損益**、**経費** **、収益、貸借対照表、職責、賃金**または**資産**タイプのすべての主要な勘定に有効になります。 このタブに空の基準を含むルールが含まれている場合、予算管理は、これらのタイプの主勘定を含む**すべて**の財務分析コードの組み合わせで有効になります。 したがって、予算管理を有効にした財務分析コードの組み合わせの範囲内で定義された予算管理ルールがオンになっていることを確認してください。  
...
> [!Important]
> 予算管理に、有効な始動した後、およびトランザクションが転記された後、年のチューニングの半ば必要があります。 予算管理を無効になると、活動が予算管理目的で記録されず、予算チェックが実行されません。 したがって、既に転記されている伝票は、削除された金額、予算管理に関連した照会およびレポートなどの残高を正確に反映していない可能性があります。 これらには、下流の予算管理統計または調整伝票および仕訳帳が含まれます。 
...
このページから、ユーザーは**期間ごとの予算管理統計選択された予算管理の分析コードの組み合わせの予算の使用の詳細と引当を表示する**ページにすることができます。 ユーザーは、[**予算管理統計**] ページを詳細表示し、予算管理で使用されるすべての財務分析コードの組み合わせの予算の利用可能性を表示できます。 
