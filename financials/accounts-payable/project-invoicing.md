...
ある品目を仕入先に注文してその品目をプロジェクトに割り当てると、品目の発注書明細行に対して選択した明細行プロパティにより、購買品目を顧客に請求するかどうかが決まります。 既定の明細行プロパティを設定すると、購買注文明細行の品目に対して表示されます (明細行の詳細は &gt; 明細行プロパティを &gt; 計画します)。 明細行プロパティを変更する 2 つの方法があります。
