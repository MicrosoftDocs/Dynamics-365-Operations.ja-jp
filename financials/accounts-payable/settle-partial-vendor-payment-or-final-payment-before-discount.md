---
title: "割引日より前に、部分的な仕入先支払および最終支払を完全に決済します。"
...
# <a name="settle-a-partial-vendor-payment-and-the-final-payment-in-full-before-the-discount-date"></a>割引日より前に、部分的な仕入先支払および最終支払を完全に決済します。

この記事は、仕入先請求書に対して一部支払が作成され、現金割引が提供されるシナリオについて説明します。

Fabrikamは、仕入先3064の商品を購入します。 仕入先は請求書がFabrikam 14日に支払う場合、1の現金割引を示します。 請求書は 30 日以内に支払う必要があります。 仕入先は、Fabrikam が一部支払に対する現金割引を選べるようにすることもできます。 決済パラメータは**買掛金勘定パラメータ**ページにあります。 6 月 25 日に、April は、仕入先 3064 に対して 1,000.00 ドルの請求書を入力します。

## <a name="vendor-invoice-on-june-25"></a>6 月 25 日の仕入先請求書
6年7月25日に、4年は、仕入先3064に対して1,000.00の請求書を入力して転記します。 エイプリルはこのトランザクションを**仕入先トランザクション**ページで表示できます。
...
|------|-------------------|-----------|---------|-----------|-----------|---------|--------------------------------|----------|------------------|
|      | 標準            | Inv-10010 | 3064    | 6/25/2015 | 7/25/2015 | 10010   | 1,000.00                       | USD)       | 990.00           |
...
エイプリルは、買掛金勘定 [**支払仕訳帳**] ページを開くことで、この支払のための支払仕訳帳を作成できます。 [新しい仕訳帳を作成し、仕入先3064に対して行を入力します。 彼女は、従業員の決済の請求書をマークするため、**決済のトランザクション**ページを開きます。 さらに、請求書にマークしたり、[**決済金額**] フィールドの値を **-500.00** に変更したりします。 [**現金割引金額**] フィールドの値が全請求書に対して **-10.00** であり、[**適用する現金割引金額**] フィールドの値が **-5.05** であることを確認します。 したがって、April はこの請求書の -505.05 ドルを決済します。
...
エイプリルは [**トランザクションの決済**] ページを閉じます。 495.00 ドルの支払明細行が仕訳帳に作成され、April は仕訳帳を転記します。 4年は**仕入先トランザクション**ページの仕入先トランザクションを確認できます。 彼女は請求書に-500.00の残高があることを示します。 また、495.00 ドルの支払で、5.00 ドルの現金割引があることも確認できます。

| 伝票番号    | トランザクション タイプ | 日      | 請求書 | トランザクション通貨での借方金額 | トランザクション通貨での貸方金額 | 残高 | 通貨 |
|------------|------------------|-----------|---------|--------------------------------------|---------------------------------------|---------|----------|
| Inv-10010  | 請求書          | 6/25/2015 | 10010   |                                      | 1,000.00                              | -500.00 | USD)       |
...
| 伝票番号    | トランザクション タイプ | 日      | 請求書 | トランザクション通貨での借方金額 | トランザクション通貨での貸方金額 | 残高 | 通貨 |
|------------|------------------|-----------|---------|--------------------------------------|---------------------------------------|---------|----------|
| Inv-10010  | 請求書          | 6/25/2015 | 10010   |                                      | 1,000.00                              | 0.00    | USD)       |
| APP-10010  |  支払         | 2015/7/1  |         | 495.00                               |                                       | 0.00    | USD)       |
| DISC-10010 | 現金割引    | 2015/7/1  |         | 5.00                                 |                                       | 0.00    | USD)       |
