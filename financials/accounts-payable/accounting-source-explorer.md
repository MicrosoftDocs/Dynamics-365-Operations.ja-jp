...
会計のソース エクスプローラーは、ソース情報を表示する新しいページです。 スタンドアロン ツールとして会計ソース エクスプローラを使用するか、または総勘定元帳エントリの後の詳細を分析できます。 たとえば、変更の残高の残高または伝票トランザクションの最も詳細ソース情報を取得する、会計ソース エクスプローラを使用できます。 これにより、MS Excel へのエクスポート機能を使用して、Microsoft Excel で情報をより詳しく分析できます (たとえば、ピボット テーブルで、またはピボット テーブルのレポートで)。
...
会計配分と、該当すると、プロジェクト会計の配分に基づいてソース ドキュメントの説明、ソース エクスプローラ[追加情報を使用するすべてのドキュメントについて。 この情報は、金額のタイプ、プロジェクト、活動、カテゴリ、および明細行プロパティが含まれます。 次に実行できる分析の例を示します。
