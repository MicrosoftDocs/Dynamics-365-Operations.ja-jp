...
-   トニーは、5 台の CNC Milicron Machine の要求を入力します。 Fabrikam の購買注文係のアリシアは、品目を供給する法人 Contoso に対して発注書を発行します。
    ...
    |-----------------------------|----------|------------|------------|---------------------|---------------|
    | 1500 – CNC Milicronの機械 | 5        | 8,000.00   | 40,000.00  | 出荷および処理 | 3,000.00      |
...
2.  エイプリルは、Fabrikam の買掛金勘定コーディネーターであり、Contoso から送信されてくる請求書を入力および確認します。 彼女は、次の情報を確認します。
    ...
    -   対面または三方一致を必要とする品目の場合は、請求明細行の価格がOperations.ThisのMicrosoft Dynamics 365に表示される価格の照合次のタイプを定義する許容範囲内にあります:
...
| 品目                        | 梱包明細から | 単価 | 正味金額 |
|-----------------------------|----------|------------|------------|
| 1500 – CNC Milicronの機械 | 5        | 8,100.00   | 40,500.00  |
| 出荷および取扱い       |          |            | 4,000.00   |
| 税申告                         |          |            | 0.00       |
| 小計                       |          |            | 44,500.00  |

、Microsoft Dynamics 365で、請求明細行は、次の情報が含まれます。

| 品目番号                 | 件数 | 単価 | 明細行の正味金額 | 照合ポリシー    | 製品受領書の数量の照合 | 価格照合 | 価格合計の照合 |
|-----------------------------|----------|------------|-----------------|--------------------|--------------------------------|-------------|-------------------|
| 1500 – CNC Milicronの機械 | 5        | 8,100.00   | 40,500.00       | スリーウェイ マッチング | 成功                         | 成功      | 成功            |
...
2.  エイプリルは、Fabrikam の買掛金勘定コーディネーターであり、Contoso から送信されてくる請求書を入力および確認します。 彼女は、次の情報を確認します。
    -   スリーウェイ マッチングを必要とする品目について、請求明細行の数量は入庫済数量と一致する。 入庫済数量は、請求書と照合する製品受領書に表示されます。
    -   対面または三方一致を必要とする品目の場合は、請求明細行の価格は、Microsoft Dynamics 365で定義された許容範囲内にあります。 これには、次のタイプの価格照合が含まれます。
...
詳細については、" [一致する買掛金請求書] (勘定支払請求matching.md)。
