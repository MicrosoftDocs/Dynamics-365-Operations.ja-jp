...
場合によっては、仕入先支払の転記後に、支払を取り消す必要があります。 取消は、支払の削除、無効化、否認とは異なります。 ステータスが**作成済**の場合にのみ支払を削除できます。 このステータスは、支払が作成済で、まだ (生成されていないことを。 この制限は支払方法に関係なく、常に適用されます。 生成されたが、後で転記される前に未転記の小切手を無効にすることができます。 生成された支払が電子資金決済(EFT)として実行される場合は、転記する前に支払を拒否できます。 支払を否認するには、**支払ステータス**値を変更します。 無効になったか、または拒否された支払は**支払ステータス**値に戻った後に再生成できます。**なし**。 

支払が転記された後、取消が使用されます。 電子的に行われる支払が転記された後で取り消すことはできません。 代わり仕入先の勘定の職責を取り戻すは、新しいトランザクションが支払金額に作成する必要があります。 転記済小切手を取り消す場合の二つの方法があります。 1 つ目の方法では、**小切手**ページで**支払取消**をクリックすることで、取消はすぐに転記されます。 もう 1 つの方法では、**小切手** ページの**支払取消**をクリックすることで、取消が [現金および銀行管理] の小切手取消仕訳帳に送信され、そこでレビュー担当者が取消を転記または否認します。 
...
> [!NOTE]
> 仕訳帳を削除すると、取消がシステムから削除されますが、元の小切手を**チェック**ページになります。 小切手のステータスは**取消の保留中**ではなくなります。
