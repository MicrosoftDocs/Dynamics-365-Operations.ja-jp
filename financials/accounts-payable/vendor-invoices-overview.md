...
発注書からの仕入先請求書は、仕入先に配置された発注書に従って製品またはサービスを受けるときに生成される請求書です。 仕入先請求書が品目またはサービスのヘッダーと一つ以上の行が含まれています。 仕入先請求書は、発注書の製品の入荷仕入先請求書にサイクルを完了します。 
...
-   仕入先請求書の登録の見越計上できる経費、発注書を参照する請求書を入力することをすぐに有効にします。 仕入先請求書承認仕訳帳を使用して、それらの請求書を選択して、仕入先残高に見越計上を逆にする転記できます。
...
仕入先請求書に発注書に含まれていない新しい行を追加できます。 品目番号または調達カテゴリを選択する必要があります。 次に、明細行に数量、価格、および金額を追加できます。 明細行は、請求合計の照合ポリシーにのみ含まれます。
