...
-   **ソース** – このコンポーネントでは、ユーザーが配賦の入力元データを指定します。 配賦は元帳残高 (**データ ソース** = **元帳**) または固定額に基づいて設定することができます (**データ ソース** = **固定値**)。 **データ ソース**が**元帳**に設定されている場合、ソースのフィルタ条件は、元帳配賦ルールに対して定義する必要があります (たとえば、広告費)。
