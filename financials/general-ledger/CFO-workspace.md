...
このトピックでは、最高財務責任者 (CFO) ワークスペースに財務分析コードを追加し、それにより元帳および予算のレポートを使用できるようにする方法を説明します。 CFO ワークスペースには、**概要** タブおよび **財務** タブがあります。 これら 2 つのタブ上のレポートが 2 つの措置によってサポートされています: LedgerActivityMeasure および BudgetActivityMeasure。 Microsoft Dynamics 365 for Finance and Operations、Enterprise edition 2017 年 7 月の更新プログラムでは、それら 2 つの措置と DimensionCombinationEntity エンティティの間に関係があります。 したがって、分析コードを選択できます。
