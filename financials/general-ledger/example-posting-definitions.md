---
title: "転記の定義"
description: "この記事では、定義を転記する発注書をブロック支出と予算に対して使用方法を示す例を示します。"
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="posting-definition-examples"></a>転記定義例

この記事では、定義を転記する発注書をブロック支出と予算に対して使用方法を示す例を示します。
...
> [!NOTE]
> 特定のトランザクション タイプおよび転記定義を関連付けるために、**トランザクションの転記定義**ページを使用します。 トランザクション タイプと** **総勘定元帳のパラメータ**ページでは、定義を関連付けた、**転記定義を使用して、選択すると、選択したトランザクション タイプのすべてのトランザクションは、転記定義を使用する必要があります。
...
|--------------------------------|--------|--------|---------|
| 606400-OU\_1-OU\_の3566トレーニング | 250.00 |        |         |
...
| 口座 + 分析コード           | デビット  | クレジット | コメント |
|--------------------------------|--------|--------|---------|
| 300143-OU\_1-OU\_の3566トレーニング | 250.00 |        |         |
| 300144-OU\_1-OU\_の3566トレーニング |        | 250.00 |         |

この例では、勘定構造 - P&L の一部であるいずれかの口座が転記の定義の基準に一致します。 したがって、606500-OU\_1-OU\_3566トレーニングが評価されると、生成されたエントリは転記定義の**生成されたエントリ**ウィンドウで定義された勘定に対して作成されます。
...
| 口座 + 分析コード           | デビット  | クレジット | コメント |
|--------------------------------|--------|--------|---------|
| 300145-OU\_1-OU\_の3566トレーニング |        | 250.00 |         |
| 300146-OU\_1-OU\_の3566トレーニング | 250.00 |        |         |

この例では、勘定構造 - P&L の一部であるいずれかの口座が転記の定義の基準に一致します。 したがって、606400-OU\_1-OU\_3566トレーニングが評価されると、生成された元帳エントリが作成されます。
